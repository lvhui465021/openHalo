...
SET pg_stat_statements.untrack_dbnames = 'contrib_regression';
CREATE TABLE pgss_test (id INT, name VARCHAR(30));
SELECT * FROM pgss_test;
SELECT query, calls FROM pg_stat_statements WHERE dbid = (SELECT oid FROM pg_database WHERE datname = 'contrib_regression')
AND query LIKE '%pgss_test%';
