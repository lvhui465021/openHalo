...
static char *pgss_untrack_dbnames = NULL;
static List *pgss_untrack_dbids_list = NIL;
static List *pgss_untrack_dbname_list = NIL;
...
static bool check_untrack_dbid(Oid dbid);
static void init_pgss_untrack_dbids(void);
static void assign_pgss_untrack_dbnames(const char *newval, void *extra);
...
_PG_init(void)
{
	...
	DefineCustomStringVariable("pg_stat_statements.untrack_dbnames",
							   "Selects which databases are untracked by pg_stat_statements.",
							   NULL,
							   &pgss_untrack_dbnames,
							   "",
							   PGC_SUSET,
							   GUC_LIST_INPUT | GUC_NOT_IN_SAMPLE,
							   NULL,
							   assign_pgss_untrack_dbnames,
							   NULL);
	...
	registerPostInitHook((postInitHooks_hook_type) installHooks);
}
		   ...
		   JumbleState *jstate)
{
	...
	if (check_untrack_dbid(MyDatabaseId))
		return;
...
check_untrack_dbid(Oid dbid)
{
	...
	if (!pgss_untrack_dbids_list && 
			(pgss_untrack_dbname_list && list_length(pgss_untrack_dbname_list) > 0))
		init_pgss_untrack_dbids();

	if (pgss_untrack_dbids_list &&
			list_length(pgss_untrack_dbids_list) > 0)
		result = list_member_oid(pgss_untrack_dbids_list, dbid); 
...
static void
assign_pgss_untrack_dbnames(const char *newval, void *extra)
{
	List	 *elems;
	ListCell *l;
	char	 *rawstr;
	...
	if (newval == NULL || newval[0] == '\0')
	{
		if (pgss_untrack_dbids_list)
		{
			list_free(pgss_untrack_dbids_list);
		}

	    if (pgss_untrack_dbname_list)
	    {
			list_free(pgss_untrack_dbname_list);
		}

		pgss_untrack_dbname_list = NIL;
		pgss_untrack_dbids_list = NIL;
		return;
	}

	rawstr = pstrdup(newval);

	if (!SplitIdentifierString(rawstr, ',', &elems))
	{
		GUC_check_errdetail("Invalid untrack_dbnames value");
        list_free(elems);
        pfree(rawstr);
        return;
	}
	...
	foreach (l, elems)
	{
		...
		pgss_untrack_dbname_list = lappend(pgss_untrack_dbname_list, pstrdup(dbname));
	}
	MemoryContextSwitchTo(oldctx);
...
init_pgss_untrack_dbids(void)
{
	...
	if (pgss_untrack_dbname_list &&
			list_length(pgss_untrack_dbname_list) > 0)
	{
		ListCell	*l;

		pgss_untrack_dbids_list = NIL;
		foreach (l, pgss_untrack_dbname_list)
		{
			char	*dbname = (char *) lfirst(l);
			Oid     dbid = get_database_oid(dbname, true);

			if (dbid != InvalidOid)
				pgss_untrack_dbids_list = lappend_oid(pgss_untrack_dbids_list, dbid);
			else
				ereport(WARNING,
				    	(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			        	errmsg("invalid database name \"%s\"",
					       dbname)));
		}
	}
