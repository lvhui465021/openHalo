 ...
 * Some code is released under the terms of the PostgreSQL License.
 *
...
initialize_environment(void)
{
	...
	if (nolocale)
	{
		 ...
		 * Also, Halo does not support macOS with locale environment
...
help(void)
{
	printf(_("Halo regression test driver\n"));
	printf(_("\n"));
	printf(_("Usage:\n  %s [OPTION]... [EXTRA-TEST]...\n"), progname);
	printf(_("\n"));
	printf(_("Options:\n"));
	...
	while ((c = getopt_long(argc, argv, "hV", long_options, &option_index)) != -1)
	{
		switch (c)
		{
			case 'h':
				...
				exit(0);
			case 'V':
				puts("pg_regress (Halo) " PG_VERSION_PREFIX PG_VERSION);
				exit(0);
			case 1:
		...
		for (i = 0; i < 16; i++)
		{
			if (system(buf2) == 0)
			{
				...
				if (port_specified_by_user || i == 15)
				{
					...
					fprintf(stderr, _("Specify an unused port using the --port option or shut down any conflicting Halo servers.\n"));
					exit(2);
				}
