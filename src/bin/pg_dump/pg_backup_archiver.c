...
RestoreArchive(Archive *AHX)
{
	...
	if (AH->archiveRemoteVersion)
		ahprintf(AH, "-- Dumped from database version %s%s\n",
				 PG_VERSION_PREFIX, AH->archiveRemoteVersion);
	if (AH->archiveDumpVersion)
		ahprintf(AH, "-- Dumped by pg_dump version %s%s\n",
				 PG_VERSION_PREFIX, AH->archiveDumpVersion);
