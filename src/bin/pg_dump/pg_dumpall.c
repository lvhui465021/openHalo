...
#define PGDUMP_VERSIONSTR "pg_dump (Halo) " PG_VERSION_PREFIX PG_VERSION "\n"
...
main(int argc, char *argv[])
{
	static struct option long_options[] = {
		...
		{"on_suffixes", no_argument, &on_suffixes, 12},
	...
	if (argc > 1)
	{
		...
		{
			puts("pg_dumpall (Halo) " PG_VERSION_PREFIX PG_VERSION);
			exit_nicely(0);
		}
	}
	...
	while ((c = getopt_long(argc, argv, "acd:E:f:gh:l:Op:rsS:tU:vwWx", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 7:
				...
				break;
			case 12:
				appendPQExpBufferStr(pgdumpopts, " --on_suffixes ");
				appendShellString(pgdumpopts, optarg);
				break;
	...
	fprintf(OPF, "--\n-- Halo database cluster dump complete\n--\n\n");
...
help(void)
{
	printf(_("%s extracts a Halo database cluster into an SQL script file.\n\n"), progname);
	printf(_("Usage:\n"));
	printf(_("  %s [OPTION]...\n"), progname);
