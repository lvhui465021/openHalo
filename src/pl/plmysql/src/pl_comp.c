/*-------------------------------------------------------------------------
 *
 * pl_comp.c		- Compiler part of the PL/MySQL
 *			  procedural language
 *
 ...
 * ----------
 */
PLMySQL_stmt_block *plmysql_parse_result;

static int	datums_alloc;
int			plmysql_nDatums;
PLMySQL_datum **plmysql_Datums;
static int	datums_last;

char	   *plmysql_error_funcname;
bool		plmysql_DumpExecTree = false;
bool		plmysql_check_syntax = false;

PLMySQL_function *plmysql_curr_compile;

/* A context appropriate for short-term allocs during compilation */
MemoryContext plmysql_compile_tmp_cxt;

/* ----------
 * Hash table for compiled functions
 * ----------
 */
static HTAB *plmysql_HashTable = NULL;

typedef struct plmysql_hashent
{
	PLMySQL_func_hashkey key;
	PLMySQL_function *function;
} plmysql_HashEnt;
 ...
 * ----------
 */
static PLMySQL_function *do_compile(FunctionCallInfo fcinfo,
									HeapTuple procTup,
									PLMySQL_function *function,
									PLMySQL_func_hashkey *hashkey,
									bool forValidator);
static void plmysql_compile_error_callback(void *arg);
static void add_parameter_name(PLMySQL_nsitem_type itemtype, int itemno, const char *name);
static void add_dummy_return(PLMySQL_function *function);
static Node *plmysql_pre_column_ref(ParseState *pstate, ColumnRef *cref);
static Node *plmysql_post_column_ref(ParseState *pstate, ColumnRef *cref, Node *var);
static Node *plmysql_param_ref(ParseState *pstate, ParamRef *pref);
static Node *resolve_column_ref(ParseState *pstate, PLMySQL_expr *expr,
								ColumnRef *cref, bool error_if_no_field);
static Node *make_datum_param(PLMySQL_expr *expr, int dno, int location);
static PLMySQL_row *build_row_from_vars(PLMySQL_variable **vars, int numvars);
static PLMySQL_type *build_datatype(HeapTuple typeTup, int32 typmod,
									Oid collation, TypeName *origtypname);
static void plmysql_start_datums(void);
static void plmysql_finish_datums(PLMySQL_function *function);
static void compute_function_hashkey(FunctionCallInfo fcinfo,
									 Form_pg_proc procStruct,
									 PLMySQL_func_hashkey *hashkey,
									 bool forValidator);
static void plmysql_resolve_polymorphic_argtypes(int numargs,
												 Oid *argtypes, char *argmodes,
												 Node *call_expr, bool forValidator,
												 const char *proname);
static PLMySQL_function *plmysql_HashTableLookup(PLMySQL_func_hashkey *func_key);
static void plmysql_HashTableInsert(PLMySQL_function *function,
									PLMySQL_func_hashkey *func_key);
static void plmysql_HashTableDelete(PLMySQL_function *function);
static void delete_function(PLMySQL_function *func);

/* ----------
 * plmysql_compile		Make an execution tree for a PL/MySQL function.
 *
 ...
 */
PLMySQL_function *
plmysql_compile(FunctionCallInfo fcinfo, bool forValidator)
{
	 ...
	 */
	function = (PLMySQL_function *) fcinfo->flinfo->fn_extra;
...
recheck:
	if (!function)
	{
		...
		/* And do the lookup */
		function = plmysql_HashTableLookup(&hashkey);
	}
...
/*
 * This is the slow part of plmysql_compile().
 *
 ...
 * allocations is the plmysql_compile_tmp_cxt.
 *
 * NB: this code is not re-entrant.  We assume that nothing we do here could
 * result in the invocation of another plmysql function.
 */
static PLMySQL_function *
		   ...
		   bool forValidator)
{
	...
	PLMySQL_variable **out_arg_variables;
	...
	plmysql_scanner_init(proc_source);

	plmysql_error_funcname = pstrdup(NameStr(procStruct->proname));

	/*
	 * Setup error traceback support for ereport()
	 */
	plerrcontext.callback = plmysql_compile_error_callback;
	 ...
	 * reasons.
	 */
	plmysql_check_syntax = forValidator;
	...
	if (function == NULL)
	{
		function = (PLMySQL_function *)
			MemoryContextAllocZero(TopMemoryContext, sizeof(PLMySQL_function));
	}
	else
	{
		/* re-using a previously existing struct, so clear it out */
		memset(function, 0, sizeof(PLMySQL_function));
	}
	plmysql_curr_compile = function;
	...
	plmysql_compile_tmp_cxt = MemoryContextSwitchTo(func_cxt);
	...
	function->resolve_option = plmysql_variable_conflict;
	function->print_strict_params = plmysql_print_strict_params;
	/* only promote extra warnings and errors at CREATE FUNCTION time */
	function->extra_warnings = forValidator ? plmysql_extra_warnings : 0;
	function->extra_errors = forValidator ? plmysql_extra_errors : 0;

	if (is_dml_trigger)
		function->fn_is_trigger = PLMYSQL_DML_TRIGGER;
	else if (is_event_trigger)
		function->fn_is_trigger = PLMYSQL_EVENT_TRIGGER;
	else
		function->fn_is_trigger = PLMYSQL_NOT_TRIGGER;
	 ...
	 */
	plmysql_ns_init();
	plmysql_ns_push(NameStr(procStruct->proname), PLMYSQL_LABEL_BLOCK);
	plmysql_DumpExecTree = false;
	plmysql_start_datums();

	switch (function->fn_is_trigger)
	{
		case PLMYSQL_NOT_TRIGGER:
			 ...
			 */
			MemoryContextSwitchTo(plmysql_compile_tmp_cxt);

			numargs = get_func_arg_info(procTup,
										&argtypes, &argnames, &argmodes);

			plmysql_resolve_polymorphic_argtypes(numargs, argtypes, argmodes,
												 fcinfo->flinfo->fn_expr,
												 forValidator,
												 plmysql_error_funcname);

			in_arg_varnos = (int *) palloc(numargs * sizeof(int));
			out_arg_variables = (PLMySQL_variable **) palloc(numargs * sizeof(PLMySQL_variable *));
			...
			for (i = 0; i < numargs; i++)
			{
				...
				PLMySQL_type *argdtype;
				PLMySQL_variable *argvariable;
				PLMySQL_nsitem_type argitemtype;

				/* Create $n name for variable */
				snprintf(buf, sizeof(buf), "$%d", i + 1);

				/* Create datatype info */
				argdtype = plmysql_build_datatype(argtypeid,
												  -1,
				...
				if (argdtype->ttype == PLMYSQL_TTYPE_PSEUDO)
					ereport(ERROR,
				 ...
				 */
				argvariable = plmysql_build_variable((argnames &&
													  argnames[i][0] != '\0') ?
													 argnames[i] : buf,
													 0, argdtype, false);

				if (argvariable->dtype == PLMYSQL_DTYPE_VAR)
				{
					argitemtype = PLMYSQL_NSTYPE_VAR;
				}
				else
				{
					Assert(argvariable->dtype == PLMYSQL_DTYPE_REC);
					argitemtype = PLMYSQL_NSTYPE_REC;
				}
			...
			{
				PLMySQL_row *row = build_row_from_vars(out_arg_variables,
													   num_out_args);
			...
			if (IsPolymorphicType(procStruct->prorettype) &&
				num_out_args == 0)
			{
				(void) plmysql_build_variable("$0", 0,
		...
		case PLMYSQL_DML_TRIGGER:
			...
			rec = plmysql_build_record("new", 0, NULL, RECORDOID, true);
			function->new_varno = rec->dno;

			/* Add the record for referencing OLD ROW */
			rec = plmysql_build_record("old", 0, NULL, RECORDOID, true);
			function->old_varno = rec->dno;

			/* Add the variable tg_name */
			var = plmysql_build_variable("tg_name", 0,
										 plmysql_build_datatype(NAMEOID,
																-1,
																function->fn_input_collation,
																NULL),
										 true);
			...
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_NAME;

			/* Add the variable tg_when */
			var = plmysql_build_variable("tg_when", 0,
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_WHEN;

			/* Add the variable tg_level */
			var = plmysql_build_variable("tg_level", 0,
										 plmysql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_LEVEL;

			/* Add the variable tg_op */
			var = plmysql_build_variable("tg_op", 0,
										 plmysql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_OP;

			/* Add the variable tg_relid */
			var = plmysql_build_variable("tg_relid", 0,
										 plmysql_build_datatype(OIDOID,
																-1,
																InvalidOid,
																NULL),
										 true);
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_RELID;

			/* Add the variable tg_relname */
			var = plmysql_build_variable("tg_relname", 0,
										 plmysql_build_datatype(NAMEOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_TABLE_NAME;

			/* tg_table_name is now preferred to tg_relname */
			var = plmysql_build_variable("tg_table_name", 0,
										 plmysql_build_datatype(NAMEOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			var = plmysql_build_variable("tg_table_schema", 0,
										 plmysql_build_datatype(NAMEOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_TABLE_SCHEMA;

			/* Add the variable tg_nargs */
			var = plmysql_build_variable("tg_nargs", 0,
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_NARGS;

			/* Add the variable tg_argv */
			var = plmysql_build_variable("tg_argv", 0,
										 plmysql_build_datatype(TEXTARRAYOID,
																-1,
																function->fn_input_collation,
																NULL),
										 true);
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_ARGV;

			break;

		case PLMYSQL_EVENT_TRIGGER:
			...
			var = plmysql_build_variable("tg_event", 0,
										 plmysql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_EVENT;

			/* Add the variable tg_tag */
			var = plmysql_build_variable("tg_tag", 0,
										 plmysql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation,
																NULL),
			...
			Assert(var->dtype == PLMYSQL_DTYPE_VAR);
			var->dtype = PLMYSQL_DTYPE_PROMISE;
			((PLMySQL_var *) var)->promise = PLMYSQL_PROMISE_TG_TAG;
	 ...
	 */
	var = plmysql_build_variable("found", 0,
								 plmysql_build_datatype(BOOLOID,
														-1,
														InvalidOid,
														NULL),
								 true);
	...
	function->action = plmysql_parse_result;

	plmysql_scanner_finish();
	pfree(proc_source);
	...
	plmysql_finish_datums(function);

	/* Debug dump for completed functions */
	if (plmysql_DumpExecTree)
		plmysql_dumptree(function);

	/*
	 * add it to the hash table
	 */
	plmysql_HashTableInsert(function, hashkey);

	/*
	 * Pop the error context stack
	 */
	error_context_stack = plerrcontext.previous;
	plmysql_error_funcname = NULL;

	plmysql_check_syntax = false;

	MemoryContextSwitchTo(plmysql_compile_tmp_cxt);
	plmysql_compile_tmp_cxt = NULL;
	return function;
}

/* ----------
 * plmysql_compile_inline	Make an execution tree for an anonymous code block.
 *
 ...
 */
PLMySQL_function *
plmysql_compile_inline(char *proc_source)
{
	...
	plmysql_error_funcname = func_name;
	...
	plmysql_check_syntax = check_function_bodies;

	/* Function struct does not live past current statement */
	function = (PLMySQL_function *) palloc0(sizeof(PLMySQL_function));
	...
	plmysql_ns_init();
	plmysql_ns_push(func_name, PLMYSQL_LABEL_BLOCK);
	plmysql_DumpExecTree = false;
	plmysql_start_datums();
 ...
 */
static void
plmysql_compile_error_callback(void *arg)
{
	if (arg)
	{
		/*
	...
	if (plmysql_error_funcname)
		errcontext("compilation of PL/MySQL function \"%s\" near line %d",
				   plmysql_error_funcname, plmysql_latest_lineno());
}
 ...
 */
static void
add_parameter_name(PLMySQL_nsitem_type itemtype, int itemno, const char *name)
{
	/*
	 ...
	 * plmysql, such names are in the same namespace, so there is no way to
	 * disambiguate.
	 */
	...
	/* OK, add the name */
	plmysql_ns_additem(itemtype, itemno, name);
}
 ...
 */
static void
add_dummy_return(PLMySQL_function *function)
{
	/*
	...
	if (function->action->body == NIL ||
		((PLMySQL_stmt *) llast(function->action->body))->cmd_type != PLMYSQL_STMT_RETURN)
	{
...
/*
 * plmysql_parser_setup		set up parser hooks for dynamic parameters
 *
 * Note: this routine, and the hook functions it prepares for, are logically
 * part of plmysql parsing.  But they actually run during function execution,
 * when we are ready to evaluate a SQL query or expression that has not
 * previously been parsed and planned.
 */
void
plmysql_parser_setup(struct ParseState *pstate, PLMySQL_expr *expr)
{
	pstate->p_pre_columnref_hook = plmysql_pre_column_ref;
	pstate->p_post_columnref_hook = plmysql_post_column_ref;
	pstate->p_paramref_hook = plmysql_param_ref;
	/* no need to use p_coerce_param_hook */
	pstate->p_ref_hook_state = (void *) expr;
}

/*
 * plmysql_pre_column_ref		parser callback before parsing a ColumnRef
 */
static Node *
plmysql_pre_column_ref(ParseState *pstate, ColumnRef *cref)
{
	PLMySQL_expr *expr = (PLMySQL_expr *) pstate->p_ref_hook_state;

	if (expr->func->resolve_option == PLMYSQL_RESOLVE_VARIABLE)
...
/*
 * plmysql_post_column_ref		parser callback after parsing a ColumnRef
 */
static Node *
plmysql_post_column_ref(ParseState *pstate, ColumnRef *cref, Node *var)
{
	PLMySQL_expr *expr = (PLMySQL_expr *) pstate->p_ref_hook_state;
	Node	   *myvar;
	...
	if (expr->func->resolve_option == PLMYSQL_RESOLVE_COLUMN && var != NULL)
	...
	if (myvar != NULL && var != NULL)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_AMBIGUOUS_COLUMN),
				 ...
				 errdetail("It could refer to either a PL/MySQL variable or a table column."),
...
/*
 * plmysql_param_ref		parser callback for ParamRefs ($n symbols)
 */
static Node *
plmysql_param_ref(ParseState *pstate, ParamRef *pref)
{
	PLMySQL_expr *expr = (PLMySQL_expr *) pstate->p_ref_hook_state;
	char		pname[32];
	PLMySQL_nsitem *nse;
	...
	nse = plmysql_ns_lookup(expr->ns, false,
							pname, NULL, NULL,
							NULL);

	if (nse == NULL)
		return NULL;			/* name not known to plmysql */

	return make_datum_param(expr, nse->itemno, pref->location);
}

/*
 * resolve_column_ref		attempt to resolve a ColumnRef as a plmysql var
 *
 ...
 */
static Node *
resolve_column_ref(ParseState *pstate, PLMySQL_expr *expr,
				   ColumnRef *cref, bool error_if_no_field)
{
	...
	switch (nse->itemtype)
	{
		...
		case PLMYSQL_NSTYPE_REC:
			...
			if (nnames == nnames_field)
			{
				...
				while (i >= 0)
				{
					PLMySQL_recfield *fld = (PLMySQL_recfield *) estate->datums[i];

					Assert(fld->dtype == PLMYSQL_DTYPE_RECFIELD &&
			...
			break;
		default:
			elog(ERROR, "unrecognized plmysql itemtype: %d", nse->itemtype);
	}

	/* Name format doesn't match the plmysql variable type */
	return NULL;
}

/*
 * Helper for columnref parsing: build a Param referencing a plmysql datum,
 * and make sure that that datum is listed in the expression's paramnos.
 */
static Node *
make_datum_param(PLMySQL_expr *expr, int dno, int location)
{
	PLMySQL_execstate *estate;
	PLMySQL_datum *datum;
	Param	   *param;
	...
	param->paramid = dno + 1;
	plmysql_exec_get_datum_type_info(estate,
									 datum,
									 &param->paramtype,
									 &param->paramtypmod,
									 &param->paramcollid);
...
/* ----------
 * plmysql_parse_word		The scanner calls this to postparse
 ...
 * and the plmysql_IdentifierLookup context permits it.
 *
 ...
 */
bool
plmysql_parse_word(char *word1, const char *yytxt, bool lookup,
				   PLwdatum *wdatum, PLword *word)
{
	 ...
	 */
	if (lookup && plmysql_IdentifierLookup == IDENTIFIER_LOOKUP_NORMAL)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
		ns = plmysql_ns_lookup(plmysql_ns_top(), false,
							   word1, NULL, NULL,
							   NULL);
		...
		if (ns != NULL)
		{
			switch (ns->itemtype)
			{
				case PLMYSQL_NSTYPE_VAR:
				case PLMYSQL_NSTYPE_REC:
					wdatum->datum = plmysql_Datums[ns->itemno];
					wdatum->ident = word1;
				...
				default:
					/* plmysql_ns_lookup should never return anything else */
					elog(ERROR, "unrecognized plmysql itemtype: %d",
						 ns->itemtype);
			}
		}
	}
...
/* ----------
 * plmysql_parse_dblword		Same lookup for two words
 *					separated by a dot.
 * ----------
 */
bool
plmysql_parse_dblword(char *word1, char *word2,
					  PLwdatum *wdatum, PLcword *cword)
{
	 ...
	 */
	if (plmysql_IdentifierLookup != IDENTIFIER_LOOKUP_DECLARE)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
			...
			switch (ns->itemtype)
			{
				...
				case PLMYSQL_NSTYPE_REC:
					if (nnames == 1)
					{
						 ...
						 */
						PLMySQL_rec *rec;
						PLMySQL_recfield *new;

						rec = (PLMySQL_rec *) (plmysql_Datums[ns->itemno]);
						new = plmysql_build_recfield(rec, word2);

						wdatum->datum = (PLMySQL_datum *) new;
					}
					else
					{
...
/* ----------
 * plmysql_parse_tripword		Same lookup for three words
 *					separated by dots.
 * ----------
 */
bool
plmysql_parse_tripword(char *word1, char *word2, char *word3,
					   PLwdatum *wdatum, PLcword *cword)
{
	PLMySQL_nsitem *ns;
	List	   *idents;
				...
				case PLMYSQL_NSTYPE_REC:
					{
						...
						{
							/* Block-qualified reference to record variable. */
							new = plmysql_build_recfield(rec, word3);
...
/* ----------
 * plmysql_parse_wordtype	The scanner found word%TYPE. word can be
 ...
 */
PLMySQL_type *
plmysql_parse_wordtype(char *ident)
{
	PLMySQL_type *dtype;
	PLMySQL_nsitem *nse;
	TypeName   *typeName;
	 ...
	 * Do a lookup in the current namespace stack
	 */
	nse = plmysql_ns_lookup(plmysql_ns_top(), false,
							ident, NULL, NULL,
							NULL);
	...
	if (nse != NULL)
	{
		...
		{
			case PLMYSQL_NSTYPE_VAR:
				return ((PLMySQL_var *) (plmysql_Datums[nse->itemno]))->datatype;
...
/* ----------
 * plmysql_parse_cwordtype		Same lookup for compositeword%TYPE
 * ----------
 */
PLMySQL_type *
plmysql_parse_cwordtype(List *idents)
{
	PLMySQL_type *dtype = NULL;
	PLMySQL_nsitem *nse;
	const char *fldname;
	Oid			classOid;
	...
	if (list_length(idents) == 2)
	{
		...
		if (nse != NULL && nse->itemtype == PLMYSQL_NSTYPE_VAR)
		{
			dtype = ((PLMySQL_var *) (plmysql_Datums[nse->itemno]))->datatype;
			goto done;
		}
...
/* ----------
 * plmysql_parse_wordrowtype		Scanner found word%ROWTYPE.
 *					So word must be a table name.
 * ----------
 */
PLMySQL_type *
plmysql_parse_wordrowtype(char *ident)
{
	Oid			classOid;
	Oid			typOid;
	...
	return plmysql_build_datatype(typOid, -1, InvalidOid,
								  makeTypeName(ident));
}

/* ----------
 * plmysql_parse_cwordrowtype		Scanner found compositeword%ROWTYPE.
 *			So word must be a namespace qualified table name.
 * ----------
 */
PLMySQL_type *
plmysql_parse_cwordrowtype(List *idents)
{
	Oid			classOid;
	Oid			typOid;
	RangeVar   *relvar;
...
/*
 * plmysql_build_variable - build a datum-array entry of a given
 * datatype
 *
 * The returned struct may be a PLMySQL_var or PLMySQL_rec
 * depending on the given datatype, and is allocated via
 * palloc.  The struct is automatically added to the current datum
 * array, and optionally to the current namespace.
 */
PLMySQL_variable *
plmysql_build_variable(const char *refname, int lineno, PLMySQL_type *dtype,
					   bool add2namespace)
{
	PLMySQL_variable *result;

	switch (dtype->ttype)
	{
		case PLMYSQL_TTYPE_SCALAR:
			{
				/* Ordinary scalar datatype */
				PLMySQL_var *var;

				var = palloc0(sizeof(PLMySQL_var));
				var->dtype = PLMYSQL_DTYPE_VAR;
				...
				plmysql_adddatum((PLMySQL_datum *) var);
				if (add2namespace)
					plmysql_ns_additem(PLMYSQL_NSTYPE_VAR,
									   var->dno,
									   refname);
				result = (PLMySQL_variable *) var;
				break;
			}
		case PLMYSQL_TTYPE_REC:
			{
				/* Composite type -- build a record variable */
				PLMySQL_rec *rec;

				rec = plmysql_build_record(refname, lineno,
										   dtype, dtype->typoid,
										   add2namespace);
				result = (PLMySQL_variable *) rec;
				break;
			}
		case PLMYSQL_TTYPE_PSEUDO:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
 ...
 */
PLMySQL_rec *
plmysql_build_record(const char *refname, int lineno,
					 PLMySQL_type *dtype, Oid rectypeid,
					 bool add2namespace)
{
	PLMySQL_rec *rec;

	rec = palloc0(sizeof(PLMySQL_rec));
	rec->dtype = PLMYSQL_DTYPE_REC;
	...
	rec->firstfield = -1;
	rec->erh = NULL;
	plmysql_adddatum((PLMySQL_datum *) rec);
	if (add2namespace)
		plmysql_ns_additem(PLMYSQL_NSTYPE_REC, rec->dno, rec->refname);
 ...
 */
static PLMySQL_row *
build_row_from_vars(PLMySQL_variable **vars, int numvars)
{
	PLMySQL_row *row;
	int			i;
	...
	for (i = 0; i < numvars; i++)
	{
		...
		switch (var->dtype)
		{
			...
			case PLMYSQL_DTYPE_PROMISE:
				typoid = ((PLMySQL_var *) var)->datatype->typoid;
				typmod = ((PLMySQL_var *) var)->datatype->atttypmod;
				typcoll = ((PLMySQL_var *) var)->datatype->collation;
				break;

			case PLMYSQL_DTYPE_REC:
				/* shouldn't need to revalidate rectypeid already... */
				typoid = ((PLMySQL_rec *) var)->rectypeid;
 ...
 */
PLMySQL_recfield *
plmysql_build_recfield(PLMySQL_rec *rec, const char *fldname)
{
	...
	while (i >= 0)
	{
		PLMySQL_recfield *fld = (PLMySQL_recfield *) plmysql_Datums[i];
	...
	recfield = palloc0(sizeof(PLMySQL_recfield));
	recfield->dtype = PLMYSQL_DTYPE_RECFIELD;
	recfield->fieldname = pstrdup(fldname);
	recfield->recparentno = rec->dno;
	recfield->rectupledescid = INVALID_TUPLEDESC_IDENTIFIER;

	plmysql_adddatum((PLMySQL_datum *) recfield);
...
/*
 * plmysql_build_datatype
 *		Build PLMySQL_type struct given type OID, typmod, collation,
 ...
 */
PLMySQL_type *
plmysql_build_datatype(Oid typeOid, int32 typmod,
					   Oid collation, TypeName *origtypname)
{
	HeapTuple	typeTup;
	PLMySQL_type *typ;
...
/*
 * Utility subroutine to make a PLMySQL_type struct given a pg_type entry
 * and additional details (see comments for plmysql_build_datatype).
 */
static PLMySQL_type *
			   ...
			   Oid collation, TypeName *origtypname)
{
	...
	typ = (PLMySQL_type *) palloc(sizeof(PLMySQL_type));
	...
	switch (typeStruct->typtype)
	{
		...
		case TYPTYPE_MULTIRANGE:
			typ->ttype = PLMYSQL_TTYPE_SCALAR;
			break;
		case TYPTYPE_COMPOSITE:
			typ->ttype = PLMYSQL_TTYPE_REC;
			break;
		case TYPTYPE_DOMAIN:
		...
		case TYPTYPE_PSEUDO:
			if (typ->typoid == RECORDOID)
				...
				typ->ttype = PLMYSQL_TTYPE_PSEUDO;
			break;
		default:
	 ...
	 */
	if (typ->ttype == PLMYSQL_TTYPE_REC && typ->typoid != RECORDOID)
	{
...
/*
 *	plmysql_recognize_err_condition
 ...
 */
int
plmysql_recognize_err_condition(const char *condname, bool allow_sqlstate)
{
	int			i;
 ...
 */
static void
plmysql_start_datums(void)
{
	datums_alloc = 128;
	plmysql_nDatums = 0;
	/* This is short-lived, so needn't allocate in function's cxt */
	plmysql_Datums = MemoryContextAlloc(plmysql_compile_tmp_cxt,
										sizeof(PLMySQL_datum *) * datums_alloc);
	/* datums_last tracks what's been seen by plmysql_add_initdatums() */
	datums_last = 0;
}

/* ----------
 * plmysql_adddatum			Add a variable, record or row
 *					to the compiler's datum list.
 * ----------
 */
void
plmysql_adddatum(PLMySQL_datum *newdatum)
{
	if (plmysql_nDatums == datums_alloc)
	{
		datums_alloc *= 2;
		plmysql_Datums = repalloc(plmysql_Datums, sizeof(PLMySQL_datum *) * datums_alloc);
	}

	newdatum->dno = plmysql_nDatums;
	plmysql_Datums[plmysql_nDatums++] = newdatum;
}

/* ----------
 * plmysql_finish_datums	Copy completed datum info into function struct.
 * ----------
 */
static void
plmysql_finish_datums(PLMySQL_function *function)
{
	Size		copiable_size = 0;
	int			i;

	function->ndatums = plmysql_nDatums;
	function->datums = palloc(sizeof(PLMySQL_datum *) * plmysql_nDatums);
	for (i = 0; i < plmysql_nDatums; i++)
	{
		function->datums[i] = plmysql_Datums[i];

		/* This must agree with copy_plmysql_datums on what is copiable */
		switch (function->datums[i]->dtype)
		{
			case PLMYSQL_DTYPE_VAR:
			case PLMYSQL_DTYPE_PROMISE:
				copiable_size += MAXALIGN(sizeof(PLMySQL_var));
				break;
			case PLMYSQL_DTYPE_REC:
				copiable_size += MAXALIGN(sizeof(PLMySQL_rec));
				break;
			default:
				break;
		}
...
/* ----------
 * plmysql_add_initdatums		Make an array of the datum numbers of
 ...
 */
int
plmysql_add_initdatums(int **varnos)
{
	int			i;
	int			n = 0;
	 ...
	 */
	for (i = datums_last; i < plmysql_nDatums; i++)
	{
		switch (plmysql_Datums[i]->dtype)
		{
			case PLMYSQL_DTYPE_VAR:
			case PLMYSQL_DTYPE_REC:
				n++;
						 ...
						 bool forValidator)
{
	...
	MemSet(hashkey, 0, sizeof(PLMySQL_func_hashkey));
	...
	if (procStruct->pronargs > 0)
	{
		...
		plmysql_resolve_polymorphic_argtypes(procStruct->pronargs,
											 hashkey->argtypes,
											 NULL,
											 fcinfo->flinfo->fn_expr,
											 forValidator,
 ...
 */
static void
plmysql_resolve_polymorphic_argtypes(int numargs,
									 Oid *argtypes, char *argmodes,
									 Node *call_expr, bool forValidator,
									 const char *proname)
{
 ...
 *
 * We can't release the PLMySQL_function struct itself, because of the
 ...
 */
static void
delete_function(PLMySQL_function *func)
{
	/* remove function from hash table (might be done already) */
	plmysql_HashTableDelete(func);
...
void
plmysql_HashTableInit(void)
{
	HASHCTL		ctl;

	/* don't allow double-initialization */
	Assert(plmysql_HashTable == NULL);

	ctl.keysize = sizeof(PLMySQL_func_hashkey);
	ctl.entrysize = sizeof(plmysql_HashEnt);
	plmysql_HashTable = hash_create("PLMySQL function hash",
									FUNCS_PER_USER,
									&ctl,
...
static PLMySQL_function *
plmysql_HashTableLookup(PLMySQL_func_hashkey *func_key)
{
	plmysql_HashEnt *hentry;

	hentry = (plmysql_HashEnt *) hash_search(plmysql_HashTable,
											 (void *) func_key,
											 HASH_FIND,
											 NULL);
	if (hentry)
...
static void
plmysql_HashTableInsert(PLMySQL_function *function,
						PLMySQL_func_hashkey *func_key)
{
	plmysql_HashEnt *hentry;
	bool		found;
...
static void
plmysql_HashTableDelete(PLMySQL_function *function)
{
	plmysql_HashEnt *hentry;
