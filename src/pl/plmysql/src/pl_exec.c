/*-------------------------------------------------------------------------
 *
 * pl_exec.c		- Executor for the PL/MySQL
 *			  procedural language
 *
...
/*
 * All plmysql function executions within a single transaction share the same
 ...
 * plmysql_inline_handler().  DO blocks still use the simple_econtext_stack,
 ...
 * EState will be cleaned up when we get back to plmysql_inline_handler.  This
...
/*
 * Memory management within a plmysql function generally works with three
 * contexts:
 *
 ...
 *
 * cast_expr_hash entries (of type plmysql_CastExprHashEntry) hold compiled
 * expression trees for casts.  These survive for the life of the session and
 * are shared across all PL/MySQL functions and DO blocks.  At some point it
 ...
 * plmysql_CastHashEntry) containing evaluation state trees for these
 ...
 * LOOP_RC_PROCESSING will do nothing except reset "rc" to PLMYSQL_RC_OK.
 *
...
#define LOOP_RC_PROCESSING(looplabel, exit_action) \
	if (rc == PLMYSQL_RC_RETURN) \
	{ \
		/* RETURN, so propagate RC_RETURN out */ \
		exit_action; \
	} \
	else if (rc == PLMYSQL_RC_EXIT) \
	{ \
	...
	else if (rc == PLMYSQL_RC_CONTINUE) \
	{ \
		if (estate->exitlabel == NULL) \
		{ \
		...
		Assert(rc == PLMYSQL_RC_OK)

/************************************************************
 * Local function forward declarations
 ************************************************************/
static void coerce_function_result_tuple(PLMySQL_execstate *estate,
										 TupleDesc tupdesc);
static void plmysql_exec_error_callback(void *arg);
static void copy_plmysql_datums(PLMySQL_execstate *estate,
...
static int	exec_stmts(PLMySQL_execstate *estate,
					   List *stmts);
static int	exec_stmt_assign(PLMySQL_execstate *estate,
							 PLMySQL_stmt_assign *stmt);
static int	exec_stmt_perform(PLMySQL_execstate *estate,
							  PLMySQL_stmt_perform *stmt);
static int	exec_stmt_call(PLMySQL_execstate *estate,
						   PLMySQL_stmt_call *stmt);
static int	exec_stmt_getdiag(PLMySQL_execstate *estate,
							  PLMySQL_stmt_getdiag *stmt);
static int	exec_stmt_if(PLMySQL_execstate *estate,
						 PLMySQL_stmt_if *stmt);
static int	exec_stmt_case(PLMySQL_execstate *estate,
						   PLMySQL_stmt_case *stmt);
static int	exec_stmt_loop(PLMySQL_execstate *estate,
...
static int	exec_stmt_exit(PLMySQL_execstate *estate,
						   PLMySQL_stmt_exit *stmt);
static int	exec_stmt_return(PLMySQL_execstate *estate,
							 PLMySQL_stmt_return *stmt);
static int	exec_stmt_return_next(PLMySQL_execstate *estate,
								  PLMySQL_stmt_return_next *stmt);
static int	exec_stmt_return_query(PLMySQL_execstate *estate,
								   PLMySQL_stmt_return_query *stmt);
static int	exec_stmt_raise(PLMySQL_execstate *estate,
							PLMySQL_stmt_raise *stmt);
static int	exec_stmt_assert(PLMySQL_execstate *estate,
							 PLMySQL_stmt_assert *stmt);
static int	exec_stmt_execsql(PLMySQL_execstate *estate,
...
static void exec_eval_cleanup(PLMySQL_execstate *estate);

static void exec_prepare_plan(PLMySQL_execstate *estate,
							  PLMySQL_expr *expr, int cursorOptions);
static void exec_simple_check_plan(PLMySQL_execstate *estate, PLMySQL_expr *expr);
static bool exec_is_simple_query(PLMySQL_expr *expr);
static void exec_save_simple_expr(PLMySQL_expr *expr, CachedPlan *cplan);
static void exec_check_rw_parameter(PLMySQL_expr *expr);
static bool exec_eval_simple_expr(PLMySQL_execstate *estate,
								  PLMySQL_expr *expr,
								  Datum *result,
								  bool *isNull,
								  Oid *rettype,
...
static void exec_assign_expr(PLMySQL_execstate *estate,
							 PLMySQL_datum *target,
							 PLMySQL_expr *expr);
static void exec_assign_c_string(PLMySQL_execstate *estate,
								 PLMySQL_datum *target,
								 const char *str);
static void exec_assign_value(PLMySQL_execstate *estate,
							  PLMySQL_datum *target,
							  Datum value, bool isNull,
							  Oid valtype, int32 valtypmod);
static void exec_eval_datum(PLMySQL_execstate *estate,
							PLMySQL_datum *datum,
							Oid *typeid,
							int32 *typetypmod,
							Datum *value,
							bool *isnull);
static int	exec_eval_integer(PLMySQL_execstate *estate,
							  PLMySQL_expr *expr,
							  bool *isNull);
static bool exec_eval_boolean(PLMySQL_execstate *estate,
							  PLMySQL_expr *expr,
							  bool *isNull);
static Datum exec_eval_expr(PLMySQL_execstate *estate,
							PLMySQL_expr *expr,
							bool *isNull,
							Oid *rettype,
							int32 *rettypmod);
...
static void plmysql_param_compile(ParamListInfo params, Param *param,
								  ExprState *state,
								  Datum *resv, bool *resnull);
static void plmysql_param_eval_var(ExprState *state, ExprEvalStep *op,
								   ExprContext *econtext);
static void plmysql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
									  ExprContext *econtext);
static void plmysql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
										ExprContext *econtext);
static void plmysql_param_eval_generic(ExprState *state, ExprEvalStep *op,
									   ExprContext *econtext);
static void plmysql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
										  ExprContext *econtext);
static void exec_move_row(PLMySQL_execstate *estate,
						  PLMySQL_variable *target,
						  HeapTuple tup, TupleDesc tupdesc);
static void revalidate_rectypeid(PLMySQL_rec *rec);
static ExpandedRecordHeader *make_expanded_record_for_rec(PLMySQL_execstate *estate,
														  PLMySQL_rec *rec,
														  TupleDesc srctupdesc,
														  ExpandedRecordHeader *srcerh);
static void exec_move_row_from_fields(PLMySQL_execstate *estate,
									  PLMySQL_variable *target,
									  ExpandedRecordHeader *newerh,
									  Datum *values, bool *nulls,
									  TupleDesc tupdesc);
static bool compatible_tupdescs(TupleDesc src_tupdesc, TupleDesc dst_tupdesc);
static HeapTuple make_tuple_from_row(PLMySQL_execstate *estate,
									 PLMySQL_row *row,
									 TupleDesc tupdesc);
static TupleDesc deconstruct_composite_datum(Datum value,
											 HeapTupleData *tmptup);
static void exec_move_row_from_datum(PLMySQL_execstate *estate,
									 PLMySQL_variable *target,
									 Datum value);
static void instantiate_empty_record_variable(PLMySQL_execstate *estate,
											  PLMySQL_rec *rec);
static char *convert_value_to_string(PLMySQL_execstate *estate,
									 Datum value, Oid valtype);
static inline Datum exec_cast_value(PLMySQL_execstate *estate,
									Datum value, bool *isnull,
									Oid valtype, int32 valtypmod,
									Oid reqtype, int32 reqtypmod);
static Datum do_cast_value(PLMySQL_execstate *estate,
						   Datum value, bool *isnull,
						   Oid valtype, int32 valtypmod,
						   Oid reqtype, int32 reqtypmod);
static plmysql_CastHashEntry *get_cast_hashentry(PLMySQL_execstate *estate,
												 Oid srctype, int32 srctypmod,
												 Oid dsttype, int32 dsttypmod);
static void exec_init_tuple_store(PLMySQL_execstate *estate);
static void exec_set_found(PLMySQL_execstate *estate, bool state);
static void plmysql_create_econtext(PLMySQL_execstate *estate);
static void plmysql_destroy_econtext(PLMySQL_execstate *estate);
static void assign_simple_var(PLMySQL_execstate *estate, PLMySQL_var *var,
							  Datum newvalue, bool isnull, bool freeable);
static void assign_text_var(PLMySQL_execstate *estate, PLMySQL_var *var,
							const char *str);
static void assign_record_var(PLMySQL_execstate *estate, PLMySQL_rec *rec,
							  ExpandedRecordHeader *erh);
static ParamListInfo exec_eval_using_params(PLMySQL_execstate *estate,
											List *params);
static Portal exec_dynquery_with_params(PLMySQL_execstate *estate,
										PLMySQL_expr *dynquery, List *params,
										const char *portalname, int cursorOptions);
static char *format_expr_params(PLMySQL_execstate *estate,
								const PLMySQL_expr *expr);
static char *format_preparedparamsdata(PLMySQL_execstate *estate,
									   ParamListInfo paramLI);
static PLMySQL_variable *make_callstmt_target(PLMySQL_execstate *estate,
											  PLMySQL_expr *expr);


/* ----------
 * plmysql_exec_function	Called by the call handler for
 *				function execution.
 *
 ...
 */
Datum
plmysql_exec_function(PLMySQL_function *func, FunctionCallInfo fcinfo,
					  EState *simple_eval_estate,
					  ResourceOwner simple_eval_resowner,
					  ResourceOwner procedure_resowner,
					  bool atomic)
{
	PLMySQL_execstate estate;
	 ...
	 */
	plmysql_estate_setup(&estate, func, (ReturnSetInfo *) fcinfo->resultinfo,
	 ...
	 */
	plerrcontext.callback = plmysql_exec_error_callback;
	...
	for (i = 0; i < func->fn_nargs; i++)
	{
		...
		switch (estate.datums[n]->dtype)
		{
			case PLMYSQL_DTYPE_VAR:
				{
					PLMySQL_var *var = (PLMySQL_var *) estate.datums[n];
			...
			case PLMYSQL_DTYPE_REC:
				{
					PLMySQL_rec *rec = (PLMySQL_rec *) estate.datums[n];

					if (!fcinfo->args[i].isnull)
					{
						/* Assign row value from composite datum */
						exec_move_row_from_datum(&estate,
												 (PLMySQL_variable *) rec,
												 fcinfo->args[i].value);
					}
					else
					{
						/* If arg is null, set variable to null */
						exec_move_row(&estate, (PLMySQL_variable *) rec,
									  NULL, NULL);
					}
	 ...
	 */
	if (*plmysql_plugin_ptr && (*plmysql_plugin_ptr)->func_beg)
		((*plmysql_plugin_ptr)->func_beg) (&estate, func);
	...
	else if (!estate.retisnull)
	{
		...
		if (estate.retistuple)
		{
			...
			else
			{
				 ...
				 * is in the PLMySQL_function struct, and that's too
				 * long-lived: composite types could change during the
				 * existence of a PLMySQL_function.
				 */
				Oid			resultTypeId;
				TupleDesc	tupdesc;
	 ...
	 * Let the instrumentation plugin peek at this function
	 */
	if (*plmysql_plugin_ptr && (*plmysql_plugin_ptr)->func_end)
		((*plmysql_plugin_ptr)->func_end) (&estate, func);

	/* Clean up any leftover temporary memory */
	plmysql_destroy_econtext(&estate);
	exec_eval_cleanup(&estate);
...
/*
 * Helper for plmysql_exec_function: coerce composite result to the specified
 ...
 */
static void
coerce_function_result_tuple(PLMySQL_execstate *estate, TupleDesc tupdesc)
{
	HeapTuple	rettup;
	TupleDesc	retdesc;
...
/* ----------
 * plmysql_exec_trigger		Called by the call handler for
 *				trigger execution.
 * ----------
 */
HeapTuple
plmysql_exec_trigger(PLMySQL_function *func,
					 TriggerData *trigdata)
{
	 ...
	 * Setup the execution state
	 */
	plmysql_estate_setup(&estate, func, NULL, NULL, NULL);
	...
	rec_new = (PLMySQL_rec *) (estate.datums[func->new_varno]);
	rec_old = (PLMySQL_rec *) (estate.datums[func->old_varno]);
...
/* ----------
 * plmysql_exec_event_trigger		Called by the call handler for
 *				event trigger execution.
 * ----------
 */
void
plmysql_exec_event_trigger(PLMySQL_function *func, EventTriggerData *trigdata)
{
	PLMySQL_execstate estate;
	ErrorContextCallback plerrcontext;
	int			rc;
 ...
 */
static void
plmysql_exec_error_callback(void *arg)
{
	PLMySQL_execstate *estate = (PLMySQL_execstate *) arg;
	...
	if (estate->err_text != NULL)
	{
		...
		if (estate->err_stmt != NULL)
		{
			 ...
			 */
			errcontext("PL/MySQL function %s line %d %s",
		...
		else
		{
			 ...
			 */
			errcontext("PL/MySQL function %s %s",
					   estate->func->fn_signature,
					   _(estate->err_text));
		}
	}
	...
	{
		/* translator: last %s is a plmysql statement type name */
		errcontext("PL/MySQL function %s line %d at %s",
				   estate->func->fn_signature,
				   estate->err_stmt->lineno,
				   plmysql_stmt_typename(estate->err_stmt));
	}
	else
		errcontext("PL/MySQL function %s",
				   estate->func->fn_signature);
}
 ...
 */
static void
copy_plmysql_datums(PLMySQL_execstate *estate,
					PLMySQL_function *func)
{
	int			ndatums = estate->ndatums;
	PLMySQL_datum **indatums;
	PLMySQL_datum **outdatums;
	char	   *workspace;
	char	   *ws_next;
	int			i;

	/* Allocate local datum-pointer array */
	estate->datums = (PLMySQL_datum **)
		palloc(sizeof(PLMySQL_datum *) * ndatums);
	...
	for (i = 0; i < ndatums; i++)
	{
		PLMySQL_datum *indatum = indatums[i];
		PLMySQL_datum *outdatum;

		/* This must agree with plmysql_finish_datums on what is copiable */
		switch (indatum->dtype)
		{
			case PLMYSQL_DTYPE_VAR:
			case PLMYSQL_DTYPE_PROMISE:
				outdatum = (PLMySQL_datum *) ws_next;
				memcpy(outdatum, indatum, sizeof(PLMySQL_var));
				ws_next += MAXALIGN(sizeof(PLMySQL_var));
				break;

			case PLMYSQL_DTYPE_REC:
				outdatum = (PLMySQL_datum *) ws_next;
				memcpy(outdatum, indatum, sizeof(PLMySQL_rec));
				ws_next += MAXALIGN(sizeof(PLMySQL_rec));
				break;
 ...
 */
static void
plmysql_fulfill_promise(PLMySQL_execstate *estate,
						PLMySQL_var *var)
{
	...
	switch (var->promise)
	{
		...
		case PLMYSQL_PROMISE_TG_TABLE_NAME:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
		...
		case PLMYSQL_PROMISE_TG_TABLE_SCHEMA:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
 ...
 */
static void
pop_stmt_mcontext(PLMySQL_execstate *estate)
{
 ...
 */
static bool
exception_matches_conditions(ErrorData *edata, PLMySQL_condition *cond)
{
...
plmysql_exec_block_initvars(PLMySQL_execstate *estate, PLMySQL_stmt_block *block)
{
	...
	for (i = 0; i < block->n_initvars; i++)
	{
		...
		PLMySQL_datum *datum = estate->datums[n];

		/*
		 * The set of dtypes handled here must match plmysql_add_initdatums().
		 *
		...
		switch (datum->dtype)
		{
			case PLMYSQL_DTYPE_VAR:
				{
					PLMySQL_var *var = (PLMySQL_var *) datum;
					...
					{
						exec_assign_expr(estate, (PLMySQL_datum *) var,
										 var->default_val);
					}
				}
				break;
			...
			case PLMYSQL_DTYPE_REC:
				{
					PLMySQL_rec *rec = (PLMySQL_rec *) datum;
					...
					{
						exec_assign_expr(estate, (PLMySQL_datum *) rec,
										 rec->default_val);
					}
				}
				break;
...
exec_stmt_block(PLMySQL_execstate *estate, PLMySQL_stmt_block *block)
{
	...
	if (block->exceptions)
	{
		...
		PG_CATCH();
		{
			...
			{
				PLMySQL_exception *exception = (PLMySQL_exception *) lfirst(e);
				...
				if (exception_matches_conditions(edata, exception->conditions))
				{
					 ...
					 */
					PLMySQL_var *state_var;
					PLMySQL_var *errm_var;

					state_var = (PLMySQL_var *)
						estate->datums[block->exceptions->sqlstate_varno];
					errm_var = (PLMySQL_var *)
 ...
 */
static int
exec_stmts(PLMySQL_execstate *estate, List *stmts)
{
	PLMySQL_stmt *save_estmt = estate->err_stmt;
	ListCell   *s;
	...
	foreach(s, stmts)
	{
		PLMySQL_stmt *stmt = (PLMySQL_stmt *) lfirst(s);
		int			rc;
...
exec_stmt(PLMySQL_execstate *estate, PLMySQL_stmt *stmt)
{
	...
	switch (stmt->cmd_type)
	{
			...
			case PLMYSQL_STMT_ASSIGN:
				rc = exec_stmt_assign(estate, (PLMySQL_stmt_assign *) stmt);
				break;

			case PLMYSQL_STMT_PERFORM:
				rc = exec_stmt_perform(estate, (PLMySQL_stmt_perform *) stmt);
				break;

			case PLMYSQL_STMT_CALL:
				rc = exec_stmt_call(estate, (PLMySQL_stmt_call *) stmt);
				break;

			case PLMYSQL_STMT_GETDIAG:
				rc = exec_stmt_getdiag(estate, (PLMySQL_stmt_getdiag *) stmt);
				break;

			case PLMYSQL_STMT_IF:
				rc = exec_stmt_if(estate, (PLMySQL_stmt_if *) stmt);
				break;

			case PLMYSQL_STMT_CASE:
				rc = exec_stmt_case(estate, (PLMySQL_stmt_case *) stmt);
				break;

			case PLMYSQL_STMT_LOOP:
				rc = exec_stmt_loop(estate, (PLMySQL_stmt_loop *) stmt);
				break;
			...
			case PLMYSQL_STMT_RETURN:
				rc = exec_stmt_return(estate, (PLMySQL_stmt_return *) stmt);
				break;

			case PLMYSQL_STMT_RETURN_NEXT:
				rc = exec_stmt_return_next(estate, (PLMySQL_stmt_return_next *) stmt);
				break;

			case PLMYSQL_STMT_RETURN_QUERY:
				rc = exec_stmt_return_query(estate, (PLMySQL_stmt_return_query *) stmt);
				break;
			...
			case PLMYSQL_STMT_EXECSQL:
				rc = exec_stmt_execsql(estate, (PLMySQL_stmt_execsql *) stmt);
				break;
			...
			case PLMYSQL_STMT_OPEN:
				rc = exec_stmt_open(estate, (PLMySQL_stmt_open *) stmt);
				break;

			case PLMYSQL_STMT_FETCH:
				rc = exec_stmt_fetch(estate, (PLMySQL_stmt_fetch *) stmt);
				break;

			case PLMYSQL_STMT_CLOSE:
				rc = exec_stmt_close(estate, (PLMySQL_stmt_close *) stmt);
				break;

			case PLMYSQL_STMT_COMMIT:
				rc = exec_stmt_commit(estate, (PLMySQL_stmt_commit *) stmt);
				break;
 ...
 */
static int
exec_stmt_assign(PLMySQL_execstate *estate, PLMySQL_stmt_assign *stmt)
{
	Assert(stmt->varno >= 0);
 ...
 */
static int
exec_stmt_perform(PLMySQL_execstate *estate, PLMySQL_stmt_perform *stmt)
{
	PLMySQL_expr *expr = stmt->expr;
 ...
 */
static int
exec_stmt_call(PLMySQL_execstate *estate, PLMySQL_stmt_call *stmt)
{
	PLMySQL_expr *expr = stmt->expr;
	...
	/*
	 * Also construct a DTYPE_ROW datum representing the plmysql variables
...
/*
 * We construct a DTYPE_ROW datum representing the plmysql variables
 * associated with the procedure's output arguments.  Then we can use
 * exec_move_row() to do the assignments.
 */
static PLMySQL_variable *
make_callstmt_target(PLMySQL_execstate *estate, PLMySQL_expr *expr)
{
	CachedPlan *cplan;
	PlannedStmt *pstmt;
	CallStmt   *stmt;
	...
	row = (PLMySQL_row *) palloc0(sizeof(PLMySQL_row));
	 ...
	 * an unadorned plmysql variable (Datum), which we can insert into the row
	 * Datum.
	 */
	nfields = 0;
	for (i = 0; i < numargs; i++)
 ...
 */
static int
exec_stmt_getdiag(PLMySQL_execstate *estate, PLMySQL_stmt_getdiag *stmt)
{
	ListCell   *lc;
	...
	foreach(lc, stmt->diag_items)
	{
		...
		PLMySQL_datum *var = estate->datums[diag_item->target];
 ...
 */
static int
exec_stmt_if(PLMySQL_execstate *estate, PLMySQL_stmt_if *stmt)
{
	bool		value;
	bool		isnull;
	ListCell   *lc;
	...
	{
		PLMySQL_if_elsif *elif = (PLMySQL_if_elsif *) lfirst(lc);
 ...
 */
static int
exec_stmt_case(PLMySQL_execstate *estate, PLMySQL_stmt_case *stmt)
{
	PLMySQL_var *t_var = NULL;
	bool		isnull;
	ListCell   *l;
	...
	if (stmt->t_expr != NULL)
	{
		...
		t_var = (PLMySQL_var *) estate->datums[stmt->t_varno];
		...
		if (t_var->datatype->typoid != t_typoid ||
			...
			t_var->datatype = plmysql_build_datatype(t_typoid,
													 t_typmod,
 ...
 */
static int
exec_stmt_loop(PLMySQL_execstate *estate, PLMySQL_stmt_loop *stmt)
{
	int			rc = PLMYSQL_RC_OK;
 ...
 */
static int
exec_stmt_while(PLMySQL_execstate *estate, PLMySQL_stmt_while *stmt)
{
	int			rc = PLMYSQL_RC_OK;
 ...
 */
static int
exec_stmt_exit(PLMySQL_execstate *estate, PLMySQL_stmt_exit *stmt)
{
	/*
 ...
 */
static int
exec_stmt_return(PLMySQL_execstate *estate, PLMySQL_stmt_return *stmt)
{
	/*
	 * If processing a set-returning PL/MySQL function, the final RETURN
	 * indicates that the function is finished producing tuples.  The rest of
	 * the work will be done at the top level.
	 */
	if (estate->retisset)
		return PLMYSQL_RC_RETURN;
	...
	if (stmt->retvarno >= 0)
	{
		PLMySQL_datum *retvar = estate->datums[stmt->retvarno];

		switch (retvar->dtype)
		{
			case PLMYSQL_DTYPE_PROMISE:
				/* fulfill promise if needed, then handle like regular var */
				plmysql_fulfill_promise(estate, (PLMySQL_var *) retvar);

				/* FALL THRU */

			case PLMYSQL_DTYPE_VAR:
				{
					PLMySQL_var *var = (PLMySQL_var *) retvar;
					...
					/*
					 * A PLMySQL_var could not be of composite type, so
					 ...
					 * is NULL.  (Note that plmysql_exec_trigger assumes that
 ...
 */
static int
exec_stmt_return_next(PLMySQL_execstate *estate,
					  PLMySQL_stmt_return_next *stmt)
{
	TupleDesc	tupdesc;
	int			natts;
	...
	if (stmt->retvarno >= 0)
	{
		...
		switch (retvar->dtype)
		{
			...
			case PLMYSQL_DTYPE_REC:
				{
					PLMySQL_rec *rec = (PLMySQL_rec *) retvar;
			...
			case PLMYSQL_DTYPE_ROW:
				{
					PLMySQL_row *row = (PLMySQL_row *) retvar;
 ...
 */
static int
exec_stmt_return_query(PLMySQL_execstate *estate,
					   PLMySQL_stmt_return_query *stmt)
{
	int64		tcount;
...
static void
exec_init_tuple_store(PLMySQL_execstate *estate)
{
	ReturnSetInfo *rsi = estate->rsi;
	MemoryContext oldcxt;
 ...
 */
static int
exec_stmt_raise(PLMySQL_execstate *estate, PLMySQL_stmt_raise *stmt)
{
	int			err_code = 0;
	char	   *condname = NULL;
	...
	if (stmt->condname)
	{
		err_code = plmysql_recognize_err_condition(stmt->condname, true);
		...
		for (cp = stmt->message; *cp; cp++)
		{
			...
			if (cp[0] == '%')
			{
				...
				paramvalue = exec_eval_expr(estate,
											(PLMySQL_expr *) lfirst(current_param),
											&paramisnull,
											&paramtypeid,
											&paramtypmod);
				...
				if (err_code)
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("RAISE option already specified: %s",
									"ERRCODE")));
				err_code = plmysql_recognize_err_condition(extval, true);
 ...
 */
static int
exec_stmt_assert(PLMySQL_execstate *estate, PLMySQL_stmt_assert *stmt)
{
	bool		value;
	bool		isnull;

	/* do nothing when asserts are not enabled */
	if (!plmysql_check_asserts)
		return PLMYSQL_RC_OK;
 ...
 */
static void
plmysql_estate_setup(PLMySQL_execstate *estate,
					 PLMySQL_function *func,
					 ReturnSetInfo *rsi,
					 EState *simple_eval_estate,
					 ResourceOwner simple_eval_resowner)
{
	...
	/* this link will be restored at exit from plmysql_call_handler */
	...
	estate->datums = NULL;
	/* the datums array will be filled by copy_plmysql_datums() */
	...
	estate->paramLI->paramFetch = plmysql_param_fetch;
	estate->paramLI->paramFetchArg = (void *) estate;
	estate->paramLI->paramCompile = plmysql_param_compile;
	estate->paramLI->paramCompileArg = NULL;	/* not needed */
	estate->paramLI->parserSetup = (ParserSetupHook) plmysql_parser_setup;
	...
	{
		ctl.keysize = sizeof(plmysql_CastHashKey);
		ctl.entrysize = sizeof(plmysql_CastExprHashEntry);
		cast_expr_hash = hash_create("PLMySQL cast expressions",
	...
	if (simple_eval_estate)
	{
		...
		ctl.keysize = sizeof(plmysql_CastHashKey);
		ctl.entrysize = sizeof(plmysql_CastHashEntry);
		ctl.hcxt = CurrentMemoryContext;
		estate->cast_hash = hash_create("PLMySQL private cast cache",
										16, /* start small and extend */
										&ctl,
	...
	else
	{
		...
		if (shared_cast_hash == NULL)
		{
			ctl.keysize = sizeof(plmysql_CastHashKey);
			ctl.entrysize = sizeof(plmysql_CastHashEntry);
			shared_cast_hash = hash_create("PLMySQL cast cache",
										   16,	/* start small and extend */
										   &ctl,
										   HASH_ELEM | HASH_BLOBS);
		}
	 ...
	 * PL/MySQL variables - note that we also give the plugin a few function
	 * pointers so it can call back into PL/MySQL for doing things like
	 * variable assignments and stack traces
	 */
	if (*plmysql_plugin_ptr)
	{
		(*plmysql_plugin_ptr)->error_callback = plmysql_exec_error_callback;
		(*plmysql_plugin_ptr)->assign_expr = exec_assign_expr;

		if ((*plmysql_plugin_ptr)->func_setup)
			((*plmysql_plugin_ptr)->func_setup) (estate, func);
	}
}
 ...
 */
static void
exec_eval_cleanup(PLMySQL_execstate *estate)
{
 ...
 */
static void
exec_prepare_plan(PLMySQL_execstate *estate,
				  PLMySQL_expr *expr, int cursorOptions)
{
	SPIPlanPtr	plan;
	 ...
	 */
	memset(&options, 0, sizeof(options));
	options.parserSetup = (ParserSetupHook) plmysql_parser_setup;
 ...
 */
static int
exec_stmt_execsql(PLMySQL_execstate *estate,
				  PLMySQL_stmt_execsql *stmt)
{
	ParamListInfo paramLI;
	long		tcount;
	int			rc;
	PLMySQL_expr *expr = stmt->sqlstmt;
	int			too_many_rows_level = 0;

	if (plmysql_extra_errors & PLMYSQL_XCHECK_TOOMANYROWS)
		too_many_rows_level = ERROR;
	else if (plmysql_extra_warnings & PLMYSQL_XCHECK_TOOMANYROWS)
	...
	switch (rc)
	{
		...
		case SPI_ERROR_COPY:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("cannot COPY to/from client in PL/MySQL")));
			break;

		case SPI_ERROR_TRANSACTION:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("unsupported transaction command in PL/MySQL")));
			break;
	...
	if (stmt->into)
	{
		...
		target = (PLMySQL_variable *) estate->datums[stmt->target->dno];
 ...
 */
static int
exec_stmt_dynexecute(PLMySQL_execstate *estate,
					 PLMySQL_stmt_dynexecute *stmt)
{
	Datum		query;
	bool		isnull;
	...
	switch (exec_res)
	{
		...
		case SPI_OK_SELINTO:
			 ...
			 * is not consistent with SELECT INTO in a normal plmysql context.
			 * (We need to reimplement EXECUTE to parse the string as a
			 * plmysql command, not just feed it to SPI_execute.)  This is not
 ...
 */
static int
exec_stmt_open(PLMySQL_execstate *estate, PLMySQL_stmt_open *stmt)
{
	PLMySQL_var *curvar;
	MemoryContext stmt_mcontext = NULL;
	char	   *curname = NULL;
 ...
 */
static int
exec_stmt_fetch(PLMySQL_execstate *estate, PLMySQL_stmt_fetch *stmt)
{
	PLMySQL_var *curvar;
 ...
 */
static int
exec_stmt_close(PLMySQL_execstate *estate, PLMySQL_stmt_close *stmt)
{
	PLMySQL_var *curvar;
	Portal		portal;
	char	   *curname;
 ...
 */
static void
exec_assign_expr(PLMySQL_execstate *estate, PLMySQL_datum *target,
				 PLMySQL_expr *expr)
{
	Datum		value;
	bool		isnull;
	...
	if (expr->plan == NULL)
	{
		 ...
		 */
		if (target->dtype == PLMYSQL_DTYPE_VAR)
 ...
 */
static void
exec_assign_c_string(PLMySQL_execstate *estate, PLMySQL_datum *target,
					 const char *str)
{
	text	   *value;
	MemoryContext oldcontext;
 ...
 */
static void
exec_assign_value(PLMySQL_execstate *estate,
				  PLMySQL_datum *target,
				  Datum value, bool isNull,
				  Oid valtype, int32 valtypmod)
{
		...
		case PLMYSQL_DTYPE_PROMISE:
			{
				 ...
				 */
				PLMySQL_var *var = (PLMySQL_var *) target;
				Datum		newvalue;
				...
				if (var->value != newvalue || var->isnull || isNull)
					...
					var->promise = PLMYSQL_PROMISE_NONE;
				break;
			}
		...
		case PLMYSQL_DTYPE_ROW:
			{
				 ...
				 */
				PLMySQL_row *row = (PLMySQL_row *) target;

				if (isNull)
				{
					/* If source is null, just assign nulls to the row */
					exec_move_row(estate, (PLMySQL_variable *) row,
								  NULL, NULL);
				}
				else
				{
					...
					exec_move_row_from_datum(estate, (PLMySQL_variable *) row,
											 value);
				}
				break;
			}
		...
		case PLMYSQL_DTYPE_REC:
			{
				 ...
				 */
				PLMySQL_rec *rec = (PLMySQL_rec *) target;
				...
				else
				{
					...
					exec_move_row_from_datum(estate, (PLMySQL_variable *) rec,
											 value);
				}
				break;
			}
		...
		case PLMYSQL_DTYPE_RECFIELD:
			{
				 ...
				 */
				PLMySQL_recfield *recfield = (PLMySQL_recfield *) target;
				PLMySQL_rec *rec;
				ExpandedRecordHeader *erh;

				rec = (PLMySQL_rec *) (estate->datums[recfield->recparentno]);
				erh = rec->erh;
...
/*
 * exec_eval_datum				Get current value of a PLMySQL_datum
 *
 * The type oid, typmod, value in Datum format, and null flag are returned.
 *
 * At present this doesn't handle PLMySQL_expr datums; that's not needed
 ...
 */
static void
exec_eval_datum(PLMySQL_execstate *estate,
				PLMySQL_datum *datum,
				Oid *typeid,
				int32 *typetypmod,
				Datum *value,
				bool *isnull)
{
	...
	switch (datum->dtype)
	{
		case PLMYSQL_DTYPE_PROMISE:
			/* fulfill promise if needed, then handle like regular var */
			plmysql_fulfill_promise(estate, (PLMySQL_var *) datum);
		...
		case PLMYSQL_DTYPE_ROW:
			{
				PLMySQL_row *row = (PLMySQL_row *) datum;
				HeapTuple	tup;
		...
		case PLMYSQL_DTYPE_RECFIELD:
			{
				PLMySQL_recfield *recfield = (PLMySQL_recfield *) datum;
				PLMySQL_rec *rec;
				ExpandedRecordHeader *erh;
...
/*
 * plmysql_exec_get_datum_type				Get datatype of a PLMySQL_datum
 *
 ...
 */
Oid
plmysql_exec_get_datum_type(PLMySQL_execstate *estate,
							PLMySQL_datum *datum)
{
	Oid			typeid;
...
/*
 * plmysql_exec_get_datum_type_info			Get datatype etc of a PLMySQL_datum
 *
 * An extended version of plmysql_exec_get_datum_type, which also retrieves the
 ...
 */
void
plmysql_exec_get_datum_type_info(PLMySQL_execstate *estate,
								 PLMySQL_datum *datum,
 ...
 */
static int
exec_eval_integer(PLMySQL_execstate *estate,
				  PLMySQL_expr *expr,
				  bool *isNull)
{
	Datum		exprdatum;
 ...
 */
static bool
exec_eval_boolean(PLMySQL_execstate *estate,
				  PLMySQL_expr *expr,
				  bool *isNull)
{
	Datum		exprdatum;
 ...
 */
static Datum
exec_eval_expr(PLMySQL_execstate *estate,
			   PLMySQL_expr *expr,
			   bool *isNull,
			   Oid *rettype,
			   int32 *rettypmod)
 ...
 */
static int
exec_run_select(PLMySQL_execstate *estate,
				PLMySQL_expr *expr, long maxtuples, Portal *portalP)
{
	ParamListInfo paramLI;
	int			rc;
 ...
 */
static bool
exec_eval_simple_expr(PLMySQL_execstate *estate,
					  PLMySQL_expr *expr,
					  Datum *result,
					  bool *isNull,
					  Oid *rettype,
 ...
 * PLMySQL_exprs in this estate.  It contains no per-param data, just hook
 * functions, so it's effectively read-only for SPI.
 *
 * An exception from pure read-only-ness is that the parserSetupArg points
 * to the specific PLMySQL_expr being evaluated.  This is not an issue for
 ...
 * create a ParamListInfo struct for each PLMySQL_expr, but for the moment
 * that seems like a waste of memory.)
 */
static ParamListInfo
setup_param_list(PLMySQL_execstate *estate, PLMySQL_expr *expr)
{
	ParamListInfo paramLI;
...
/*
 * plmysql_param_fetch		paramFetch callback for dynamic parameter fetch
 *
...
plmysql_param_fetch(ParamListInfo params,
					int paramid, bool speculative,
					ParamExternData *prm)
{
	int			dno;
	...
	estate = (PLMySQL_execstate *) params->paramFetchArg;
	expr = (PLMySQL_expr *) params->parserSetupArg;
	 ...
	 */
	if (datum->dtype == PLMYSQL_DTYPE_VAR)
		prm->value = MakeExpandedObjectReadOnly(prm->value,
												prm->isnull,
												((PLMySQL_var *) datum)->datatype->typlen);
	else if (datum->dtype == PLMYSQL_DTYPE_REC)
		prm->value = MakeExpandedObjectReadOnly(prm->value,
												prm->isnull,
												-1);
...
/*
 * plmysql_param_compile		paramCompile callback for plmysql parameters
 */
static void
plmysql_param_compile(ParamListInfo params, Param *param,
					  ExprState *state,
					  Datum *resv, bool *resnull)
{
	PLMySQL_execstate *estate;
	...
	if (datum->dtype == PLMYSQL_DTYPE_VAR)
	{
		if (param != expr->expr_rw_param &&
			((PLMySQL_var *) datum)->datatype->typlen == -1)
			scratch.d.cparam.paramfunc = plmysql_param_eval_var_ro;
		else
			scratch.d.cparam.paramfunc = plmysql_param_eval_var;
	}
	else if (datum->dtype == PLMYSQL_DTYPE_RECFIELD)
		scratch.d.cparam.paramfunc = plmysql_param_eval_recfield;
	else if (datum->dtype == PLMYSQL_DTYPE_PROMISE)
	{
		if (param != expr->expr_rw_param &&
			((PLMySQL_var *) datum)->datatype->typlen == -1)
			scratch.d.cparam.paramfunc = plmysql_param_eval_generic_ro;
		else
			scratch.d.cparam.paramfunc = plmysql_param_eval_generic;
	}
	else if (datum->dtype == PLMYSQL_DTYPE_REC &&
	 ...
	 * different estates for the same PL/MySQL function.
	 */
...
/*
 * plmysql_param_eval_var		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we do not need to invoke MakeExpandedObjectReadOnly.
 */
static void
plmysql_param_eval_var(ExprState *state, ExprEvalStep *op,
					   ExprContext *econtext)
{
	...
	/* now we can access the target datum */
	var = (PLMySQL_var *) estate->datums[dno];
	Assert(var->dtype == PLMYSQL_DTYPE_VAR);
...
/*
 * plmysql_param_eval_var_ro		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we need to invoke MakeExpandedObjectReadOnly.
 */
static void
plmysql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
						  ExprContext *econtext)
{
	ParamListInfo params;
	PLMySQL_execstate *estate;
...
/*
 * plmysql_param_eval_recfield		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_RECFIELD variables, for which
 * we never need to invoke MakeExpandedObjectReadOnly.
 */
static void
plmysql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
							ExprContext *econtext)
{
	ParamListInfo params;
	PLMySQL_execstate *estate;
	int			dno = op->d.cparam.paramid - 1;
	PLMySQL_recfield *recfield;
	PLMySQL_rec *rec;
	ExpandedRecordHeader *erh;
	...
	/* now we can access the target datum */
	recfield = (PLMySQL_recfield *) estate->datums[dno];
	Assert(recfield->dtype == PLMYSQL_DTYPE_RECFIELD);
...
/*
 * plmysql_param_eval_generic		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This handles all variable types, but assumes we do not need to invoke
 * MakeExpandedObjectReadOnly.
 */
static void
plmysql_param_eval_generic(ExprState *state, ExprEvalStep *op,
						   ExprContext *econtext)
{
	ParamListInfo params;
	PLMySQL_execstate *estate;
...
/*
 * plmysql_param_eval_generic_ro	evaluation of EEOP_PARAM_CALLBACK step
 *
 ...
 */
static void
plmysql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
							  ExprContext *econtext)
{
	ParamListInfo params;
	PLMySQL_execstate *estate;
 ...
 */
static void
exec_move_row(PLMySQL_execstate *estate,
			  PLMySQL_variable *target,
			  HeapTuple tup, TupleDesc tupdesc)
{
	 ...
	 */
	if (target->dtype == PLMYSQL_DTYPE_REC)
	{
		PLMySQL_rec *rec = (PLMySQL_rec *) target;
...
/*
 * Verify that a PLMySQL_rec's rectypeid is up-to-date.
 */
static void
revalidate_rectypeid(PLMySQL_rec *rec)
{
	PLMySQL_type *typ = rec->datatype;
...
make_expanded_record_for_rec(PLMySQL_execstate *estate,
							 PLMySQL_rec *rec,
							 TupleDesc srctupdesc,
							 ExpandedRecordHeader *srcerh)
{
 ...
 */
static void
exec_move_row_from_fields(PLMySQL_execstate *estate,
						  PLMySQL_variable *target,
						  ExpandedRecordHeader *newerh,
						  Datum *values, bool *nulls,
						  TupleDesc tupdesc)
{
	...
	if (tupdesc != NULL)
	{
		if (plmysql_extra_errors & PLMYSQL_XCHECK_STRICTMULTIASSIGNMENT)
			strict_multiassignment_level = ERROR;
		else if (plmysql_extra_warnings & PLMYSQL_XCHECK_STRICTMULTIASSIGNMENT)
	...
	if (target->dtype == PLMYSQL_DTYPE_REC)
	{
		 ...
		 * types.  That's sort of a pain, but historically plmysql has allowed
	 ...
	 * the row variable.
	 */
	if (target->dtype == PLMYSQL_DTYPE_ROW)
	{
		PLMySQL_row *row = (PLMySQL_row *) target;
		...
		for (fnum = 0; fnum < row->nfields; fnum++)
		{
			...
			var = (PLMySQL_var *) (estate->datums[row->varnos[fnum]]);
			...
			exec_assign_value(estate, (PLMySQL_datum *) var,
 ...
 */
static HeapTuple
make_tuple_from_row(PLMySQL_execstate *estate,
					PLMySQL_row *row,
					TupleDesc tupdesc)
{
 ...
 */
static void
exec_move_row_from_datum(PLMySQL_execstate *estate,
						 PLMySQL_variable *target,
						 Datum value)
{
 ...
 */
static void
instantiate_empty_record_variable(PLMySQL_execstate *estate, PLMySQL_rec *rec)
{
 ...
 */
static char *
convert_value_to_string(PLMySQL_execstate *estate, Datum value, Oid valtype)
{
	char	   *result;
	MemoryContext oldcontext;
	Oid			typoutput;
 ...
 */
static inline Datum
exec_cast_value(PLMySQL_execstate *estate,
				Datum value, bool *isnull,
				Oid valtype, int32 valtypmod,
				Oid reqtype, int32 reqtypmod)
{
 ...
 */
static Datum
do_cast_value(PLMySQL_execstate *estate,
			  Datum value, bool *isnull,
			  Oid valtype, int32 valtypmod,
			  Oid reqtype, int32 reqtypmod)
{
	plmysql_CastHashEntry *cast_entry;
 ...
 *
 * Returns a plmysql_CastHashEntry if an expression has to be evaluated,
 ...
 * ----------
 */
static plmysql_CastHashEntry *
get_cast_hashentry(PLMySQL_execstate *estate,
				   Oid srctype, int32 srctypmod,
				   Oid dsttype, int32 dsttypmod)
{
	plmysql_CastHashKey cast_key;
	plmysql_CastHashEntry *cast_entry;
	plmysql_CastExprHashEntry *expr_entry;
	bool		found;
	...
	cast_entry = (plmysql_CastHashEntry *) hash_search(estate->cast_hash,
													   (void *) &cast_key,
													   HASH_ENTER, &found);
	if (!found)					/* initialize if new entry */
	{
		/* We need a second lookup to see if a cast_expr_hash entry exists */
		expr_entry = (plmysql_CastExprHashEntry *) hash_search(cast_expr_hash,
															   &cast_key,
															   HASH_ENTER,
															   &found);
		...
		!expr_entry->cast_cexpr->is_valid)
	{
		 ...
		 * COERCION_PLPGSQL to match plmysql's historical behavior, namely
		 ...
		 * an I/O coercion; this is semantically dubious but matches plmysql's
	 ...
	 * hash table with the new tree because all plmysql functions within a
 ...
 */
static void
exec_simple_check_plan(PLMySQL_execstate *estate, PLMySQL_expr *expr)
{
	List	   *plansources;
 ...
 */
static bool
exec_is_simple_query(PLMySQL_expr *expr)
{
	List	   *plansources;
	CachedPlanSource *plansource;
	Query	   *query;
 ...
 */
static void
exec_save_simple_expr(PLMySQL_expr *expr, CachedPlan *cplan)
{
	PlannedStmt *stmt;
	Plan	   *plan;
	Expr	   *tle_expr;
 ...
 */
static void
exec_check_rw_parameter(PLMySQL_expr *expr)
{
	int			target_dno;
	Oid			funcid;
	List	   *fargs;
 ...
 */
static void
exec_set_found(PLMySQL_execstate *estate, bool state)
{
	PLMySQL_var *var;

	var = (PLMySQL_var *) (estate->datums[estate->found_varno]);
	assign_simple_var(estate, var, BoolGetDatum(state), false, false);
}

/*
 * plmysql_create_econtext --- create an eval_econtext for the current function
 *
 ...
 */
static void
plmysql_create_econtext(PLMySQL_execstate *estate)
{
	 ...
	 * required EState was already made by plmysql_inline_handler.  However,
...
/*
 * plmysql_destroy_econtext --- destroy function's econtext
 *
 * We check that it matches the top stack entry, and destroy the stack
 * entry along with the context.
 */
static void
plmysql_destroy_econtext(PLMySQL_execstate *estate)
{
...
/*
 * plmysql_xact_cb --- post-transaction-commit-or-abort cleanup
 *
 * If a simple-expression EState was created in the current transaction,
 * it has to be cleaned up.  The same for the simple-expression resowner.
 */
void
plmysql_xact_cb(XactEvent event, void *arg)
{
	/*
...
/*
 * plmysql_subxact_cb --- post-subtransaction-commit-or-abort cleanup
 *
 ...
 */
void
plmysql_subxact_cb(SubXactEvent event, SubTransactionId mySubid,
 ...
 */
static void
assign_simple_var(PLMySQL_execstate *estate, PLMySQL_var *var,
				  Datum newvalue, bool isnull, bool freeable)
{
	Assert(var->dtype == PLMYSQL_DTYPE_VAR ||
		   var->dtype == PLMYSQL_DTYPE_PROMISE);
 ...
 */
static void
assign_text_var(PLMySQL_execstate *estate, PLMySQL_var *var, const char *str)
{
 ...
 */
static void
assign_record_var(PLMySQL_execstate *estate, PLMySQL_rec *rec,
				  ExpandedRecordHeader *erh)
{
	Assert(rec->dtype == PLMYSQL_DTYPE_REC);
 ...
 */
static ParamListInfo
exec_eval_using_params(PLMySQL_execstate *estate, List *params)
{
	ParamListInfo paramLI;
	int			nargs;
	MemoryContext stmt_mcontext;
	...
	foreach(lc, params)
	{
		PLMySQL_expr *param = (PLMySQL_expr *) lfirst(lc);
 ...
 */
static Portal
exec_dynquery_with_params(PLMySQL_execstate *estate,
 ...
 */
static char *
format_expr_params(PLMySQL_execstate *estate,
				   const PLMySQL_expr *expr)
{
	int			paramno;
	int			dno;
	...
	while ((dno = bms_next_member(expr->paramnos, dno)) >= 0)
	{
		...
		curvar = (PLMySQL_var *) estate->datums[dno];

		exec_eval_datum(estate, (PLMySQL_datum *) curvar,
 ...
 */
static char *
format_preparedparamsdata(PLMySQL_execstate *estate,
						  ParamListInfo paramLI)
{
	int			paramno;
	StringInfoData paramstr;
