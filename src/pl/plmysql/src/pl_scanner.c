 ...
 *
 * IDENTIFICATION
 *	  src/pl/plmysql/src/pl_scanner.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
IdentifierLookup plmysql_IdentifierLookup = IDENTIFIER_LOOKUP_NORMAL;
 ...
 * isn't a known variable name.  If plmysql_IdentifierLookup is DECLARE then
...
/* ScanKeywordList lookup data for PL/MySQL keywords */
#include "pl_reserved_kwlist_d.h"
#include "pl_unreserved_kwlist_d.h"

/* Token codes for PL/MySQL keywords */
 ...
 * PL/MySQL executable statement (that is, proc_sect or proc_stmt in the
 ...
 * into a YY_EXTRA struct.  For the moment, there is no need for plmysql's
...
/* Current token's length (corresponds to plmysql_yylval and plmysql_yylloc) */
static int	plmysql_yyleng;

/* Current token's code (corresponds to plmysql_yylval and plmysql_yylloc) */
static int	plmysql_yytoken;
...
/* State for plmysql_location_to_lineno() */
...
/*
 * This is the yylex routine called from the PL/MySQL grammar.
 * It is a wrapper around the core lexer, with the ability to recognize
 * PL/MySQL variables and return them as special T_DATUM tokens.  If a
 * word or compound word does not match any variable name, or if matching
 * is turned off by plmysql_IdentifierLookup, it is returned as
...
plmysql_yylex(void)
{
	...
	if (tok1 == IDENT || tok1 == PARAM)
	{
		...
		if (tok2 == '.')
		{
			...
			if (tok3 == IDENT)
			{
				...
				if (tok4 == '.')
				{
					...
					if (tok5 == IDENT)
					{
						if (plmysql_parse_tripword(aux1.lval.str,
												   aux3.lval.str,
												   aux5.lval.str,
												   &aux1.lval.wdatum,
												   &aux1.lval.cword))
					...
					else
					{
						...
						if (plmysql_parse_dblword(aux1.lval.str,
												  aux3.lval.str,
												  &aux1.lval.wdatum,
												  &aux1.lval.cword))
							tok1 = T_DATUM;
			...
			else
			{
				...
				if (plmysql_parse_word(aux1.lval.str,
					...
					tok1 = T_DATUM;
						 ...
						 (kwnum = ScanKeywordLookup(aux1.lval.word.ident,
													&UnreservedPLMySQLKeywords)) >= 0)
				{
					aux1.lval.keyword = GetScanKeyword(kwnum,
													   &UnreservedPLMySQLKeywords);
		...
		else
		{
			 ...
			 * be used that would conflict with plmysql or core keywords that
			 * introduce statements (e.g., "comment").  Without this special
			 * logic, every statement-introducing keyword would effectively be
			 * reserved in PL/MySQL, which would be unpleasant.
			 *
			 * If it isn't a variable name, try to match against unreserved
			 * plmysql keywords.  If not one of those either, it's T_WORD.
			 *
			 * Note: we must call plmysql_parse_word even if we don't want to
			 * do variable lookup, because it sets up aux1.lval.word for the
			 * non-variable cases.
			 */
			if (plmysql_parse_word(aux1.lval.str,
								   core_yy.scanbuf + aux1.lloc,
								   (!AT_STMT_START(plmysql_yytoken) ||
	...
	{
		/*
		 * Not a potential plmysql variable name, just return the data.
		 *
		 ...
		 * plmysql_yytoken in the is-this-start-of-statement test above.
		 */
	}

	plmysql_yylval = aux1.lval;
	plmysql_yylloc = aux1.lloc;
	plmysql_yyleng = aux1.leng;
	plmysql_yytoken = tok1;
	return tok1;
}

/*
 * Return the length of the token last returned by plmysql_yylex().
 *
 * In the case of compound tokens, the length includes all the parts.
 */
int
plmysql_token_length(void)
{
	return plmysql_yyleng;
}
...
/*
 * Push back a single token to be re-read by next plmysql_yylex() call.
 *
 ...
 */
void
plmysql_push_back_token(int token)
{
	TokenAuxData auxdata;

	auxdata.lval = plmysql_yylval;
	auxdata.lloc = plmysql_yylloc;
	auxdata.leng = plmysql_yyleng;
 ...
 */
bool
plmysql_token_is_unreserved_keyword(int token)
{
	int			i;
 ...
 */
void
plmysql_append_source_text(StringInfo buf,
 ...
 */
void
plmysql_peek2(int *tok1_p, int *tok2_p, int *tok1_loc, int *tok2_loc)
{
	int			tok1,
				tok2;
	TokenAuxData aux1,
 ...
 * parsing of a plmysql function, since it requires the scanorig string
 * to still be available.
 */
int
plmysql_scanner_errposition(int location)
{
	int			pos;
 ...
 * last returned by plmysql_yylex()).
 ...
 */
void
plmysql_yyerror(const char *message)
{
	char	   *yytext = core_yy.scanbuf + plmysql_yylloc;
	...
	if (*yytext == '\0')
	{
		...
		/* translator: %s is typically the translation of "syntax error" */
				 ...
				 plmysql_scanner_errposition(plmysql_yylloc)));
	}
	else
	{
		/*
 ...
 */
int
plmysql_location_to_lineno(int location)
{
	const char *loc;
...
/* initialize or reset the state for plmysql_location_to_lineno */
static void
 ...
 *
 * Note: the passed "str" must remain valid until plmysql_scanner_finish().
...
/*
 * Called after parsing is done to clean up after plmysql_scanner_init()
 */
void
