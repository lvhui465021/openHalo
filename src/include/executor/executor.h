...
ExecProject(ProjectionInfo *projInfo)
{
	 ...
	 */
	ExecClearTuple(slot);
	state->restrain_errors = true;	

	/* Run the expression, discarding scalar result from the last column. */
	(void) ExecEvalExprSwitchContext(state, econtext, &isnull);

	state->restrain_errors = false; 
	/*
...
extern void standard_ExecReScan(PlanState *node);
extern Node *standard_MultiExecProcNode(PlanState *node);
extern bool standard_ExecShutdownNode(PlanState *node);

/* standard replication routines */
extern bool standard_RelationFindReplTupleByIndex(Relation rel, Oid idxoid,
										 		  LockTupleMode lockmode,
										 		  TupleTableSlot *searchslot,
										 		  TupleTableSlot *outslot);
extern bool standard_RelationFindReplTupleSeq(Relation rel, LockTupleMode lockmode,
									 		  TupleTableSlot *searchslot, TupleTableSlot *outslot);
extern void standard_ExecSimpleRelationInsert(ResultRelInfo *resultRelInfo,
									 		  EState *estate, TupleTableSlot *slot);
extern void standard_ExecSimpleRelationUpdate(ResultRelInfo *resultRelInfo,
									 		  EState *estate, EPQState *epqstate,
									 		  TupleTableSlot *searchslot, TupleTableSlot *slot);
extern void standard_ExecSimpleRelationDelete(ResultRelInfo *resultRelInfo,
									 		  EState *estate, EPQState *epqstate,
									 		  TupleTableSlot *searchslot);
extern void standard_CheckCmdReplicaIdentity(Relation rel, CmdType cmd);
extern void standard_CheckSubscriptionRelkind(char relkind, const char *nspname,
									 		  const char *relname);
