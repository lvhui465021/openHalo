 ...
 */
void
standard_ExecEvalSQLValueFunction(ExprState *state, ExprEvalStep *op)
{
	...
	*op->resnull = false;
	...
	switch (svf->op)
	{
		...
		case SVFOP_CURRENT_TIMEZONE:
			*op->resvalue = TimeTzADTPGetDatum(GetSQLCurrentTimeZone(svf->typmod));
			break;
	...
	switch (op->d.nextvalueexpr.seqtypid)
	{
		...
		case INT8OID:
			*op->resvalue = Int64GetDatum((int64) newval);
			break;
		case NUMERICOID:
			*op->resvalue = Int64GetDatum((int64) 25);
			break;
		default:
