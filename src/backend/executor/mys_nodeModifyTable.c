...
/* PG18: trigger2.h not present; use PG18's ExecBRUpdateTriggers */
...
typedef struct UpdateContext
{
	...
	TU_UpdateIndexes update_indexes;	/* index update required? */
				   ...
				   List *recheckIndexes)
{
	...
	if (resultRelInfo->ri_NumIndices > 0 && updateCxt->update_indexes)
		recheckIndexes = ExecInsertIndexTuples(resultRelInfo,
											   slot, context->estate,
											   true, false,
											   NULL, NIL, false);
...
mys_ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
	...
	EvalPlanQualInit(&mtstate->mt_epqstate, estate, NULL, NIL, node->epqParam, node->resultRelations);
	...
	foreach(l, node->resultRelations)
	{
		 ...
		 */
		CheckValidResultRel(resultRelInfo, operation, node->onConflictAction, NIL);
...
mys_ExecEndModifyTable(ModifyTableState *node)
{
	 ...
	 */
	FreeExprContext(node->ps.ps_ExprContext, false);
