 ...
 *
 * mys_tablecmds.c
 *	  MySQL DDL commands (CREATE TABLE, ALTER TABLE, etc.)
 *
...
/*
 * Compatibility: ObjectClass enum and getObjectClass() were removed in PG18.
 * Provide local copies for the MySQL DDL code that still uses them.
 */
...
static AlterTableCmd *ATParseTransformCmd(List **wqueue, AlteredTableInfo *tab,
										  ...
										  int cur_pass,
										  AlterTableUtilityContext *context);
static void mysQueuePrivateEnumDrop(AlteredTableInfo *tab, Relation rel,
							 const char *colname);
static bool mysIsPrivateEnumDomain(Form_pg_type type_form);
...
static void ATPrepSetAccessMethod(AlteredTableInfo *tab, Relation rel,
								  const char *amname);
 ...
 *
 * Also note that ud_dump uses only an AccessShareLock, meaning that anything
 * that takes a lock less than AccessExclusiveLock can change object definitions
 * while ud_dump is running. Be careful to check that the appropriate data is
 * derived by ud_dump using an MVCC snapshot, rather than syscache lookups,
		  ...
		  AlterTableUtilityContext *context)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_DropColumn:		/* DROP COLUMN */
			ATSimplePermissions(rel,
								ATT_ORDINARY_TABLE | ATT_PARTITIONED_TABLE | ATT_COMPOSITE_TYPE | ATT_FOREIGN_TABLE);
			mysQueuePrivateEnumDrop(tab, rel, cmd->name);
...
/*
 * MySQL ENUM is a table-column-private text domain.  PostgreSQL removes the
 * column dependency but retains the domain, so schedule a restricted drop
 * after the column operation.  The internal name guard excludes ordinary or
 * shared PostgreSQL domains.
 */
static bool
mysIsPrivateEnumDomain(Form_pg_type type_form)
{
	return type_form->typtype == TYPTYPE_DOMAIN &&
		strncmp(NameStr(type_form->typname), "enum_", 5) == 0;
}

static void
mysQueuePrivateEnumDrop(AlteredTableInfo *tab, Relation rel, const char *colname)
{
	HeapTuple	att_tuple;
	...
	att_tuple = SearchSysCacheCopyAttName(RelationGetRelid(rel), colname);
	if (!HeapTupleIsValid(att_tuple))
		return;
	att_form = (Form_pg_attribute) GETSTRUCT(att_tuple);
	type_tuple = SearchSysCache1(TYPEOID, ObjectIdGetDatum(att_form->atttypid));
	heap_freetuple(att_tuple);
	if (!HeapTupleIsValid(type_tuple))
		return;
	type_form = (Form_pg_type) GETSTRUCT(type_tuple);
	if (!mysIsPrivateEnumDomain(type_form))
	{
		ReleaseSysCache(type_tuple);
		return;
	}
	namespace_name = pstrdup(get_namespace_name(type_form->typnamespace));
	type_name = pstrdup(NameStr(type_form->typname));
	ReleaseSysCache(type_tuple);

	drop_stmt = makeNode(DropStmt);
	drop_stmt->removeType = OBJECT_DOMAIN;
	type_name_node = makeTypeNameFromNameList(
		list_make2(makeString(namespace_name), makeString(type_name)));
	drop_stmt->objects = list_make1(type_name_node);
	drop_stmt->behavior = DROP_RESTRICT;
	drop_stmt->missing_ok = true;
	drop_stmt->concurrent = false;
	tab->afterStmts = lappend(tab->afterStmts, drop_stmt);
}
				...
				AlterTableUtilityContext *context)
{
	...
	/* attstattarget is in CATALOG_VARLEN section in PG18; default is fine */
					...
					int cur_pass, AlterTableUtilityContext *context)
{
	...
	/*
	 * This file is reached only through the MySQL utility path.  In
	 * particular, MODIFY/CHANGE carry MySQL-only subcommand tags whose
	 * ColumnDef needs the MySQL transformer to create or replace the private
	...
	foreach(lc, atstmt->cmds)
	{
		...
		switch (cmd2->subtype)
		{
			case AT_DropConstraint:
				/* Generated by MySQL SET MODIFY to replace its table CHECK. */
				ATCheckPartitionsNotInUse(rel, lockmode);
				if (recurse)
					cmd2->recurse = true;
				pass = AT_PASS_DROP;
...
ATPrepChangeColumn(AlteredTableInfo *tab, Relation rel, AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	aclResult = object_aclcheck(TypeRelationId, newTypeOid, GetUserId(), ACL_USAGE);
	if (aclResult != ACLCHECK_OK)
		...
		tab->relkind == RELKIND_PARTITIONED_TABLE)
    {
        ...
        
        {
            ...
            if (mysIsPrivateEnumDomain(colOldTypeInfo) &&
                mysIsPrivateEnumDomain(colNewTypeInfo))
            {
                /* Private ENUM domains have no direct conversion relation. */
                ...
                case OCLASS_TYPE:
                    {
                        ...
                        if (mysIsPrivateEnumDomain(oldColTypeInfo))
                        {
                            if (mysIsPrivateEnumDomain(targetTypeForm))
                            {
                                ok = true;
                            }
                        }
...
ATExecTableOptionAutoIncrement(Relation rel, Node *newStartValue, bool isTableEmpty)
{
    ...
    {
        if (IsA(newStartValue, Integer))
        {
			int64		val64 = intVal(newStartValue);
	        ...
	        else if (IsA(newStartValue, Float))
	        {
	            int64 val64;
	            /* could be an oversize integer as well as a float ... */
				val64 = pg_strtoint64(castNode(Float, newStartValue)->fval);
				/*
...
ATExecSetStatistics(Relation rel, const char *colName, int16 colNum, Node *newValue, LOCKMODE lockmode)
{
	...
	{
		newtarget = 10000;
		elog(LOG, "lowering statistics target to %d",
			 newtarget); /* M3: ereport→elog avoids MySQL wire pollution */
	}
	...
	/* Use repl mechanism; attstattarget is in CATALOG_VARLEN section in PG18 */
	memset(repl_null, false, sizeof(repl_null));
	memset(repl_repl, false, sizeof(repl_repl));
					  ...
					  bool recursing, LOCKMODE lockmode)
{
	 ...
	 * they are not directly connected to this constraint; also, ud_dump would
...
getTableOnUpdateNowColsNum(Relation rel)
{
    ...
    for (i = 0; i < columnsDesc->natts; i++)
    {
        ...
        Form_pg_attribute attrForm = TupleDescAttr(columnsDesc, i);
 ...
 * clause.
 * fkdelsetcols is the attnum array of columns in the ON DELETE SET
 * NULL/DEFAULT clause.
                              ...
                              char *newColName, List **stmts)
{
    ...
    if (strncasecmp(oldTypeName, "set_", 4) == 0)
    {
        ...
        *stmts = lappend(*stmts, renameStmt);
        ...
        /* Keep the SET normalizer discoverable for a later MODIFY/CHANGE. */
        oldTrigName = makeObjectName("setnormtrig", tableName, oldColName);
        oldTrigOid = get_trigger_oid(targetRel->rd_id, oldTrigName, true);
        if (!OidIsValid(oldTrigOid))
            return;
        ...
        trigTuple = systable_getnext(trigScan);
        if (!HeapTupleIsValid(trigTuple))
            elog(ERROR, "trigger %u disappeared during SET column rename", oldTrigOid);
        trigInfo = (Form_pg_trigger) GETSTRUCT(trigTuple);
        oldFuncOid = trigInfo->tgfoid;
        systable_endscan(trigScan);
        table_close(trigRel, AccessShareLock);

        newTrigName = makeObjectName("setnormtrig", tableName, newColName);
        renameTrigStmt = renameTrig(oldTrigOid, namespaceName, tableName,
                                    newTrigName);
        if (renameTrigStmt != NULL)
            *stmts = lappend(*stmts, renameTrigStmt);

        newFuncName = makeObjectName("setnorm", tableName, newColName);
        renameFuncStmt = renameFunc(oldFuncOid, namespaceName, newFuncName);
        if (renameFuncStmt != NULL)
            *stmts = lappend(*stmts, renameFuncStmt);
    }
    else if (mysIsPrivateEnumDomain((Form_pg_type) GETSTRUCT(typTuple)))
    {
		Oid oldTrigOid;
		Relation trigRel;
		ScanKeyData trigKey;
        ...
        *stmts = lappend(*stmts, renameStmt);
		...
		/* Keep a later MODIFY pointed at the live ENUM normalizer. */
		oldTrigName = makeObjectName("enumnormtrig", tableName, oldColName);
		oldTrigOid = get_trigger_oid(targetRel->rd_id, oldTrigName, true);
		if (!OidIsValid(oldTrigOid))
			goto enum_done;
		...
		ScanKeyInit(&trigKey, Anum_pg_trigger_oid, BTEqualStrategyNumber,
					F_OIDEQ,
					ObjectIdGetDatum(oldTrigOid));
		trigScan = systable_beginscan(trigRel, TriggerOidIndexId, true,
								  NULL, 1, &trigKey);
		trigTuple = systable_getnext(trigScan);
		if (!HeapTupleIsValid(trigTuple))
			elog(ERROR, "trigger %u disappeared during ENUM column rename", oldTrigOid);
		trigInfo = (Form_pg_trigger) GETSTRUCT(trigTuple);
		oldFuncOid = trigInfo->tgfoid;
		systable_endscan(trigScan);
		table_close(trigRel, AccessShareLock);

		newTrigName = makeObjectName("enumnormtrig", tableName, newColName);
		renameTrigStmt = renameTrig(oldTrigOid, namespaceName, tableName,
								newTrigName);
		if (renameTrigStmt != NULL)
			*stmts = lappend(*stmts, renameTrigStmt);

		newFuncName = makeObjectName("enumnorm", tableName, newColName);
		renameFuncStmt = renameFunc(oldFuncOid, namespaceName, newFuncName);
		if (renameFuncStmt != NULL)
			*stmts = lappend(*stmts, renameFuncStmt);
                                   ...
                                   char *newTableName, List **stmts)
{
    ...
    for (int i = 1; i < targetRel->rd_att->natts; i++)
    {
        ...
        char* oldTypeName;
        char* attName;
        Form_pg_attribute attrForm = TupleDescAttr(targetRel->rd_att, i);
		HeapTuple heapTuple = 
        ...
        else if (mysIsPrivateEnumDomain(formPgType))
        {
