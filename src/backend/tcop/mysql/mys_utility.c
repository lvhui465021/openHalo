 ...
 *
 * mys_utility.c
 *	  MySQL utility command dispatch (ProcessUtility)
 *
 ...
 *
 * Copied from PG18 src/backend/tcop/utility.c where it is static.
 */
static void
 ...
 *		Determine whether a utility command is read-only.
 *
 * Copied from PG18 src/backend/tcop/utility.c where it is static.
 */
static int
                       ...
                       QueryCompletion *qc)
{
	...
	PG_TRY();
	{
		...
		switch (nodeTag(parsetree))
		{
			...
			case T_CreateSubscriptionStmt:
				address = CreateSubscription(pstate, (CreateSubscriptionStmt *) parsetree,
											 isTopLevel);
				break;

			case T_AlterSubscriptionStmt:
				address = AlterSubscription(pstate, (AlterSubscriptionStmt *) parsetree,
											isTopLevel);
				break;
...
MysExecSelectIntoStmt(ParseState *pstate, MysSelectIntoStmt *parsetree, ParamListInfo params, QueryCompletion *qc)
{
    ...
    {
        elog(LOG, "No data - zero rows fetched, selected, or processed"); /* M3: ereport→elog avoids MySQL wire pollution */
    }
    else
    {
        TupleTableSlot *slot;
