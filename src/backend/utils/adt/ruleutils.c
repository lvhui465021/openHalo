					   ...
					   int prettyFlags, bool missing_ok)
{
	...
	if (!attrsOnly)
	{
		if (!isConstraint)
		{
			...
			{
				appendStringInfo(&buf, "CREATE %sINDEX %s USING %s ON %s%s (",
			...
			/* Subselect-in-FROM: examine sub-select's output expr */
			{
				if (rte->subquery)
				{
					...
					if(rte->subquery->commandType == CMD_SELECT)
						ste = get_tle_by_resno(rte->subquery->targetList,
															attnum);
					else
						ste = get_tle_by_resno(rte->subquery->returningList,
															attnum);
		...
		case T_SQLValueFunction:
			{
				...
				switch (svf->op)
				{
					...
					case SVFOP_CURRENT_TIMEZONE:
						appendStringInfo(buf, "CURRENT_TIMEZONE");
						break;
...
quote_identifier(const char *ident)
{
    ...
    if ((MyProcPort != NULL) && 
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        return mys_quote_identifier(ident);
    }
    else
    {
        /* Nothing to do. */
