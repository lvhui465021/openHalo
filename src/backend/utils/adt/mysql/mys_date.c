 ...
 *
 * mys_date.c
 *    MySQL ADT compatibility: date and time input/output parsing.
 *
 * Portions Copyright (c) 2026, HaloLab / UDB-TX Contributors
 *
 * src/backend/utils/adt/mysql/mys_date.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
    ...
    if (mys_strToDateInternal(str, timeFlags, &result) == false)
		ereturn(escontext, (Datum) 0,
				(errcode(ERRCODE_INVALID_DATETIME_FORMAT),
				 errmsg("Incorrect datetime value: '%s'", str)));
...
mys_pre_time_in(PG_FUNCTION_ARGS)
{
	...
	if (dterr == 0)
		dterr = DecodeTimeOnly(field, ftype, nf, &dtype, tm, &fsec, &tz, NULL);
	if (dterr != 0)
	{
		DateTimeParseError(dterr, NULL, str, "time", escontext);
