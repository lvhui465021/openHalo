 ...
 *
 * mys_timestamp.c
 *    MySQL ADT compatibility: timestamp parsing and validation.
 *
 * Portions Copyright (c) 2026, HaloLab / UDB-TX Contributors
 *
 * src/backend/utils/adt/mysql/mys_timestamp.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
mys_timestamp_in(PG_FUNCTION_ARGS)
{
	...
	if (!AdjustTimestampForTypmod(&result, typmod, escontext))
		PG_RETURN_NULL();
