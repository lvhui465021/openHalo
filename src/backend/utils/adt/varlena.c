...
text_position(text *t1, text *t2, Oid collid)
{
	...
	int			result;
    char *backupBuf1 = NULL;  /* for MySQL mode*/
    char *backupBuf2 = NULL;  /* for MySQL mode*/
    bool lowered = false;  /* for MySQL mode*/
...
texteq(PG_FUNCTION_ARGS)
{
        ...
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
            ...
            pg_newlocale_from_collation(collid)->deterministic)
        {
            ...
            int64 orig_len1 = strlen(str1);
            Datum datum2 = PG_GETARG_DATUM(1);
            char* str2 = TextDatumGetCString(datum2);
            int64 orig_len2 = strlen(str2);
            int64 len1 = orig_len1;
            int64 len2 = orig_len2;
            int64 i;
					  ...
					  List **namelist)
{
    ...
    if ((MyProcPort != NULL) && 
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        return mys_SplitIdentifierString(rawstring, separator, namelist);
    }
    else
    {
        /* Nothing to do. */
...
replace_text(PG_FUNCTION_ARGS)
{
        ...
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        if (collid == caseInsensitiveId)
        {
            text_position_setup(src_text, from_sub_text, 100, &state);
        }
        else 
        {
...
split_text(FunctionCallInfo fcinfo, SplitTextOutputData *tstate)
{
    ...
    if ((MyProcPort != NULL) && 
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        if (collation == caseInsensitiveId)
        {
            collation = 100;
        }
    }
