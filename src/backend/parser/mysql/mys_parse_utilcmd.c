...
setDefaultValWithSeq(char *namespaceName, char *seqName, char *colName)
{
    ...
    snprintf(nextvalFuncArgStr, 1024, "%s.%s", namespaceName, seqName);
    nextvalFuncArg = makeNode(A_Const);
    nextvalFuncArg->val.sval.type = T_String;
    nextvalFuncArg->val.sval.sval = pstrdup(nextvalFuncArgStr);
    nextvalFuncArg->location = -1;
    nextvalFuncArgs = list_make1(nextvalFuncArg);
    nextvalFuncCall = makeFuncCall(nextvalFuncName, 
                                   nextvalFuncArgs, 
...
createAutoUpdateTimeStampTriggerFunc(char *namespace, char *relName, char *colName)
{
	...
	snprintf(funcBody,
			 funcBodySize,
			 "begin if (NEW.%s IS NOT DISTINCT FROM OLD.%s) AND (NEW IS DISTINCT FROM OLD) THEN NEW.%s = pg_catalog.now(); end if; return NEW; end;",
			 colName,
			 colName,
			 colName);
    ...
    else
    {
        ...
        if (autoIncrementColumn->raw_default == NULL)
        {
            /* OK */
				 ...
				 ((A_Const *)autoIncrementColumn->raw_default)->isnull)
        {
            /* OK */
        }
        else
		...
		snamenode->val.sval.type = T_String;
						 ...
						 char **snamespace_p, char **sname_p)
{
	 ...
	 * used by ud_dump.  Else, generate a name.
	 *
...
transformTableLikeClause(CreateStmtContext *cxt, TableLikeClause *table_like_clause)
{
	...
	{
		aclresult = object_aclcheck(TypeRelationId, relation->rd_rel->reltype,
									GetUserId(), ACL_USAGE);
    ...
    if (cxt->AutoIncrement)
    {
        ...
        if (attnum > 0)
        {
            ...
            else
            {
                ...
                seqNameNode->val.sval.type = T_String;
                seqNameNode->val.sval.sval = seqString;
