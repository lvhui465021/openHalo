...
static Node *makeByteaStringConst(char *str, int location);
            ...
            MysqlUserVariableName plassign_equals a_expr
                {
                    ...
                    List *argList = list_make2(makeStringConst($1, @1), $3);
            ...
            | SET TRANSACTION transaction_mode_list
                {
                    ...
                    foreach(lc, $3)
                    {
                        ...
                        if (((A_Const*)(defElem->arg))->val.type == T_Integer)
                        {
                            int val = ((A_Const*)(defElem->arg))->val.val.ival;
                            snprintf(arg, 256, "%d", val);
                        }
                        else
                        {
                            char *val = ((A_Const*)(defElem->arg))->val.val.str;
                            int valLen = strlen(val);
                            memcpy(arg, val, valLen);
                            arg[valLen] = '\0';
                        }
                        argList = list_make2(makeStringConst(pstrdup(defElem->defname), -1),
                                             makeStringConst(pstrdup(arg), -1));
            ...
            IDENT plassign_equals on_a_expr
                {
                    ...
                    if (IsA($3, ColumnRef))
                    {
                        ...
                        foreach(lc, c->fields)
                        {
                            ...
                            if (strncasecmp(v->val.str, "off", 3) != 0)
                            {
                                argList = list_make2(makeStringConst($1, @1), makeStringConst(v->val.str, -1));
                            }
                            else
                            {
                                argList = list_make2(makeStringConst($1, @1), 
                                                     makeIntConst(0, -1));
                    ...
                    {
                        argList = list_make2(makeStringConst($1, @1), $3);
                    }
                    ...
                    if (isSystemVariable($2))
                    {
                        ...
                        if (IsA($4, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(v->val.str, "off", 3) != 0)
                                {
                                    argList = list_make2(makeStringConst($2, @2), makeStringConst(v->val.str, -1));
                                }
                                else
                                {
                                    argList = list_make2(makeStringConst($2, @2), 
                                                         makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(makeStringConst($2, @2), $4);
                        }
                        funcCall = (Node *) makeFuncCall(funcName, argList, COERCE_EXPLICIT_CALL, @1);
                        result->name = NULL;
                        result->indirection = NIL;
                    ...
                    if (strncasecmp($1, "local.", 6) == 0)
                    {
                        ...
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(v->val.str, "off", 3) != 0)
                                {
                                    argList = list_make2(makeStringConst(($1 + 6), @1), makeStringConst(v->val.str, -1));
                                }
                                else
                                {
                                    argList = list_make2(makeStringConst(($1 + 6), @1), 
                                                         makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(makeStringConst(($1 + 6), @1), $3);
                        }
                    }
                    else if (strncasecmp($1, "session.", 8) == 0)
                    {
                        ...
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(v->val.str, "off", 3) != 0)
                                {
                                    argList = list_make2(makeStringConst(($1 + 8), @1), makeStringConst(v->val.str, -1));
                                }
                                else
                                {
                                    argList = list_make2(makeStringConst(($1 + 8), @1), 
                                                         makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(makeStringConst(($1 + 8), @1), $3);
                        }
                    }
                    else if (strncasecmp($1, "global.", 7) == 0)
                    {
                        ...
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(v->val.str, "off", 3) != 0)
                                {
                                    argList = list_make2(makeStringConst(($1 + 7), @1), makeStringConst(v->val.str, -1));
                                }
                                else
                                {
                                    argList = list_make2(makeStringConst(($1 + 7), @1), 
                                                         makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(makeStringConst(($1 + 7), @1), $3);
                        }
                    }
                    else
                    {
			...
			| NAMES IDENT
                {
                    ...
                    List *argList = list_make1(makeStringConst($2, @2));
...
on_a_expr:
                ...
                { 
                    ...
                    {
                        $$ = makeStringConst("halo_mysql_system_var_default", @1);
                    }
                    else 
                    {
                        $$ = $1; 
            ...
            | SHOW VARIABLES LIKE SCONST 
                {
                    ...
                    whereClause = makeSimpleA_Expr(AEXPR_LIKE, "~~", 
                                                   makeColumnRef("variable_name", NIL, -1, yyscanner), 
                                                   makeStringConst($4, -1), -1);
			...
			SHOW DATABASES
				{
					...
					*/
                    ...
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("Database", "nspname", yyscanner));

                    selectStmt->fromClause = list_make1(createRangeVar("pg_catalog", "pg_namespace"));

					leftExpr = (Node *) makeA_Expr(AEXPR_OP,
												 list_make1(makeString(pstrdup("!~"))),
												 makeColumnRef("nspname", NIL, -1, yyscanner),
												 makeStringConst("^pg_", -1),
												 -1);
					rightExpr = (Node *) makeA_Expr(AEXPR_OP,
												 list_make1(makeString(pstrdup("<>"))),
												 makeColumnRef("nspname", NIL, -1, yyscanner),
												 makeStringConst("public", -1),
												 -1);
					leftExpr = makeAndExpr(leftExpr, rightExpr, -1);
					rightExpr = (Node *) makeA_Expr(AEXPR_OP,
												 list_make1(makeString(pstrdup("<>"))),
												 makeColumnRef("nspname", NIL, -1, yyscanner),
												 makeStringConst("mys_informa_schema", -1),
												 -1);
					selectStmt->whereClause = makeAndExpr(leftExpr, rightExpr, -1);

					order = (SortBy *) makeNode(SortBy);
					order->node = makeIntConst(1, -1);
                    ...
                    if ($3 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)makeStringConst($3->relname, -1), 
                                                         -1); 
                    }
                    else
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)makeStringConst(get_namespace_name(getCurrentNamespaceOid()), -1), 
                                                         -1); 
                    }
                    if ($4 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                makeAndExpr(selectStmtTables->whereClause, 
                                            (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                      "~~", 
                                                                      makeColumnRef("table_name", NIL, -1, yyscanner), 
                                                                      makeStringConst($4, -1), 
                                                                      -1), 
                                            -1);
                    }
                    ...
                    if ($4 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)makeStringConst($4->relname, -1), 
                                                         -1); 
                    }
                    else
                    {
                ...
                | desc_describe_explain qualified_name col_name_wild_for_desc
				{
                    ...
                    if ($2->schemaname != NULL)
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")),
                                                ...
                                                list_make2(makeStringConst(pstrdup($2->schemaname), -1),
                                                           makeStringConst(pstrdup($2->relname), -1)),
                                                COERCE_EXPLICIT_CALL,
                                                -1);
                    }
                    else
                    ...
                    if ($3 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP,
                                                                            "~~",
                                                                            makeColumnRef("Field", NIL, -1, yyscanner),
                                                                            makeStringConst($3, -1),
                                                                            -1);
                    }
			...
			| SHOW show_columns_fields_opt show_from_table_clause show_from_db_clause show_like_clause where_clause
				{
                    ...
                    */
                    ...
                    if ($4 != NULL)
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_table_columns"))),
                                                list_make2(makeStringConst(pstrdup($4->relname), -1), 
                                                           makeStringConst(pstrdup($3->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
                    else
                    {
                        if ($3->schemaname != NULL)
                        {
                            funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                               makeString(pstrdup("show_table_columns"))),
                                                    list_make2(makeStringConst(pstrdup($3->schemaname), -1), 
                                                               makeStringConst(pstrdup($3->relname), -1)),
                                                    COERCE_EXPLICIT_CALL, 
                                                    -1);
                        }
                    ...
                    */
                    ...
                    if ($5 != NULL)
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                ...
                                                list_make2(makeStringConst(pstrdup($5->relname), -1), 
                                                           makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
                    else
                    {
                        if ($4->schemaname != NULL)
                        {
                            funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                               makeString(pstrdup("show_table_full_columns"))),
                                                    list_make2(makeStringConst(pstrdup($4->schemaname), -1), 
                                                               makeStringConst(pstrdup($4->relname), -1)),
                                                    COERCE_EXPLICIT_CALL, 
                                                    -1);
                        }
            ...
            | SHOW CREATE FUNCTION qualified_name
                {
                    ...
                    else 
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_create_function"))),
                                                list_make2(makeStringConst(get_namespace_name(getCurrentNamespaceOid()), -1), 
                                                           makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
            ...
            | SHOW FUNCTION STATUS show_like_clause where_clause
                {
                    ...
                    if ($4 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                            "~~", 
                                                                            makeColumnRef("name", NIL, -1, yyscanner),
                                                                            makeStringConst($4, -1),
                                                                            -1);
                    }
                    else if ($5 != NULL)
			...
			| SHOW TRIGGERS show_from_db_clause show_like_clause where_clause
				{
                    ...
                    if ($3 != NULL)
                    {
                        selectStmt->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)makeColumnRef("trigger_schema", NIL, -1, yyscanner), 
                                                         (Node *)makeStringConst($3->relname, -1), 
                                                         -1); 
                    }
                    else
					...
					if ($4 != NULL)
                    {
						Node *cond4 = (Node *) makeSimpleA_Expr(AEXPR_OP, 
																"~~", 
																makeColumnRef("event_object_table", NIL, -1, yyscanner),
																makeStringConst($4, -1),
																-1);
			...
			| SHOW CREATE TRIGGER qualified_name
				{
					...
					funcCall = makeFuncCall(list_make2(makeString("mysql"), makeString("show_create_trigger")),
											list_make2(makeStringConst($4->schemaname, -1), makeStringConst($4->relname, -1)),
											COERCE_EXPLICIT_CALL,
											-1);
            ...
            LOCK_P Lock_Tables LockTables
                {
                    ...
                    funcName = list_make2(makeString(pstrdup("mysql")),
                                          makeString(pstrdup("set_system_session_variable")));
                    argList = list_make2(makeStringConst(pstrdup("halo_mysql_dummy_stmt_return_ok"), -1),
			...
			| PRIMARY KEY opt_index_name access_method_clause '(' index_params ')' index_options
				{
					...
					n->contype = CONSTR_PRIMARY;
					n->location = @1;
                    // Mysql 主键名只能是"primary"，使用这种方法实现的话，Halo将会存在问题，
                    // Halo约束与相关索引的名字相同，但索引的名字在同一个schema下唯一
			...
			| DROP object_type_any_name any_name_list opt_drop_behavior on_table index_algorithm_opt
				{
                    ...
                    if ($5 != NULL)
                    {
                        ...
                        foreach(lc, $3)
                        {
                            ...
                            {
                                names = list_insert_nth(names, 1, makeStringConst(pstrdup($5->relname), -1));
                            }
			...
			| DROP TEMPORARY object_type_any_name any_name_list opt_drop_behavior on_table index_algorithm_opt
				{
                    ...
                    if ($6 != NULL)
                    {
                        ...
                        foreach(lc, $4)
                        {
                            ...
                            {
                                names = list_insert_nth(names, 1, makeStringConst(pstrdup($6->relname), -1));
                            }
                                ...
                                if (resTarget->val->type == T_A_Expr)
                                {
                                    ...
                                    if (lexpr->type == T_ColumnRef)
                                    {
                                        ...
                                        if (list_length(columnRef->fields) == 1)
                                        {
                                            ...
                                            {
                                                Value *val = (Value *)lfirst(subLc);
                                                Node *colNode = (Node *)makeString(val->val.str);
                                                List *indi = list_make1(colNode);
                                                ((A_Expr *)(resTarget->val))->lexpr = (Node *)makeColumnRef(tableName, indi, -1, 0);
                                                break;
                                            }
                                        }
                                    }
                                    if (rexpr->type == T_ColumnRef)
                                    {
                                        ...
                                        if (list_length(columnRef->fields) == 1)
                                        {
                                            ...
                                            foreach (subLc, columnRef->fields)
                                            {
                                                ...
                                                Node *colNode = (Node *)makeString(val->val.str);
                                                List *indi = list_make1(colNode);
                                                ((A_Expr *)(resTarget->val))->rexpr = (Node *)makeColumnRef(tableName, indi, -1, 0);
                                                break;
                                            }
                                        }
                                    }
...
DeleteStmt: with_clause DELETE_P delete_options FROM relation_expr_opt_alias using_clause
                where_or_current_clause opt_sort_clause opt_select_limit returning_clause
				{
                    ...
                    if ($9 && $9->limitOption != LIMIT_OPTION_DEFAULT)
                    {
            ...
            | DELETE_P delete_options FROM relation_expr_opt_alias using_clause
                where_or_current_clause opt_sort_clause opt_select_limit returning_clause
				{
                    ...
                    if ($8 && $8->limitOption != LIMIT_OPTION_DEFAULT)
                    {
                        ...
                        foreach (lc, $5)
                        {
                            ...
                            if ((i % 2) == 0)
                            {
                                if (node != NULL)
                                {
                                    ...
                                    if (i == 0)
                                    {
                                        targetRelationName = rel->val.val.str;
                                        len = strlen(targetRelationName);
                                    }
                                    else
                                    {
                                        if (strncasecmp(targetRelationName, rel->val.val.str, len) != 0)
                                        {
                                            ereport(ERROR,
                                                    (errcode(ERRCODE_SYNTAX_ERROR),
                    ...
                    if (IsA($5, ColumnRef))
                    {
                        ...
                        if (list_length(columnRef->fields) == 1)
                        {
                            ...
                            if (columnName != NULL)
                            {
                                ...
                                newColumnRef = makeColumnRef("excluded", indi, -1, 0);
                                $1->val = newColumnRef;
                                $$ = list_make1($1);
                            }
                            else
			...
			| ColId '.' ColId '=' a_expr
				{
					...
					$$ = list_make2(makeStringConst($1, -1), (Node*)resTarget);
				}
			| USER '=' a_expr
            {
                ResTarget* resTarget = makeNode(ResTarget);
 ...
 UsedbStmt: USE name
                {
					...
					n->args = list_make4(makeStringConst($2, @2),
                                         makeStringConst(pstrdup("mysql"), -1),
                                         makeStringConst(pstrdup("pg_catalog"), -1),
                                         makeStringConst(pstrdup("public"), -1));
                    n->is_local = false;
					$$ = (Node *)n;
                }
            ;
...
separator_clause:
			SEPARATOR Sconst					    { $$ = makeStringConst($2, @2); }
...
SimpleTypename:
            ...
            | BINARY '(' Iconst ')'                 {
                ...
                $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                         makeString("binary")));
                //$$->typmods = list_make1(makeIntConst($3, -1));
                }
            | VARBINARY '(' Iconst ')'              {
                //$$ = SystemTypeName("bytea");
                $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
            ...
            BIT opt_varying '(' ICONST ')'
				{
                    ...
                    $$->typmods = list_make1(makeIntConst($4, -1));
                    $$->location = @1;
				}
			| BIT opt_varying
				{
            ...
            LONG varchar opt_charset_with_opt_binary
                {
                    ...
                    //        if (strncasecmp(v->val.str, "binary", 6) == 0)
                    //        {
                    //            $$ = SystemTypeName("bytea");
                    //        }
                    //    }
            ...
            | TEXT_P opt_field_length opt_charset_with_opt_binary
                {
                    ...
                    //            $$ = SystemTypeName("bytea");
                    //        }
                    //        //else if (strncasecmp(v->val.str, "byte", 4) == 0)
                    //        //{
                    //        //    //$$ = SystemTypeName("bytea");
                    //        //    $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
			...
			| a_expr '^' a_expr
				{
                    ...
                    //ereport(ERROR,
                    //        (errcode(ERRCODE_SYNTAX_ERROR),
                    //         errmsg("Halo-MySQL does not support ^ operator"),
                    //         parser_errposition(@2)));
                }
            ...
            INTERVAL a_expr interval_option
                {
                    ...
                    {
                        Value *unit = (Value *)linitial($3);
                        if (strcmp(unit->val.str, "week") == 0)
                        {
                            multiple = 7;
                            intervalTypeMods = list_make1(makeIntConst(INTERVAL_MASK(DAY), @3));
                        }
                        else if (strcmp(strVal(unit), "quarter") == 0)
                        {
                            multiple = 3;
                            intervalTypeMods = list_make1(makeIntConst(INTERVAL_MASK(MONTH), @3));
                        }
                        else // if (strcmp(unit->val.str, "microsecond") == 0)
                        {
                            divisor = 1000000;
                            intervalTypeMods = list_make1(makeIntConst(INTERVAL_MASK(SECOND), @3));
                        }
                    }
                    else
                    {
                    ...
                    if (multiple > 0)
                    {
                        result = (Node *) makeSimpleA_Expr(AEXPR_OP, "*", (Node *)transferType2Interval, (Node *)makeIntConst(multiple, -1), -1);
                    }
                    else if (divisor > 0)
                    {
                        ...
                        int2Numeric->arg = (Node *)makeIntConst(divisor, -1);
            ...
            IDENT
                {
                    ...
                    {
                        $$ = list_make2(makeIntConst(INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "minute_second") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "hour_microsecond") == 0)
                    {
                        $$ = list_make2(makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "hour_second") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "hour_minute") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE), @1));
                    }
                    else if (strcmp($1, "day_microsecond") == 0)
                    {
                        $$ = list_make2(makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "day_second") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "day_minute") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE), @1));
                    }
                    else if (strcmp($1, "day_hour") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR), @1));
                    }
                    else if (strcmp($1, "year_month") == 0)
                    {
                        $$ = list_make1(makeIntConst(INTERVAL_MASK(YEAR) | INTERVAL_MASK(MONTH), @1));
                    }
                    else
                    {
                        ereport(ERROR,
...
func_expr_common_subexpr:
            ...
            | REPEAT '(' a_expr ',' a_expr ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("repeat")),
											   list_make2($3, $5),
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
...
columnref:	ColId
            ...
            | USER
				{
					$$ = makeColumnRef(pstrdup("user"), NIL, @1, yyscanner);
				}
			| ColId indirection
				{
			...
			| USER indirection
				{
					$$ = makeColumnRef(pstrdup("user"), $2, @1, yyscanner);
				}
		;
...
AexprConst: Iconst
            ...
            | UNDERSCORE_BINARY BCONST
                {
                    $$ = makeBitStringConst($2, @2);
                }
            | UNDERSCORE_BINARY XCONST
                {
                    $$ = makeByteaStringConst($2, @2);
                }
			//| func_name Sconst
			//	{
            ...
            /* | TIMESTAMP '(' Sconst ')'
                {
                    ...
                    $$ = (Node *)makeStringConstCast($3, @3, t);
                } */
            | TIMESTAMP '(' a_expr ')'
                {
                    TypeName *t = SystemTypeName("timestamp");

                    $$ = (Node *)makeTypeCast($3, t, @3);
                }
            ...
            | TIMESTAMP Sconst
                {
                    ...
                    $$ = (Node *)makeStringConstCast($2, @2, t);
                }
            | TIME '(' a_expr ')'
                {
            ...
            | ConstInterval Iconst opt_interval
                {
                    ...
                    $$ = (Node *)makeStringConstCast(s, @2, t);
                }
			| ConstInterval '(' Iconst ')' Sconst
				{
					TypeName *t = $1;
...
static Node *
makeByteaStringConst(char *str, int location)
{
	A_Const *n = makeNode(A_Const);

	n->val.type = T_ByteaString;
	n->val.val.str = str;
	n->location = location;
...
createColumnDef(char *columnName, char *columnType, int typeMod)
{
    ...
    if (0 < typeMod)
    {
        columnDef->typeName->typmods = list_make1(makeIntConst(typeMod, -1));
    }
    else 
    {
    ...
    if (node != NULL)
    {
        if (IsA(node, ColumnRef))
        {
            ...
            foreach (lc, columnRef->fields)
            {
                ...
                {
                    if (('a' <= val->val.str[0]) && (val->val.str[0] <= 'z'))
                    {
                        val->val.str[0] -= 32;
                    }
                }
...
createResTargetWithColumn(char *name, char *val, core_yyscan_t yyscanner)
{
    ...
    resTarget->indirection = NIL; 
    resTarget->val = (Node *)makeColumnRef(val, NIL, -1, yyscanner);
    resTarget->location = -1; 
...
createResTargetWithString(char *name, char *val)
{
    ...
    if (val != NULL)
    {
        resTarget->val = (Node *)makeStringConst(pstrdup(val), -1);
    }
    else 
    {
        resTarget->val = (Node *)makeNullAConst(-1);
    }
    resTarget->location = -1; 
...
createResTargetWithInt(char *name, int val)
{
    ...
    resTarget->indirection = NIL; 
    resTarget->val = (Node *)makeIntConst(val, -1);
    resTarget->location = -1; 
                             ...
                             char *arg1, char *arg2)
{
    ...
    if (arg1 != NULL)
    {
        funcArgs = lappend(funcArgs, makeStringConst(pstrdup(arg1), -1));
    }
    else 
    {
        funcArgs = lappend(funcArgs, makeNullAConst(1));
    }
    if (arg2 != NULL)
    {
        funcArgs = lappend(funcArgs, makeStringConst(pstrdup(arg2), -1));
    }
    else 
    {
        funcArgs = lappend(funcArgs, makeNullAConst(1));
