 ...
 *
 * mys_gram.y
 *    MySQL-compatibility grammar.
 *
 * Re-parented from openHalo PG14 via UDB-TX PG16 adaptation and
 * further adapted to PG18 APIs (ParseLoc, ReturningClause, etc.).
 *
 * Portions Copyright (c) 2026, HaloLab / UDB-TX Contributors
 *
 * src/backend/parser/mysql/mys_gram.y
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#define parser_yyerror(msg)  mys_scanner_yyerror(msg, yyscanner)
#define parser_errposition(pos)  mys_scanner_errposition(pos, yyscanner)

static int param_index = 1;

static void mys_yyerror(YYLTYPE *yylloc, core_yyscan_t yyscanner,
						 const char *msg);


static RawStmt *makeRawStmt(Node *stmt, int stmt_location);
static void updateRawStmtEnd(RawStmt *rs, int end_location);
static Node *mys_makeColumnRef(char *colname, List *indirection,
						   int location, core_yyscan_t yyscanner);
static Node *mys_makeTypeCast(Node *arg, TypeName *typename, int location);
static Node *mys_makeStringConst(char *str, int location);
static Node *mys_makeStringConstCast(char *str, int location, TypeName *typename);
static Node *mys_makeIntConst(int val, int location);
static Node *mys_makeFloatConst(char *str, int location);
static Node *mys_makeBitStringConst(char *str, int location);
static Node *mys_makeByteaStringConst(char *str, int location);
static Node *mys_makeNullAConst(int location);
static Node *mys_makeAConst(Node *v, int location);
static Node *mys_makeBoolAConst(bool state, int location);
...
%type <node>	stmt toplevel_stmt routine_body_stmt
		...
		GrantStmt GrantRoleStmt ImportForeignSchemaStmt IndexStmt InsertStmt MysInsertStmt
		ListenStmt LoadStmt NotifyStmt ExplainableStmt PreparableStmt
...
%type <str>		copy_file_name
				...
				index_access_method_clause table_access_method_clause name cursor_name file_name
				opt_index_name cluster_index_specification index_opt
...
%type <list>	parse_toplevel stmtmulti routine_body_stmt_list
				...
				createfunc_opt_list opt_createfunc_opt_list alterfunc_opt_list
				...
				distinct_clause distinct_clause1 distinct_optimizer_hints_clause
				target_list opt_target_list insert_column_list set_target_list
				set_clause_list set_clause_list_for_mysql relation_expr_opt_aliass relation_expr_opt_alias_list set_clause set_clause_for_mysql
...
%type <list>    alter_table_options opt_create_table_options
...
%type <node>	join_qual
%type <jtype>	join_type update_join_type

%type <list>	extract_list overlay_list timestampadd_list timestampdiff_list position_list
%type <list>	substr_list trim_list
%type <list>	opt_interval interval_option interval_second
...
%type <range>	relation_expr on_table
...
/*%type <typnam>	MySQLConvertType Typename SimpleTypename ConstTypename*/
%type <typnam>	MySQLConvertType Typename SimpleTypename
				GenericType Numeric opt_float Text
                Character
...
/*				Bit ConstBit BitWithLength BitWithoutLength*/
				Bit Blob
%type <str>		character ncharacter charset_name opt_field_option
%type <list>    opt_charset_with_opt_binary
%type <str>		extract_arg
%type <boolean> opt_varying opt_timezone opt_no_inherit

%type <ival>	Iconst SignedIconst now
%type <str>     opt_field_length field_length
...
%type <node>    opt_references references
...
%type <node> separator_clause
%type <node> array_agg_list
%type <list> character_set_collates
%type <node> character_set_collate
%type <node> stored_virtual
%type <list> index_options
%type <list>	grant_role_opt_list
%type <defelt>	grant_role_opt
%type <node>	grant_role_opt_value
%type <node> index_option
%type <node> algorithm_value
%type <str> show_like_clause
%type <range> show_from_table_clause
%type <range> show_from_db_clause
%type <str> col_name_wild_for_desc
 ...
 * src/include/parser/mysql/mys_kwlist.h and add new keywords to the appropriate one
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	AGGREGATE ALGORITHM ALL ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ANY_VALUE ARRAY AS ASC ASCII
	ASENSITIVE ASSERTION ASSIGNMENT ASYMMETRIC ATOMIC AT ATTACH ATTRIBUTE 
    AUTO_INCREMENT AVG_ROW_LENGTH

	BACKWARD BEFORE BEGIN_P BETWEEN BIGINT BINARY BIT BIT_AS_BYTEA BLOB
	BOOL_P BOOLEAN_P BOTH BREADTH BY BYTE

	CACHE CALL CALLED CASCADE CASCADED CASE CAST CATALOG_P CHAIN CHANGE CHAR_P
	CHARACTER CHARACTERISTICS CHARSET CHECK CHECKPOINT CHECKSUM CLASS CLOSE
	CLUSTER COALESCE COLLATE COLLATION COLUMN COLUMNS COLUMN_FORMAT COMMENT COMMENTS COMMIT
	COMMITTED COMPRESSION CONCURRENTLY CONFIGURATION CONFLICT
	CONNECTION CONSTRAINT CONSTRAINTS CONTAINS CONTENT_P CONTINUE_P CONVERSION_P CONVERT COPY
	...
	INNER_P INOUT INPLACE INPUT_P INSENSITIVE INSERT INSERT_METHOD INSTEAD INT_P INTEGER
	INTERSECT INTERVAL INTO INVISIBLE INVOKER IS ISOLATION ITERATE
...
/*
 * The grammar thinks these are keywords, but they are not in the mysql/mys_kwlist.h
 * list and so can never be entered directly.  The filter in mys_parser.c
 ...
 */
%token		NOT_LA NULLS_LA WITH_LA BINARY_LA
...
/*
 * 解决a_expr中使用:=为用户变量赋值的情况
 *      需要COLON_EQUALS为右结合，并且优先级比a_expr中的其他操作符低
 */
%right      COLON_EQUALS
%left		OR
%left		AND Op_And Op_Or
%right		NOT
%nonassoc	IS NOTNULL	/* IS sets precedence for IS NULL, etc */
%nonassoc	'<' '>' '=' LESS_EQUALS GREATER_EQUALS NOT_EQUALS XOR
%nonassoc	BETWEEN IN_P LIKE ILIKE RLIKE REGEXP BINARY_P SIMILAR NOT_LA
...
parse_toplevel:
			stmtmulti
			{
				mys_yyget_extra(yyscanner)->parsetree = $1;
			}
			| MODE_TYPE_NAME Typename
			{
				mys_yyget_extra(yyscanner)->parsetree = list_make1($2);
			}
			| MODE_PLPGSQL_EXPR PLpgSQL_Expr
			{
				mys_yyget_extra(yyscanner)->parsetree =
...
CallStmt:	CALL func_application
				{
                    ...
                    /* n->inProc = false; -- field removed in PG18 */
					$$ = (Node *)n;
				}
		;
...
CreateSchemaStmt:
			CREATE SCHEMA ColId character_set_collates
			    {
			...
			| CREATE SCHEMA IF_P NOT EXISTS ColId character_set_collates
				{
					CreateSchemaStmt *n = makeNode(CreateSchemaStmt);
					/* ...but not here */
					n->schemaname = $6;
...
character_set_collates:
            character_set_collates opt_comma character_set_collate
                {
                    $$ = NIL;
                }
            | /* empty */
...
/*****************************************************************************
 *
 * Set Mysql user variable
 *	  SET @name = | := var_value
 *
 *****************************************************************************/

//MysqlUserVariableSetStmt:
//            SET mysql_user_variables
//                {
...
//                }
//
//mysql_user_variables:
//            mysql_user_variable
//                { $$ = list_make1($1); }
//            | mysql_user_variables ',' mysql_user_variable
//                { $$ = lappend($1, $3); }
...
mysql_system_variable:
            IDENT plassign_equals on_a_expr
                {
                    ResTarget *result;
                    List *funcName;
                    List *argList;
                    Node *funcCall;
                    result = makeNode(ResTarget);
                    funcName = list_make2(makeString(pstrdup("mysql")),
                                          makeString(pstrdup("set_system_session_variable")));
                    argList = NIL;
                    if (IsA($3, ColumnRef))
                    {
                        ListCell   *lc;
                        ColumnRef *c = castNode(ColumnRef, $3);
                        foreach(lc, c->fields)
                        {
                            Node *v = (Node *) lfirst(lc);
                            if (strncasecmp(strVal(v), "off", 3) != 0)
                            {
                                argList = list_make2(mys_makeStringConst($1, @1), mys_makeStringConst(strVal(v), -1));
                            }
                            else
                            {
                                argList = list_make2(mys_makeStringConst($1, @1), 
                                                     mys_makeIntConst(0, -1));
                    ...
                    {
                        argList = list_make2(mys_makeStringConst($1, @1), $3);
                    }
                    funcCall = (Node *) makeFuncCall(funcName, argList, COERCE_EXPLICIT_CALL, @1);
                    result->name = NULL;
                    result->indirection = NIL;
                    result->val = funcCall;
                    result->location = @1;
            ...
            | LOCAL IDENT plassign_equals on_a_expr
                {
                    if (MysIsSessionVariableSupported($2) || isSystemVariable($2))
                    {
                        ...
                        {
                            ListCell   *lc;
                            ColumnRef *c = castNode(ColumnRef, $4);
                            foreach(lc, c->fields)
                            {
                                Node *v = (Node *) lfirst(lc);
                                if (strncasecmp(strVal(v), "off", 3) != 0)
                                {
                                    argList = list_make2(mys_makeStringConst($2, @2), mys_makeStringConst(strVal(v), -1));
                                }
                                else
                                {
                                    argList = list_make2(mys_makeStringConst($2, @2), 
                                                         mys_makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(mys_makeStringConst($2, @2), $4);
                        }
                        funcCall = (Node *) makeFuncCall(funcName, argList, COERCE_EXPLICIT_CALL, @1);
                        result->name = NULL;
                        result->indirection = NIL;
                    ...
                    else
                    {
                        ereport(ERROR,
                                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                                 errmsg("Unknown system variable '%s'", $1),
            ...
            | MysSysVarName plassign_equals on_a_expr
                {
                    ResTarget *result;
                    List *funcName;
                    List *argList;
                    ...
                    if (strncasecmp($1, "local.", 6) == 0)
                    {
                        ...
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(strVal(v), "off", 3) != 0)
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 6), @1), mys_makeStringConst(strVal(v), -1));
                                }
                                else
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 6), @1), 
                                                         mys_makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(mys_makeStringConst(($1 + 6), @1), $3);
                        }
                    }
                    else if (strncasecmp($1, "session.", 8) == 0)
                    {
                        funcName = list_make2(makeString(pstrdup("mysql")),
                                              makeString(pstrdup("set_system_session_variable")));
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(strVal(v), "off", 3) != 0)
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 8), @1), mys_makeStringConst(strVal(v), -1));
                                }
                                else
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 8), @1), 
                                                         mys_makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(mys_makeStringConst(($1 + 8), @1), $3);
                        }
                    }
                    else if (strncasecmp($1, "global.", 7) == 0)
                    {
                        funcName = list_make2(makeString(pstrdup("mysql")),
                                              makeString(pstrdup("set_system_global_variable")));
                        if (IsA($3, ColumnRef))
                        {
                            ...
                            foreach(lc, c->fields)
                            {
                                ...
                                if (strncasecmp(strVal(v), "off", 3) != 0)
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 7), @1), mys_makeStringConst(strVal(v), -1));
                                }
                                else
                                {
                                    argList = list_make2(mys_makeStringConst(($1 + 7), @1), 
                                                         mys_makeIntConst(0, -1));
                                }
                                break;
                            }
                        ...
                        {
                            argList = list_make2(mys_makeStringConst(($1 + 7), @1), $3);
                        }
                    }
                    else
                    {
			...
			| NAMES IDENT
                {
                    ...
                    List *funcName = list_make2(makeString(pstrdup("mysql")),
                                                makeString(pstrdup("set_system_names_variable")));
                    List *argList = list_make1(mys_makeStringConst($2, @2));
                    Node *funcCall = (Node *) makeFuncCall(funcName, argList, COERCE_EXPLICIT_CALL, @1);
                    result->name = NULL;
					result->indirection = NIL;
					result->val = funcCall;
					result->location = @1;
			...
			| NAMES SCONST opt_collate_clause
                {
                    ResTarget *result = makeNode(ResTarget);
                    List *funcName = list_make2(makeString(pstrdup("mysql")),
                                                makeString(pstrdup("set_system_names_variable")));
...
set_rest_more:	/* Generic SET syntaxes: */
			...
			| SCHEMA Sconst
				{
					...
					n->kind = VAR_SET_VALUE;
					n->name = "search_path";
					n->args = list_make1(mys_makeStringConst($2, @2));
					$$ = n;
				}
			| NAMES opt_encoding
				{
			...
			//		n->args = list_make1(mys_makeStringConst($3, @3));
			//		$$ = n;
			//	}
			...
			| ConstInterval Sconst opt_interval
				{
					...
					t->typmods = $3;
					$$ = mys_makeStringConstCast($2, @2, t);
				}
			| ConstInterval '(' Iconst ')' Sconst
				{
					TypeName *t = $1;
					t->typmods = list_make2(mys_makeIntConst(INTERVAL_FULL_RANGE, -1),
											mys_makeIntConst($3, @3));
					$$ = mys_makeStringConstCast($5, @5, t);
				}
			| NumericOnly							{ $$ = mys_makeAConst($1, @1); }
			| DEFAULT								{ $$ = NULL; }
			| LOCAL									{ $$ = NULL; }
		;
...
VariableShowStmt:
            ...
            | SHOW VARIABLES LIKE SCONST 
                {
                    // select * from mys_informa_schema.empty_session_variables where variable_name LIKE 'lower_case_%'
            ...
            | SHOW LOCAL VARIABLES LIKE SCONST 
                {
                    // select * from mys_informa_schema.empty_session_variables where variable_name LIKE 'lower_case_%'
					SelectStmt *selectStmt;
					ResTarget *resTarget;
            ...
            | SHOW SESSION VARIABLES LIKE SCONST 
                {
                    // select * from mys_informa_schema.empty_session_variables where variable_name LIKE 'lower_case_%'
					SelectStmt *selectStmt;
					ResTarget *resTarget;
            ...
            | SHOW GLOBAL VARIABLES LIKE SCONST 
                {
                    // select * from mys_informa_schema.empty_global_variables where variable_name LIKE 'lower_case_%'
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
                    A_Expr *whereClause;
            ...
            | SHOW VARIABLES where_clause 
                {
                    /* SHOW VARIABLES where variable_name = '**' OR Variable_name ='language' OR Variable_name = 'net_write_timeout' */
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
            ...
            | SHOW LOCAL VARIABLES where_clause 
                {
                    /* SHOW local VARIABLES where variable_name = '**' OR Variable_name ='language' OR Variable_name = 'net_write_timeout' */
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
            ...
            | SHOW SESSION VARIABLES where_clause 
                {
                    /* SHOW session VARIABLES where variable_name = '**' OR Variable_name ='language' OR Variable_name = 'net_write_timeout' */
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
            ...
            | SHOW GLOBAL VARIABLES where_clause 
                {
                    /* SHOW global VARIABLES where variable_name = '**' OR Variable_name ='language' OR Variable_name = 'net_write_timeout' */
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
            ...
            | SHOW show_engines_opt ENGINES
                {
                    /* select * from mys_informa_schema.engines; */
					SelectStmt *selectStmt;
					ResTarget *resTarget;
					ColumnRef *columnRef;
                    ...
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "engines"));
            ...
            | SHOW PLUGINS
                {
                    ...
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "plugins"));
            ...
            | SHOW COLLATION
                {
                    ...
                    /*  CHARACTER_SET_NAME as Charset, Id, \ */
                    /*  IS_DEFAULT as Default, IS_COMPILED as Compiled, \ */
                    /*  Sortlen from mys_informa_schema.collations order by 1"; */
					SelectStmt *selectStmt;
                    List *targetList;

					selectStmt = makeNode(SelectStmt);
                    targetList = NIL;
                    targetList = lappend(targetList, createResTargetWithColumn("Collation", "collation_name", yyscanner));
                    targetList = lappend(targetList, createResTargetWithColumn("Charset", "character_set_name", yyscanner));
                    targetList = lappend(targetList, createResTargetWithColumn(NULL, "id", yyscanner));
                    targetList = lappend(targetList, createResTargetWithColumn("Default", "is_default", yyscanner));
                    targetList = lappend(targetList, createResTargetWithColumn("Compiled", "is_compiled", yyscanner));
                    targetList = lappend(targetList, createResTargetWithColumn(NULL, "sortlen", yyscanner));
					selectStmt->targetList = targetList;
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "collations"));
					$$ = (Node *)selectStmt;
                }
		;
...
DBTableInfoShowStmt:
			...
			| SHOW TABLES show_from_db_clause show_like_clause where_clause
				{
					...
					SelectStmt *selectStmt;
                    char tableNameAlias[1024];
					SelectStmt *selectStmtTables;
					SelectStmt *selectStmtViews;
					SortBy *order;
					...
					selectStmtTables = makeNode(SelectStmt);
                    selectStmtTables->targetList = NIL;
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn(tableNameAlias, "table_name", yyscanner));
                    selectStmtTables->fromClause = list_make1(createRangeVar("mys_informa_schema", "tables"));
                    if ($3 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)mys_makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)mys_makeStringConst($3->relname, -1), 
                                                         -1); 
                    }
                    else
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)mys_makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)mys_makeStringConst(get_namespace_name(getCurrentNamespaceOid()), -1), 
                                                         -1); 
                    }
                    if ($4 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                makeAndExpr(selectStmtTables->whereClause, 
                                            (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                      "~~", 
                                                                      mys_makeColumnRef("table_name", NIL, -1, yyscanner), 
                                                                      mys_makeStringConst($4, -1), 
                                                                      -1), 
                                            -1);
                    }
                    else if ($5 != NULL)
					...
					selectStmtViews = makeNode(SelectStmt);
                    selectStmtViews->targetList = NIL;
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithColumn(tableNameAlias, "table_name", yyscanner));
                    selectStmtViews->fromClause = list_make1(createRangeVar("mys_informa_schema", "views"));
                    if ($3 != NULL)
                    {
                        selectStmtViews->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)mys_makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)mys_makeStringConst($3->relname, -1), 
                    ...
                    {
                        selectStmtViews->whereClause = 
                                makeAndExpr(selectStmtViews->whereClause, 
                                            (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                      "~~", 
                                                                      mys_makeColumnRef("table_name", NIL, -1, yyscanner), 
                                                                      mys_makeStringConst($4, -1), 
					...
					selectStmt = (SelectStmt *)makeSetOp(SETOP_UNION, 
                                                         true, 
                                                         (Node *)selectStmtTables, 
                                                         (Node *)selectStmtViews);

					order = (SortBy *) makeNode(SortBy);
					order->node = mys_makeIntConst(1, -1);
					order->sortby_dir = SORTBY_DEFAULT;
					order->sortby_nulls = SORTBY_NULLS_FIRST;
					order->useOp = NIL;
					order->location = -1;
					insertSelectOptions(selectStmt, list_make1(order), NIL,
										NULL, NULL, yyscanner);

					$$ = (Node *)selectStmt;
				}
			| SHOW FULL TABLES show_from_db_clause show_like_clause where_clause
				{
                    /* SHOW [FULL] TABLES [{FROM | IN} db_name] [LIKE 'pattern' | WHERE expr] */
                    /* "select table_name as `Tables_in_%s`, Table_type  */
                    /* from mys_informa_schema.views  */
                    /* where table_schema = '%s' %s %s %s  */
                    /* union  */
                    /* select table_name as `Tables_in_%s`, Table_type  */
                    /* from mys_informa_schema.tables  */
                    /* where table_schema = '%s' %s %s %s  */
                    ...
                    selectStmtTables->targetList = NIL;
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn(tableNameAlias, "table_name", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn(NULL, "table_type", yyscanner));
                    selectStmtTables->fromClause = list_make1(createRangeVar("mys_informa_schema", "tables"));
                    if ($4 != NULL)
                    {
                        selectStmtTables->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)mys_makeColumnRef("table_schema", NIL, -1, yyscanner), 
                                                         (Node *)mys_makeStringConst($4->relname, -1), 
                                                         -1); 
                    }
                    else
                    {
                    ...
                    selectStmtViews->targetList = NIL;
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithColumn(tableNameAlias, "table_name", yyscanner));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithColumn(NULL, "table_type", yyscanner));
                    selectStmtViews->fromClause = list_make1(createRangeVar("mys_informa_schema", "views"));
                    if ($4 != NULL)
                    {
                        selectStmtViews->whereClause = 
			...
			| SHOW TABLE STATUS show_from_db_clause show_like_clause where_clause
				{
                    /* SHOW TABLE STATUS [{FROM | IN} db_name] [LIKE 'pattern' | WHERE expr] */
                    /*
                     * "select table_name as Name, \
                     * NULL as Engine, NULL as Version, NULL as Row_format, \
                     * NULL as Rows, NULL as Avg_row_length, NULL as Data_length, \
                     * NULL as Max_data_length, NULL as Index_length, NULL as Data_free, \
                     * NULL as Auto_increment, NULL as Create_time, NULL as Update_time, \
                     * NULL as Check_time, NULL as Collation, NULL as Checksum, \
                     * NULL as Create_options, 'VIEW' as Comment \
                     * from mys_informa_schema.views \
                     * where Table_Schema = '%s' %s %s %s \
                     * union \
                     * select Table_Name as Name, Engine, Version, Row_format, \
                     * Table_Rows as Rows, Avg_row_length, Data_length, Max_data_length, \
                     * Index_length, Data_free, Auto_increment, Create_time, \
                     * Update_time, Check_time, Table_Collation as Collation, Checksum, \
                     * Create_options, Table_Comment as Comment \
                     * from mys_informa_schema.tables \
                     * where Table_Schema = '%s' %s %s %s", 
                     */
					SelectStmt *selectStmt;
					...
					selectStmtTables = makeNode(SelectStmt);
                    selectStmtTables->targetList = NIL;
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Name", "table_name", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Engine", "engine", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Version", "version", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Row_format", "row_format", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Rows", "table_rows", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Avg_row_length", "avg_row_length", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Data_length", "data_length", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Max_data_length", "max_data_length", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Index_length", "index_length", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Data_free", "data_free", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Auto_increment", "auto_increment", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Create_time", "create_time", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Update_time", "update_time", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Check_time", "check_time", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Collation", "table_collation", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Checksum", "checksum", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Create_options", "create_options", yyscanner));
                    selectStmtTables->targetList = lappend(selectStmtTables->targetList, createResTargetWithColumn("Comment", "table_comment", yyscanner));
                    selectStmtTables->fromClause = list_make1(createRangeVar("mys_informa_schema", "tables"));
                    if ($4 != NULL)
                    {
                        selectStmtTables->whereClause = 
					...
					selectStmtViews = makeNode(SelectStmt);
                    selectStmtViews->targetList = NIL;
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithColumn("Name", "table_name", yyscanner));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Engine", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Version", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Row_format", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Rows", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Avg_row_length", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Data_length", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Max_data_length", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Index_length", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Data_free", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Auto_increment", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Create_time", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Update_time", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Check_time", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Collation", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Checksum", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Create_options", NULL));
                    selectStmtViews->targetList = lappend(selectStmtViews->targetList, createResTargetWithString("Comment", NULL));
                    selectStmtViews->fromClause = list_make1(createRangeVar("mys_informa_schema", "views"));
                    if ($4 != NULL)
                    {
                        selectStmtViews->whereClause = 
			...
			| SHOW show_session_global_opt STATUS show_like_clause where_clause
				{
                    /* SHOW [GLOBAL | SESSION] STATUS [LIKE 'pattern' | WHERE expr] */
                    /* select Variable_name, Value from mys_informa_schema.db_status order by 1 */
					SelectStmt *selectStmt;
					SortBy *order;

					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("Variable_name", "variable_name", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("Value", "value", yyscanner));
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "db_status"));
                    if ($4 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                            "~~", 
                                                                            mys_makeColumnRef("variable_name", NIL, -1, yyscanner),
                                                                            mys_makeStringConst($4, -1),
                                                                            -1);
                ...
                | desc_describe_explain qualified_name col_name_wild_for_desc
				{
                    /* {EXPLAIN | DESCRIBE | DESC} */
					SelectStmt *selectStmt;
                    RangeFunction *rangeFunction;
                    FuncCall *funcCall;
                    List *funcAliasClause;
                    List *tableFuncElementList;

					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithStart());

                    rangeFunction = makeNode(RangeFunction);
                    rangeFunction->lateral = false;
                    rangeFunction->ordinality = false;
                    rangeFunction->is_rowsfrom = false;
                    if ($2->schemaname != NULL)
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")),
                                                           makeString(pstrdup("show_table_columns"))),
                                                list_make2(mys_makeStringConst(pstrdup($2->schemaname), -1),
                                                           mys_makeStringConst(pstrdup($2->relname), -1)),
                                                COERCE_EXPLICIT_CALL,
                                                -1);
                    }
                    else
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")),
                                                           makeString(pstrdup("show_table_columns"))),
                                                list_make2(mys_makeNullAConst(-1),
                                                           mys_makeStringConst(pstrdup($2->relname), -1)),
                                                COERCE_EXPLICIT_CALL,
                                                -1);
                    }
                    rangeFunction->functions = list_make1(list_make2((Node *)funcCall, NIL));
                    tableFuncElementList = NIL;
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Field", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Type", "varchar", 64));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Null", "varchar", 8));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Key", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Default", "text", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Extra", "text", 0));
                    funcAliasClause = list_make2(NULL, tableFuncElementList);
                    rangeFunction->alias = linitial(funcAliasClause);
                    rangeFunction->coldeflist = lsecond(funcAliasClause);
					selectStmt->fromClause = list_make1((Node *)rangeFunction);

                    if ($3 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP,
                                                                            "~~",
                                                                            mys_makeColumnRef("Field", NIL, -1, yyscanner),
                                                                            mys_makeStringConst($3, -1),
                                                                            -1);
                    }

					$$ = (Node *)selectStmt;
				}
			| SHOW show_columns_fields_opt show_from_table_clause show_from_db_clause show_like_clause where_clause
				{
                    /* SHOW COLUMNS FROM `test_db`.`ttt` */
                    /* SHOW [FULL] {COLUMNS | FIELDS} {FROM | IN} tbl_name [{FROM | IN} db_name] [LIKE 'pattern' | WHERE expr] */
                    /* SELECT * from mysql.show_table_columns('%s', '%s') as 
                    (`Field` varchar(256), `Type` varchar(64), `Null` varchar(8), `Key` varchar(256), `Default` text, `Extra` text) 
                    */
					SelectStmt *selectStmt;
                    RangeFunction *rangeFunction;
                    FuncCall *funcCall;
                    ...
                    if ($4 != NULL)
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_table_columns"))),
                                                list_make2(mys_makeStringConst(pstrdup($4->relname), -1), 
                                                           mys_makeStringConst(pstrdup($3->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
                    else
                    {
                        if ($3->schemaname != NULL)
                        {
                            funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                               makeString(pstrdup("show_table_columns"))),
                                                    list_make2(mys_makeStringConst(pstrdup($3->schemaname), -1), 
                                                               mys_makeStringConst(pstrdup($3->relname), -1)),
                                                    COERCE_EXPLICIT_CALL, 
                                                    -1);
                        }
                    ...
                    {
                        rectifyColumnsName($6);
                        selectStmt->whereClause = $6;
                    }

					$$ = (Node *)selectStmt;
				}
			| SHOW FULL show_columns_fields_opt show_from_table_clause show_from_db_clause show_like_clause where_clause
				{
                    /* SHOW FULL COLUMNS FROM `test_db`.`ttt` */
                    /* SHOW [FULL] {COLUMNS | FIELDS} {FROM | IN} tbl_name [{FROM | IN} db_name] [LIKE 'pattern' | WHERE expr] */
                    /* SELECT * from mysql.show_table_full_columns('%s', '%s') as 
                    (`Field` varchar(256), `Type` varchar(64), `Collation` varchar(256), `Null` varchar(8), 
                    `Key` varchar(256), `Default` text, `Extra` text, `Privileges` varchar(256), `Comment` varchar(512))" 
                    */
					SelectStmt *selectStmt;
                    RangeFunction *rangeFunction;
                    FuncCall *funcCall;
                    ...
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_table_full_columns"))),
                                                list_make2(mys_makeStringConst(pstrdup($5->relname), -1), 
                                                           mys_makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
                    else
                    {
                        if ($4->schemaname != NULL)
                        {
                            funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                               makeString(pstrdup("show_table_full_columns"))),
                                                    list_make2(mys_makeStringConst(pstrdup($4->schemaname), -1), 
                                                               mys_makeStringConst(pstrdup($4->relname), -1)),
                                                    COERCE_EXPLICIT_CALL, 
                                                    -1);
                        }
                    ...
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Field", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Type", "varchar", 64));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Collation", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Null", "varchar", 8));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Key", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Default", "text", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Extra", "text", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Privileges", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Comment", "varchar", 512));
                    funcAliasClause = list_make2(NULL, tableFuncElementList);
                    rangeFunction->alias = linitial(funcAliasClause);
                    rangeFunction->coldeflist = lsecond(funcAliasClause);
					selectStmt->fromClause = list_make1((Node *)rangeFunction);
                    ...
                    if ($6 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                            "~~", 
                                                                            mys_makeColumnRef("Field", NIL, -1, yyscanner),
                                                                            mys_makeStringConst($6, -1),
                                                                            -1);
                    }
                    else if ($7 != NULL)
                    {
                        rectifyColumnsName($7);
                        selectStmt->whereClause = $7;
                    }
			...
			| SHOW CREATE TABLE qualified_name
				{
                    ...
                    /* SHOW CREATE TABLE `test_db`.`ttt` */
                    /* SELECT '%s' as Table, mysql.show_create_table('%s', '%s') as `Create Table` */
					SelectStmt *selectStmt;
					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("Table", $4->relname));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithFunc2Args("Create Table", "mysql", "show_create_table", $4->schemaname, $4->relname));
					$$ = (Node *)selectStmt;
				}
			| SHOW CREATE VIEW qualified_name
				{
                    /* SHOW CREATE VIEW `ttt` */
                    /* SHOW CREATE VIEW `test_db`.`ttt` */
                    /* select '%s' as `View`, mysql.show_create_view('%s', '%s') as `Create View`, 
                    'utf8mb4' as `character_set_client`, 'utf8mb4_general_ci' as `collation_connection` */
					SelectStmt *selectStmt;
					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("View", $4->relname));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithFunc2Args("Create View", "mysql", "show_create_view", $4->schemaname, $4->relname));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("character_set_client", "utf8mb4"));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("collation_connection", "utf8mb4_general_ci"));
					$$ = (Node *)selectStmt;
				}
			| SHOW show_index_opt show_from_table_clause show_from_db_clause where_clause
				{
                    /* SHOW {INDEX | INDEXES | KEYS} {FROM | IN} tbl_name [{FROM | IN} db_name] [WHERE expr] */
                    /* SELECT * from mysql.show_table_indexs('%s', '%s') as */
                    /* (`Table` varchar(256), `Non_unique` int, `Key_name` varchar(256), */
                    /* `Seq_in_index` int, `Column_name` varchar(256), `Collation` varchar(256), `Cardinality` int, `Sub_part` int, */
                    /* `Packed` varchar(256), `Null` varchar(8) , `Index_type` varchar(256), `Comment` varchar(512), */
                    /* `Index_comment` varchar(512))",  */
					SelectStmt *selectStmt;
                    RangeFunction *rangeFunction;
                    FuncCall *funcCall;
                    List *funcAliasClause;
                    ...
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_table_indexs"))),
                                                list_make2(mys_makeStringConst(pstrdup($4->relname), -1), 
                                                           mys_makeStringConst(pstrdup($3->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    ...
                    rangeFunction->functions = list_make1(list_make2((Node *)funcCall, NIL));
                    tableFuncElementList = NIL;
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Table", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Non_unique", "int4", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Key_name", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Seq_in_index", "int4", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Column_name", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Collation", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Cardinality", "int4", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Sub_part", "int4", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Packed", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Null", "varchar", 8));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Index_type", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Comment", "varchar", 512));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Index_comment", "varchar", 512));
                    funcAliasClause = list_make2(NULL, tableFuncElementList);
                    rangeFunction->alias = linitial(funcAliasClause);
                    rangeFunction->coldeflist = lsecond(funcAliasClause);
					selectStmt->fromClause = list_make1((Node *)rangeFunction);

                    rectifyColumnsName($5);
                    selectStmt->whereClause = $5;

					$$ = (Node *)selectStmt;
				}
            | SHOW CREATE FUNCTION qualified_name
                {
                    /* SHOW CREATE FUNCTION func_name */
                    /*
                     * "SELECT * FROM mysql.show_create_%s('%s', '%s') \
                     * as (
                     * `%s` varchar(256), sql_mode varchar(256), `Create %s` text, 
                     * Character_set_client varchar(128), Collation_connection varchar(128), 
                     * `Database Collation` varchar(128))",
                     */
					SelectStmt *selectStmt;
                    RangeFunction *rangeFunction;
                    FuncCall *funcCall;
                    ...
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_create_function"))),
                                                list_make2(mys_makeStringConst(pstrdup($4->schemaname), -1), 
                                                           mys_makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    ...
                    else 
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_create_function"))),
                                                list_make2(mys_makeStringConst(get_namespace_name(getCurrentNamespaceOid()), -1), 
                                                           mys_makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    }
                    rangeFunction->functions = list_make1(list_make2((Node *)funcCall, NIL));
                    tableFuncElementList = NIL;
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Function", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("sql_mode", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Create Function", "text", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("character_set_client", "varchar", 128));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("collation_connection", "varchar", 128));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Database Collation", "varchar", 128));
                    funcAliasClause = list_make2(NULL, tableFuncElementList);
                    rangeFunction->alias = linitial(funcAliasClause);
                    rangeFunction->coldeflist = lsecond(funcAliasClause);
					selectStmt->fromClause = list_make1((Node *)rangeFunction);
            ...
            | SHOW FUNCTION STATUS show_like_clause where_clause
                {
                    /* SHOW FUNCTION STATUS [LIKE 'pattern' | WHERE expr] */
                    /*
                     * "SELECT Db, Name, Type, Definer, Modified, Created, \
                     * Security_type, Comment, Character_set_client, \
                     * Collation_connection, Database_Collation as `Database Collation` \
                     * FROM mys_informa_schema.%s %s", 
                     */
					SelectStmt *selectStmt;
					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "db", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "name", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "type", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "definer", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "modified", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "created", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "security_type", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "comment", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "character_set_client", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "collation_connection", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("Database Collation", "database_collation", yyscanner));
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "functions"));
                    if ($4 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                            "~~", 
                                                                            mys_makeColumnRef("name", NIL, -1, yyscanner),
                                                                            mys_makeStringConst($4, -1),
                                                                            -1);
                    }
                    else if ($5 != NULL)
            ...
            | SHOW CREATE PROCEDURE qualified_name
                {
                    /* SHOW CREATE FUNCTION func_name */
                    /*
                     * "SELECT * FROM mysql.show_create_%s('%s', '%s') \
                    ...
                    {
                        funcCall = makeFuncCall(list_make2(makeString(pstrdup("mysql")), 
                                                           makeString(pstrdup("show_create_procedure"))),
                                                list_make2(mys_makeStringConst(pstrdup($4->schemaname), -1), 
                                                           mys_makeStringConst(pstrdup($4->relname), -1)),
                                                COERCE_EXPLICIT_CALL, 
                                                -1);
                    ...
                    rangeFunction->functions = list_make1(list_make2((Node *)funcCall, NIL));
                    tableFuncElementList = NIL;
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Procedure", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("sql_mode", "varchar", 256));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Create Procedure", "text", 0));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("character_set_client", "varchar", 128));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("collation_connection", "varchar", 128));
                    tableFuncElementList = lappend(tableFuncElementList, (Node *)createColumnDef("Database Collation", "varchar", 128));
                    funcAliasClause = list_make2(NULL, tableFuncElementList);
            ...
            | SHOW PROCEDURE STATUS show_like_clause where_clause
                {
                    ...
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "collation_connection", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("Database Collation", "database_collation", yyscanner));
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "procedures"));
                    if ($4 != NULL)
                    {
                        selectStmt->whereClause = (Node *) makeSimpleA_Expr(AEXPR_OP, 
                                                                            "~~", 
            ...
            | SHOW ShowFullProcesslistOpt PROCESSLIST
                {
                    /* SHOW [FULL] PROCESSLIST */
                    /* "select NULL::int as Id, ' '::varchar(256) as User, ' '::varchar(256) as Host, 
                     * ' '::varchar(256) as db, ' '::varchar(256) as Command, ' '::bigint as Time, 
                     * ' '::varchar(256) as State, ' '::varchar(256) as Info"); 
                     */
					SelectStmt *selectStmt;
					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithInt("Id", 1));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("User", " "));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("Host", " "));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("db", " "));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("Command", " "));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithInt("Time", 0));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("State", " "));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithString("Info", " "));
					$$ = (Node *)selectStmt;
                }
			| SHOW TRIGGERS show_from_db_clause show_like_clause where_clause
				{
					/* SHOW TRIGGERS [{FROM | IN} tbl_name] [LIKE 'pattern' | WHERE expr] */
                    /*
                     * "SELECT trigger_name as `trigger`, event_manipulation as event, event_object_table as `table`, \
					 * action_statement as `statement`, action_timing as timing, created, sql_mode, definer\
                     * character_set_client, collation_connection, database_collation as `database collation` \
                     * FROM mys_informa_schema.triggers %s", 
                     */
					SelectStmt *selectStmt;
					selectStmt = makeNode(SelectStmt);
                    selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("trigger", "trigger_name", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("event", "event_manipulation", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("table", "event_object_table", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("statement", "action_statement", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("timing", "action_timing", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "created", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "sql_mode", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "definer", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "character_set_client", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn(NULL, "collation_connection", yyscanner));
                    selectStmt->targetList = lappend(selectStmt->targetList, createResTargetWithColumn("database collation", "database_collation", yyscanner));
                    selectStmt->fromClause = list_make1(createRangeVar("mys_informa_schema", "triggers"));
                    if ($3 != NULL)
                    {
                        selectStmt->whereClause = 
                                (Node *)makeSimpleA_Expr(AEXPR_OP, 
                                                         "=", 
                                                         (Node *)mys_makeColumnRef("trigger_schema", NIL, -1, yyscanner), 
                                                         (Node *)mys_makeStringConst($3->relname, -1), 
                                                         -1); 
                    }
                    else
					...
					if ($4 != NULL)
                    {
						Node *cond4 = (Node *) makeSimpleA_Expr(AEXPR_OP, 
																"~~", 
																mys_makeColumnRef("event_object_table", NIL, -1, yyscanner),
																mys_makeStringConst($4, -1),
																-1);
						selectStmt->whereClause = makeAndExpr(selectStmt->whereClause, cond4, -1);
                    }
                    else if ($5 != NULL)
                    {
                        selectStmt->whereClause = makeAndExpr(selectStmt->whereClause, $5, -1);
                    }
			...
			| SHOW CREATE TRIGGER qualified_name
				{
					/* SHOW CREATE TRIGGER qualified_name */
                    /*
                     * select * from mysql.show_create_trigegr(schema, trigger_name);
                     */
					SelectStmt *selectStmt = makeNode(SelectStmt);
					FuncCall *funcCall = NULL;
					ResTarget *target = makeNode(ResTarget);
					ColumnRef *n1 = makeNode(ColumnRef);
					RangeFunction *n2 = makeNode(RangeFunction);

					n1->fields = list_make1(makeNode(A_Star));
					n1->location = -1;
					target->name = NULL;
					target->indirection = NIL;
					target->val = (Node *)n1;
					target->location = -1;
					selectStmt->targetList = NIL;
                    selectStmt->targetList = lappend(selectStmt->targetList, target);

					if ($4->schemaname == NULL)
					{
						$4->schemaname = get_namespace_name(getCurrentNamespaceOid());
					}

					funcCall = makeFuncCall(list_make2(makeString("mysql"), makeString("show_create_trigger")),
											list_make2(mys_makeStringConst($4->schemaname, -1), mys_makeStringConst($4->relname, -1)),
											COERCE_EXPLICIT_CALL,
											-1);

					n2->lateral = false;
					n2->ordinality = false;
					n2->is_rowsfrom = false;
					n2->functions = list_make1(list_make2(funcCall, NIL));
					n2->alias = NULL;
					n2->coldeflist = NIL;
					
					selectStmt->fromClause = list_make1(n2);

					$$ = (Node *)selectStmt;
				}
        ;

desc_describe_explain:
            DESC
            | DESCRIBE
//            | EXPLAIN     // 语法冲突
        ;

col_name_wild_for_desc:
            ColId                                   { $$ = $1; }
            | Sconst                                { $$ = $1; }
			| /*EMPTY*/								{ $$ = NULL; }
        ;
...
show_from_table_clause:
			FROM qualified_name					    { $$ = $2; }
			| IN_P qualified_name					{ $$ = $2; }
		;

show_from_db_clause:
            FROM ColId
				{
					if (memcmp($2, "information_schema", 18) == 0)
					{
						$$ = makeRangeVar(NULL, pstrdup("mys_informa_schema"), @2);
					}
					else if ((memcmp($2, "sys", 3) == 0) && (strlen($2) == 3))
					{
						$$ = makeRangeVar(NULL, pstrdup("mys_sys"), @2);
					}
					else
					{
						$$ = makeRangeVar(NULL, $2, @2);
					}
				}
			| IN_P ColId
				{
...
show_session_global_opt:
             /* empty */
            | SESSION 
            | GLOBAL
        ;

show_columns_fields_opt:
            COLUMNS
            | FIELDS
        ;
...
AlterTableStmt:
        ...
        |   ALTER TABLE relation_expr character_set_collates
				{
					AlterTableStmt *n = makeNode(AlterTableStmt);
					n->relation = $3;
					n->cmds = NULL;
        ...
        |   ALTER TABLE relation_expr CONVERT TO character_set_collates
				{
					AlterTableStmt *n = makeNode(AlterTableStmt);
					n->relation = $3;
					n->cmds = NULL;
        ...
        |   ALTER TABLE relation_expr ALGORITHM algorithm_value
            {
					AlterTableStmt *n = makeNode(AlterTableStmt);
					n->relation = $3;
					n->cmds = NULL;
...
alter_table_cmds:
            ...
            | alter_table_cmds ',' alter_table_options
                {
                    AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_TableOption;
					n->def = (Node *)$3;
...
alter_table_cmd:
			...
			| ADD_P IF_P NOT EXISTS columnDef after_col
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_AddColumn;
					n->def = $5;
			...
			| ADD_P COLUMN columnDef after_col
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_AddColumn;
					n->def = $3;
			...
			| ADD_P COLUMN IF_P NOT EXISTS columnDef after_col
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
					n->subtype = AT_AddColumn;
					n->def = $6;
            ...
            | MODIFY opt_column columnDef after_col
                {
                    AlterTableCmd *n = makeNode(AlterTableCmd);
                    ColumnDef *def = (ColumnDef *) $3;

                    n->subtype = AT_ModifyColumn;
                    n->name = pstrdup(def->colname);
                    n->def = $3;

                    $$ = (Node *)n;
                }
            | CHANGE opt_column ColId columnDef
                {
                    AlterTableCmd *n = makeNode(AlterTableCmd);

                    n->subtype = AT_ChangeColumn;
                    n->name = $3;
                    n->def = $4;
            ...
            | DROP PRIMARY KEY
                {
                    AlterTableCmd *n = makeNode(AlterTableCmd);
                    n->subtype = AT_DropPrimaryKey;
                    n->name = NULL;
                    n->behavior = DROP_RESTRICT;
                    n->missing_ok = false;
                    $$ = (Node *)n;
                }
            | DROP key_or_index name
                {
                    AlterTableCmd *n = makeNode(AlterTableCmd);
...
CreateStmt:	CREATE OptTemp TABLE qualified_name '(' OptTableElementList ')'
			    opt_create_table_options OptPartitionSpec
				{
            ...
            | CREATE OptTemp TABLE IF_P NOT EXISTS qualified_name TableLikeClause
                {
                    CreateStmt *n = makeNode(CreateStmt);
                    $7->relpersistence = $2;
                    n->relation = $7;
            ...
            | CREATE OptTemp TABLE USER '(' OptTableElementList ')'
                opt_create_table_options OptPartitionSpec
                {
                    CreateStmt *n = makeNode(CreateStmt);
                    RangeVar* rv = makeRangeVar(NULL, pstrdup("user"), @1);
                    rv->relpersistence = $2;
                    n->relation = rv;
                    n->tableElts = $6;
                    n->inhRelations = NIL;
            ...
            | CREATE OptTemp TABLE IF_P NOT EXISTS USER '(' OptTableElementList ')'
                opt_create_table_options OptPartitionSpec
                {
                    CreateStmt *n = makeNode(CreateStmt);
                    RangeVar* rv = makeRangeVar(NULL, pstrdup("user"), @1);
            ...
            | CREATE OptTemp TABLE USER TableLikeClause
                {
                    CreateStmt *n = makeNode(CreateStmt);
                    RangeVar* rv = makeRangeVar(NULL, pstrdup("user"), @1);
                    rv->relpersistence = $2;
            ...
            | CREATE OptTemp TABLE IF_P NOT EXISTS USER TableLikeClause
                {
                    CreateStmt *n = makeNode(CreateStmt);
                    RangeVar* rv = makeRangeVar(NULL, pstrdup("user"), @1);
                    rv->relpersistence = $2;
...
alter_table_option:
                AUTO_INCREMENT opt_equal NumericOnly
                    {
                        $$ = makeDefElem("auto_increment", (Node *)$3, @1);
                    }
                | COMMENT opt_equal Sconst
                    {
                        $$ = makeDefElem("comment", (Node *)makeString($3), @1);
                    }
                | AVG_ROW_LENGTH opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                | CHECKSUM opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                | COMPRESSION opt_equal Sconst
                    {
                        $$ = NULL;
                    }
                ...
                | DELAY_KEY_WRITE opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                ...
                | KEY_BLOCK_SIZE opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                | MAX_ROWS opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                | MIN_ROWS opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                ...
                | PACK_KEYS opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                ...
                | STATS_AUTO_RECALC opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                ...
                | STATS_PERSISTENT opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
                | STATS_SAMPLE_PAGES opt_equal I_or_F_const
                    {
                        $$ = NULL;
                    }
...
opt_create_table_options:
                ...
                | opt_create_table_options opt_comma create_table_option
                    {
                        if ($3 != NULL)
                        {
                            $$ = lappend($1, $3);
...
OptTableElementList:
			TableElementList
                {
                    /* MySQL 5.7.32 InnoDB，最多只支持1017个字段（tinyint型）， */
                    /* 且字段类型如果是varchar(8)，最多只支持300多个。报错信息如下： */
                    /* Row size too large (> 8126). Changing some columns to TEXT or BLOB may help. */
                    /* In current row format, BLOB prefix of 0 bytes is stored inline */
                    /* TODO: 进一步根据各字段占空间长度总和及系统相关参数设置来做判断。 */
                    if (1017 < list_length($1))
                    {
                        int columnDefCount;
                        ListCell *lc;
                        columnDefCount = 0;
                        ...
                        if (1017 < columnDefCount)
                        {
                            ereport(ERROR,
                                    (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                                     errmsg("Too many columns"),
                                     parser_errposition(@1)));
                        }
                    }
                    $$ = $1;
        ...
        | USER Typename optColumnCompression create_generic_options ColQualList opt_references
				{
                    ...
                    if (list_length($2->names) == 1)
                    {
                        char *typeName = strVal(linitial($2->names));
                        if ((strncasecmp(typeName, "char", 4) == 0) || 
                            (strncasecmp(typeName, "bpchar", 6) == 0) || 
                            (strncasecmp(typeName, "varchar", 7) == 0) || 
                            (strncasecmp(typeName, "tinytext", 8) == 0) || 
                    ...
                    {
                        char *typeName = strVal(lsecond($2->names));
                        if ((strncasecmp(typeName, "char", 4) == 0) || 
                            (strncasecmp(typeName, "bpchar", 6) == 0) || 
                            (strncasecmp(typeName, "varchar", 7) == 0) || 
                            (strncasecmp(typeName, "tinytext", 8) == 0) || 
			...
			ColQualList ColConstraint
                {
                    if ($2 != NULL)
                    {
                        if (IsA($2, Constraint))
                        {
                            Constraint *n = castNode(Constraint, $2);
                            if (n->contype == CONSTR_DEFAULT)
                            {
                                ListCell   *lc;
                                foreach (lc, $1)
                                {
                                    Constraint *cons = (Constraint *) lfirst(lc);
                                    if (cons->contype == CONSTR_DEFAULT)
                                    {
                                        $1 = list_delete_cell($1, lc);
                                        break;
                                    }
                                }
                            }
            ...
            | ON UPDATE now
                {
                    ...
                    len = pg_sprintf(str, "on update current_timestamp(%d)", $3);
                    str[len] = '\0';
                    v = (Node *)makeString(str);
...
ColConstraintElem:
			...
			| GENERATED generated_when AS '(' a_expr ')' stored_virtual
				{
					Constraint *n = makeNode(Constraint);
			...
			LIKE qualified_name TableLikeOptionList
				{
					...
					n->relation = $2;
					//n->options = $3;
					//n->options = CREATE_TABLE_LIKE_ALL;
					n->options = CREATE_TABLE_LIKE_ALL & ~CREATE_TABLE_LIKE_DEFAULTS;
...
ConstraintElem:
			...
			| UNIQUE opt_key_or_index opt_index_name access_method_clause '(' index_params ')' index_options
				{
					Constraint *n = makeNode(Constraint);
					n->contype = CONSTR_UNIQUE;
					n->location = @1;
			...
			| PRIMARY KEY opt_index_name access_method_clause '(' index_params ')' index_options
				{
					Constraint *n = makeNode(Constraint);
					n->contype = CONSTR_PRIMARY;
					n->location = @1;
            ...
            | fs_key_or_index opt_index_name access_method_clause '(' index_params ')' index_options
                {
                    Constraint *n = makeNode(Constraint);
					n->contype = CONSTR_KEY;
					n->location = @1;
                    n->conname = $2;
                    // n->access_method = $3 != NULL ? $3 : $7;
			...
			| FOREIGN KEY opt_index_name '(' columnList ')' REFERENCES qualified_name
				opt_column_list key_match key_actions ConstraintAttributeSpec
				{
                    // Mysql创建外键时，会内部创建相关索引，此处的名字是索引名，约束名不是该名字
					Constraint *n = makeNode(Constraint);
					n->contype = CONSTR_FOREIGN;
					n->location = @1;
					n->conname = NULL;
...
index_option:
            KEY_BLOCK_SIZE '=' Iconst    { $$ = NULL; }
            | KEY_BLOCK_SIZE Iconst      { $$ = NULL; }
            | USING name
            {
                if ((strncasecmp($2, "btree", 5) == 0) ||
                    (strncasecmp($2, "hash", 4) == 0))
                {
                    $$ = (Node*)makeString($2);
                }
                else
                {
                    $$ = (Node*)makeString(DEFAULT_INDEX_TYPE);
                }
            }
            | COMMENT Sconst
            {
            ...
            a_expr //opt_collate //opt_class
				{
                    ...
                    if (IsA($1, ColumnRef))
                    {
                        ColumnRef *columnRef = castNode(ColumnRef, $1);
                        if (list_length(columnRef->fields) == 1)
                        {
                            n->name = pstrdup(strVal(linitial(columnRef->fields)));
                        }
                        n->expr = NULL;
                    }
                    else
			...
			ALTER STATISTICS any_name SET STATISTICS SignedIconst
				{
					...
					n->defnames = $3;
					n->missing_ok = false;
					/* n->stxstattarget = $6; -- PG18 changed to Node* */;
					$$ = (Node *)n;
				}
			| ALTER STATISTICS IF_P EXISTS any_name SET STATISTICS SignedIconst
				{
					...
					n->defnames = $5;
					n->missing_ok = true;
					/* n->stxstattarget = $8; -- PG18 changed to Node* */;
					$$ = (Node *)n;
				}
			;
...
CreateAsStmt:
		...
		| CREATE OptTemp TABLE qualified_name SelectStmt opt_with_data
				{
					CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
					...
					ctas->into->viewQuery = NULL;
					ctas->into->skipData = false;		/* might get changed later */
					ctas->into->rel->relpersistence = $2;
					ctas->into->skipData = !($6);
					ctas->query = $5;
					ctas->objtype = OBJECT_TABLE;
					ctas->is_select_into = false;
					ctas->if_not_exists = false;
		...
		| CREATE OptTemp TABLE qualified_name AS SelectStmt opt_with_data
				{
					CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
					ctas->into = makeNode(IntoClause);
					ctas->into->rel = $4;
		...
		//| CREATE OptTemp TABLE create_as_target SelectStmt opt_with_data
		//		{
		//			CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
		//			ctas->query = $5;
		//			ctas->into = $4;
		...
		| CREATE OptTemp TABLE IF_P NOT EXISTS qualified_name SelectStmt opt_with_data
				{
					CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
					ctas->into = makeNode(IntoClause);
					ctas->into->rel = $7;
		...
		| CREATE OptTemp TABLE IF_P NOT EXISTS qualified_name AS SelectStmt opt_with_data
				{
					CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
					ctas->into = makeNode(IntoClause);
					ctas->into->rel = $7;
		...
		//| CREATE OptTemp TABLE IF_P NOT EXISTS create_as_target SelectStmt opt_with_data
		//		{
		//			CreateTableAsStmt *ctas = makeNode(CreateTableAsStmt);
		//			ctas->query = $8;
		//			ctas->into = $7;
...
create_as_target:
            qualified_name '(' OptTableElementList ')'
                opt_create_table_options OptPartitionSpec
                {
					$$ = makeNode(IntoClause);
					$$->rel = $1;
...
SeqOptElem: AS SimpleTypename
			...
			| SEQUENCE NAME_P any_name
				{
					/* not documented, only used by ud_dump */
...
DropStmt:	DROP object_type_any_name IF_P EXISTS any_name_list opt_drop_behavior
            ...
            | DROP TEMPORARY object_type_any_name IF_P EXISTS any_name_list opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = $3;
					n->missing_ok = true;
			...
			| DROP object_type_any_name any_name_list opt_drop_behavior on_table index_algorithm_opt
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = $2;
					n->missing_ok = false;
                    ...
                    if ($5 != NULL)
                    {
                        ...
                        foreach(lc, $3)
                        {
                            ...
                            {
                                names = lcons(makeString(pstrdup($5->relname)), names);
                            }
                            else
                            {
                                names = list_insert_nth(names, 1, mys_makeStringConst(pstrdup($5->relname), -1));
                            }
			...
			| DROP TEMPORARY object_type_any_name any_name_list opt_drop_behavior on_table index_algorithm_opt
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = $3;
					n->missing_ok = false;
                    ...
                    if ($6 != NULL)
                    {
                        ...
                        foreach(lc, $4)
                        {
                            ...
                            if (list_length(names) == 1)
                            {
                                names = lcons(makeString(pstrdup($6->relname)), names);
                            }
                            else
                            {
                                names = list_insert_nth(names, 1, mys_makeStringConst(pstrdup($6->relname), -1));
                            }
			...
			| DROP TEMPORARY drop_type_name IF_P EXISTS name_list opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = $3;
					n->missing_ok = true;
			...
			| DROP TEMPORARY drop_type_name name_list opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = $3;
					n->missing_ok = false;
...
index_algorithm_opt:
              /* empty */
            | ALGORITHM opt_equal index_algorithm
        ;
...
any_name:	ColId						{ $$ = list_make1(makeString($1)); }
			| USER				        { $$ = list_make1(makeString(pstrdup("user"))); }
...
//MySQL TRUNCATE TABLE的默认行为等价于
//PG    TRUNCATE TABLE relation_expr_list RESTART IDENTITY_P RESTRICT
TruncateStmt:
...
/* 
 * opt_restart_seqs: Currently unused in MySQL grammar, but kept for 
 * potential future extension to support PostgreSQL-style TRUNCATE options
 */
opt_restart_seqs:
...
IndexStmt:
            CREATE opt_unique INDEX opt_concurrently opt_index_name index_access_method_clause
			ON relation_expr '(' index_params ')'
            index_opt index_algorithm_opt index_lock_opt index_visible_opt
				{
...
index_opt:
                  KEY_BLOCK_SIZE opt_equal ICONST   { $$ = NULL; }
                | index_access_method_clause        { $$ = $1; }
                | WITH PARSER ColId                 { $$ = NULL; }
                | COMMENT SCONST                    { $$ = NULL; }
        ;

index_lock_opt:
               /* empty */
            | LOCK_P opt_equal lock_opt
        ;
...
index_elem: ColId index_elem_options
			...
			| ColId '(' Iconst ')' index_elem_options
                {
                    $$ = $5;
					$$->name = $1;
                }
...
common_func_opt_item:
			...
			| SQL_P SECURITY DEFINER
				{
					$$ = makeDefElem("security", (Node *)makeInteger(true), @2);
				}
			| SQL_P SECURITY INVOKER
				{
					$$ = makeDefElem("security", (Node *)makeInteger(false), @2);
				}
			| LEAKPROOF
				{
...
RemoveFuncStmt:
			...
			| DROP PROCEDURE procedure_with_argtypes opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
			...
			| DROP PROCEDURE IF_P EXISTS procedure_with_argtypes opt_drop_behavior
				{
					DropStmt *n = makeNode(DropStmt);
					n->removeType = OBJECT_PROCEDURE;
					n->objects = list_make1($5);
...
CreateCastStmt: CREATE CAST '(' Typename AS Typename ')'
            ...
            //| CREATE CAST '(' BIT_AS_BYTEA ')'
			//		WITH FUNCTION function_with_argtypes cast_context
			//	{
            //        // 为了绕过语法问题（bit 被改写为varbit了），将“bit as bytea”写为“bit_as_bytea”
			//		CreateCastStmt *n = makeNode(CreateCastStmt);
			//		n->sourcetype = SystemTypeName("bit");
			//		n->targettype = SystemTypeName("bytea");
			//		n->func = $8;
			//		n->context = (CoercionContext) $9;
			//		n->inout = false;
			//		$$ = (Node *)n;
			//	}
...
RenameStmt: ALTER AGGREGATE aggregate_with_argtypes RENAME TO name
			...
			| ALTER TABLE relation_expr RENAME TO relation_expr
				{
					RenameStmt *n = makeNode(RenameStmt);
			...
			| ALTER TABLE relation_expr RENAME AS relation_expr
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_TABLE;
					n->relation = $3;
			...
			| ALTER TABLE IF_P EXISTS relation_expr RENAME TO relation_expr
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_TABLE;
					n->relation = $5;
            ...
            | ALTER TABLE IF_P EXISTS relation_expr RENAME AS relation_expr
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_TABLE;
					n->relation = $5;
            ...
            | ALTER TABLE relation_expr RENAME key_or_index name TO name
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_INDEX;
                    // 传递表的信息
			...
			| ALTER TABLE IF_P EXISTS relation_expr RENAME key_or_index name TO name
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_INDEX;
                    // 传递表的信息
            ...
            | RENAME TABLE relation_expr TO relation_expr
				{
                    /* MySQL中rename table语句中可以包含修改库， */
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_TABLE;
					n->relation = $3;
                    n->object = (Node *)$5;
...
transaction_mode_item:
			ISOLATION LEVEL iso_level
					{ $$ = makeDefElem("transaction_isolation",
									   mys_makeStringConst($3, @3), @1); }
			| READ ONLY
					{ $$ = makeDefElem("transaction_read_only",
									   mys_makeIntConst(true, @1), @1); }
			| READ WRITE
					{ $$ = makeDefElem("transaction_read_only",
...
ViewStmt: 
		CREATE opt_or_replace view_algorithm_opt opt_definer view_sql_security_opt VIEW qualified_name opt_column_list opt_reloptions
        ...
        | CREATE opt_or_replace opt_definer view_sql_security_opt VIEW qualified_name opt_column_list opt_reloptions
				AS SelectStmt opt_check_option
				{
					ViewStmt *n = makeNode(ViewStmt);
					n->view = $6;
...
/*
 *	Though the equals sign doesn't match other WITH options, ud_dump uses
...
PrepareStmt: PREPARE name prep_type_clause FROM PreparableStmt
				{
...
InsertStmt:
            opt_with_clause MysInsertStmt
            {
                ((InsertStmt *)$2)->withClause = $1;
...
MysInsertStmt:
			INSERT insert_low_priority_delayed INTO insert_target insert_rest opt_on_conflict
				{
					$5->relation = $4;
                    if ($6 != NULL)
                    {
                        ListCell *lc;
                        foreach (lc, $6->targetList)
                        {
                            Node *node = lfirst(lc);
                            if (node->type == T_ResTarget)
                            {
                                ResTarget *resTarget = (ResTarget *)node;
                                if (resTarget->val->type == T_A_Expr)
                                {
                                    char *tableName;
                                    RangeVar *rangeVar;
                                    Node *lexpr;
                                    ...
                                    lexpr = ((A_Expr *)(resTarget->val))->lexpr;
                                    rexpr = ((A_Expr *)(resTarget->val))->rexpr;
                                    if (lexpr->type == T_ColumnRef)
                                    {
                                        ColumnRef *columnRef = (ColumnRef *)lexpr;
                                        if (list_length(columnRef->fields) == 1)
                                        {
                                            ListCell *subLc;
                                            foreach (subLc, columnRef->fields)
                                            {
                                                Node *val = (Node *)lfirst(subLc);
                                                Node *colNode = (Node *)makeString(strVal(val));
                                                List *indi = list_make1(colNode);
                                                ((A_Expr *)(resTarget->val))->lexpr = (Node *)mys_makeColumnRef(tableName, indi, -1, 0);
                                                break;
                                            }
                                        }
                                    }
                                    if (rexpr->type == T_ColumnRef)
                                    {
                                        ColumnRef *columnRef = (ColumnRef *)rexpr;
                                        if (list_length(columnRef->fields) == 1)
                                        {
                                            ListCell *subLc;
                                            foreach (subLc, columnRef->fields)
                                            {
                                                ...
                                                Node *colNode = (Node *)makeString(strVal(val));
                                                List *indi = list_make1(colNode);
                                                ((A_Expr *)(resTarget->val))->rexpr = (Node *)mys_makeColumnRef(tableName, indi, -1, 0);
                                                break;
                                            }
                                        }
                                    }
					...
					$5->returningClause = NULL;
					$5->withClause = NULL;
					$$ = (Node *) $5;
				}
			| INSERT insert_low_priority_delayed INTO insert_target SET set_clause_list_for_mysql opt_on_conflict
				{
                    InsertStmt* insertRest;
                    List* insertColumnList;
                    SelectStmt* insertValueClause;
                    ListCell* lc;
                    List* values;
                    int i;
                    insertRest = makeNode(InsertStmt);
                    ...
                    foreach (lc, $6)
                    {
                        if ((i % 2) == 1)
                        {
                            ResTarget* newResTarget;
                            ResTarget* resTarget;
                            newResTarget = makeNode(ResTarget);
                            resTarget = (ResTarget *) lfirst(lc);
                            newResTarget->name = pstrdup(resTarget->name);
                            newResTarget->indirection = check_indirection(NIL, yyscanner);
                            newResTarget->val = NULL;
                            newResTarget->location = -1;
                            insertColumnList = lappend(insertColumnList, newResTarget);
                        }
                        ++i;
                    }
                    insertValueClause = makeNode(SelectStmt);
                    insertValueClause->valuesLists = NIL;
                    values = NIL;
                    i = 0;
                    foreach(lc, $6)
                    {
                        if ((i % 2) == 1)
                        {
                            ...
                            values = lappend(values, copyObject(resTarget->val));
                        }
                        ++i;
                    }
                    insertValueClause->valuesLists = list_make1(values);

                    insertRest->cols = insertColumnList;
                    insertRest->selectStmt = (Node*)insertValueClause;
					insertRest->relation = $4;
                    if ($7 != NULL)
                    {
                        ListCell *lc;
                        foreach (lc, $7->targetList)
                        {
                            Node *node = lfirst(lc);
                            if (node->type == T_ResTarget)
                            {
					...
					insertRest->onConflictClause = $7;
					insertRest->returningClause = NULL;
					insertRest->withClause = NULL;
					$$ = (Node *) insertRest;
				}
        |   INSERT insert_low_priority_delayed insert_target insert_rest opt_on_conflict
				{
					$4->relation = $3;
                    if ($5 != NULL)
                    {
                        ListCell *lc;
                        foreach (lc, $5->targetList)
                        {
                            Node *node = lfirst(lc);
                            if (node->type == T_ResTarget)
                            {
					...
					$4->returningClause = NULL;
					$4->withClause = NULL;
					$$ = (Node *) $4;
				}
        |   INSERT insert_low_priority_delayed insert_target SET set_clause_list_for_mysql opt_on_conflict
				{
                    InsertStmt* insertRest;
                    List* insertColumnList;
                    SelectStmt* insertValueClause;
					...
					insertRest->onConflictClause = $6;
					insertRest->returningClause = NULL;
					insertRest->withClause = NULL;
					$$ = (Node *) insertRest;
				}
        |   REPLACE insert_low_priority_delayed INTO insert_target insert_rest
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    onconflict->action = ONCONFLICT_REPLACE;
                    onconflict->infer = NULL;
                    onconflict->targetList = NIL;
                    onconflict->whereClause = NULL;
                    onconflict->location = @1;

                    $5->relation = $4;
                    $5->onConflictClause = onconflict;
                    $5->returningClause = NULL;
                    $5->withClause = NULL;
                    $$ = (Node *) $5;
                }
        |   REPLACE insert_low_priority_delayed INTO insert_target SET set_clause_list_for_mysql
				{
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    InsertStmt* insertRest;
                    List* insertColumnList;
                    ...
                    insertRest->selectStmt = (Node*)insertValueClause;
					insertRest->relation = $4;
					insertRest->onConflictClause = onconflict;
					insertRest->returningClause = NULL;
					insertRest->withClause = NULL;
					$$ = (Node *) insertRest;
				}
        |   REPLACE insert_low_priority_delayed insert_target insert_rest
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    onconflict->action = ONCONFLICT_REPLACE;
                    onconflict->infer = NULL;
                    ...
                    $4->relation = $3;
                    $4->onConflictClause = onconflict;
                    $4->returningClause = NULL;
                    $4->withClause = NULL;
                    $$ = (Node *) $4;
                }
        |   REPLACE insert_low_priority_delayed insert_target SET set_clause_list_for_mysql
				{
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    InsertStmt* insertRest;
                    List* insertColumnList;
	...
	|   INSERT insert_low_priority_delayed IGNORE INTO insert_target insert_rest
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    ...
                    $6->relation = $5;
                    $6->onConflictClause = onconflict;
                    $6->returningClause = NULL;
                    $6->withClause = NULL;
                    $$ = (Node *) $6;
                }
        |   INSERT insert_low_priority_delayed IGNORE insert_target insert_rest
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    onconflict->action = ONCONFLICT_NOTHING;
                    onconflict->infer = NULL;
        ...
        |   INSERT insert_low_priority_delayed IGNORE INTO insert_target SET set_clause_list_for_mysql
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    InsertStmt* insertRest;
                    List* insertColumnList;
                    ...
                    insertRest->cols = insertColumnList;
                    insertRest->selectStmt = (Node*)insertValueClause;
					insertRest->relation = $5;
					insertRest->onConflictClause = onconflict;
					insertRest->returningClause = NULL;
					insertRest->withClause = NULL;
					$$ = (Node *) insertRest;
                }
        |   INSERT insert_low_priority_delayed IGNORE insert_target SET set_clause_list_for_mysql
                {
                    OnConflictClause *onconflict = makeNode(OnConflictClause);
                    InsertStmt* insertRest;
                    List* insertColumnList;
...
insert_target:
			...
			| USER
				{
					$$ = makeRangeVar(NULL, pstrdup("user"), @1);
				}
...
insert_rest:
			...
			| '(' insert_column_list ')' insert_value_clause
				{
					$$ = makeNode(InsertStmt);
					$$->cols = $2;
					$$->selectStmt = $4;
...
insert_column_item:
			...
			| IDENT '.' ColId opt_indirection
				{
					$$ = makeNode(ResTarget);
					$$->name = $3;
					$$->indirection = check_indirection($4, yyscanner);
					$$->val = NULL;
					$$->location = @1;
				}
			| IDENT '.' USER opt_indirection
				{
					$$ = makeNode(ResTarget);
					$$->name = pstrdup("user");
					$$->indirection = check_indirection($4, yyscanner);
...
DeleteStmt: with_clause DELETE_P delete_options FROM relation_expr_opt_alias using_clause
                where_or_current_clause opt_sort_clause opt_select_limit returning_clause
				{
            ...
            | DELETE_P delete_options FROM relation_expr_opt_alias using_clause
                where_or_current_clause opt_sort_clause opt_select_limit returning_clause
				{
					DeleteStmt *n = makeNode(DeleteStmt);
					n->relation = $4;
...
delete_options:
            ...
            | ColId                     { isIgnoreStmt = false; }
            | IGNORE                    { isIgnoreStmt = true; }
            | LOW_PRIORITY              { isIgnoreStmt = false; }
            | ColId IGNORE              { isIgnoreStmt = true; }
            | LOW_PRIORITY IGNORE       { isIgnoreStmt = true; }
            | IGNORE ColId              { isIgnoreStmt = true; }
            | IGNORE LOW_PRIORITY       { isIgnoreStmt = true; }
            | LOW_PRIORITY ColId        { isIgnoreStmt = false; }
            | ColId ColId               { isIgnoreStmt = false; }
            | LOW_PRIORITY IGNORE ColId { isIgnoreStmt = true; }
            | IGNORE LOW_PRIORITY ColId { isIgnoreStmt = true; }
            | IGNORE ColId ColId        { isIgnoreStmt = true; }
            | ColId IGNORE ColId        { isIgnoreStmt = true; }
        ;
...
//				}
UpdateStmt:
            opt_with_clause MysUpdateStmt
                {
                    ((UpdateStmt *)$2)->withClause = $1;
...
MysUpdateStmt:
            UPDATE update_options relation_expr_opt_alias_list
			...
			returning_clause
				{
                    // MySQL支持这样的多表update
                    // update ab1 as t1, ab2 as t2 set t1.nm = 'ab1_nm', t2.nm = 'ab2_nm' where t1.id = 2 and t1.id = t2.id;
					UpdateStmt *n = makeNode(UpdateStmt);
                    ...
                    if (list_length($3) < 2)
                    {
                        int i;
                        ListCell *lc;
                        n->relation = (RangeVar*)linitial($3);
                        i = 0;
                        foreach(lc, $5)
                        {
                            Node *node = lfirst(lc);
                    ...
                    else
                    {
                        ...
                        foreach (lc, $5)
                        {
                            ...
                            if ((i % 2) == 0)
                            {
                                if (node != NULL)
                                {
                                    ...
                                    else
                                    {
                                        if (strncasecmp(targetRelationName, rel->val.sval.sval, len) != 0)
                                        {
                                            ereport(ERROR,
                                                    (errcode(ERRCODE_SYNTAX_ERROR),
                                                     errmsg("syntax error, multi table name or alias in set.")));
                                        }
                                    }
                                }
                                else
                                {
                                    ereport(ERROR,
                                            (errcode(ERRCODE_SYNTAX_ERROR),
                                             errmsg("syntax error, talbe name or alias is missing in set.")));
                                }
                            }
                            else
                            {
                        ...
                        foreach (lc, $3)
                        {
                            ...
                            if (rangeVar->alias != NULL)
                            {
                                ...
                                {
                                    n->fromClause = lappend(n->fromClause, rangeVar);
                                }
                            }
                            else
                            {
                                if ((strncasecmp(targetRelationName, rangeVar->relname, len)) == 0)
                                {
                                    n->relation = rangeVar;
                                }
                                else
                        ...
                        if (n->relation == NULL)
                        {
                            ereport(ERROR,
                                    (errcode(ERRCODE_SYNTAX_ERROR),
                                     errmsg("syntax error after UPDATE")));
                        }
...
set_clause:
            ...
            | set_target '=' VALUES '(' a_expr ')'
				{
                    /* from "name=values(name)"  */
                    /* to:  "name=excluded.name" */
                    if (IsA($5, ColumnRef))
                    {
                        ColumnRef *columnRef = (ColumnRef *) $5;
                        if (list_length(columnRef->fields) == 1)
                        {
                            char *columnName;
                            ListCell *subLc;
                            ...
                            foreach (subLc, columnRef->fields)
                            {
                                columnName = strVal(lfirst(subLc));
                            }
                            if (columnName != NULL)
                            {
                                Node *newColumnRef;
                                List *indi;
                                Node *colNode;
                                colNode = (Node *)makeString(columnName);
                                indi = list_make1(colNode);
                                newColumnRef = mys_makeColumnRef("excluded", indi, -1, 0);
                                $1->val = newColumnRef;
                                $$ = list_make1($1);
                            }
                            else
                            {
                                ereport(ERROR,
                                        (errcode(ERRCODE_SYNTAX_ERROR),
                                         errmsg("syntax is not supported in duplicate"),
                                         errhint("syntax is not supported in duplicate"),
                                         parser_errposition(@1)));
                            }
                        }
                        else
...
relation_expr_opt_alias_list:
			relation_expr_opt_aliass							            { $$ = $1; }
			| relation_expr_opt_alias_list ',' relation_expr_opt_aliass	    { $$ = list_concat($1, $3); }
		;

set_clause_list_for_mysql:
			set_clause_for_mysql
            {
                $$ = $1;
            }
			| set_clause_list_for_mysql ',' set_clause_for_mysql
            {
                $$ = list_concat($1, $3);
            }
		;
...
set_clause_for_mysql:
			ColId '=' a_expr
            {
                ...
                resTarget->name = $1;
                resTarget->indirection = check_indirection(NIL, yyscanner);
                resTarget->val = $3;	/* upper production sets this */
                resTarget->location = @1;
                $$ = list_make2(NULL, (Node*)resTarget);
            }
			| ColId '.' ColId '=' a_expr
				{
					ResTarget* resTarget = makeNode(ResTarget);
					resTarget->name = $3;
					resTarget->indirection = check_indirection(NIL, yyscanner);
					resTarget->val = $5;	/* upper production sets this */
					resTarget->location = @3;
					$$ = list_make2(mys_makeStringConst($1, -1), (Node*)resTarget);
				}
			| USER '=' a_expr
            {
                ResTarget* resTarget = makeNode(ResTarget);
                resTarget->name = pstrdup("user");
                resTarget->indirection = check_indirection(NIL, yyscanner);
                resTarget->val = $3;	/* upper production sets this */
                resTarget->location = @1;
                $$ = list_make2(NULL, (Node*)resTarget);
			...
			group_clause having_clause window_clause
				{
                    ...
                    foreach (lc, $6)
                    {
                        ...
                        {
                            JoinExpr *joinExpr = castNode(JoinExpr, node);
                            if ((IsA(joinExpr->larg, JoinExpr)) || (IsA(joinExpr->rarg, JoinExpr)))
                            {
                                bool hasNotOn = false;
                                checkJoinOn(joinExpr, &hasNotOn);
                                if (hasNotOn)
                                {
                                    Node *expr;
                                    expr = NULL;
                                    makeOnClause(&expr, joinExpr);
                                    joinExpr->quals = expr;
                                }
                            }
                        }
...
common_table_expr:  name opt_name_list AS opt_materialized '(' select_no_parens ')' opt_search_clause opt_cycle_clause
			{
		...
		| CYCLE columnList SET ColId USING ColId
			{
				...
				n->cycle_col_list = $2;
				n->cycle_mark_column = $4;
				n->cycle_mark_value = (Node *) mys_makeBoolAConst(true, -1);
				n->cycle_mark_default = (Node *) mys_makeBoolAConst(false, -1);
...
select_var:
            ...
            | IDENT
                {
                    $$ = mys_makeColumnRef($1, NIL, @1, yyscanner);
                }
        ;
...
distinct_optimizer_hints_clause:
			/*EMPTY*/                               { $$ = NIL; }
            | distinct_clause1                      { $$ = $1; }
            | opt_cache_clause                      { $$ = NIL; }
            | opt_optimizer_hints                   { $$ = NIL; }
            | distinct_clause1 opt_cache_clause     { $$ = $1; }
            | distinct_clause1 opt_optimizer_hints  { $$ = $1; }
            | opt_cache_clause distinct_clause1     { $$ = $2; }
            | opt_cache_clause opt_optimizer_hints  { $$ = NIL; }
            | opt_optimizer_hints distinct_clause1  { $$ = $2; }
            | opt_optimizer_hints opt_cache_clause  { $$ = NIL; }
            | distinct_clause1 opt_cache_clause opt_optimizer_hints     { $$ = $1; }
            | distinct_clause1 opt_optimizer_hints opt_cache_clause     { $$ = $1; }
            | opt_cache_clause distinct_clause1 opt_optimizer_hints     { $$ = $2; }
            | opt_cache_clause opt_optimizer_hints distinct_clause1     { $$ = $3; }
            | opt_optimizer_hints distinct_clause1 opt_cache_clause     { $$ = $2; }
            | opt_optimizer_hints opt_cache_clause distinct_clause1     { $$ = $3; }
        ;
...
separator_clause:
			SEPARATOR Sconst					    { $$ = mys_makeStringConst($2, @2); }
			| /*EMPTY*/								{ $$ = mys_makeStringConst(",", -1); }
		;
			...
			| FETCH first_or_next row_or_rows ONLY
				{
					SelectLimit *n = (SelectLimit *) palloc(sizeof(SelectLimit));
					n->limitOffset = NULL;
					n->limitCount = mys_makeIntConst(1, -1);
					n->limitOption = LIMIT_OPTION_COUNT;
					$$ = n;
				}
...
I_or_F_const:
			Iconst									{ $$ = mys_makeIntConst($1,@1); }
			| FCONST								{ $$ = mys_makeFloatConst($1,@1); }
		;
 ...
 */
group_clause:
			GROUP_P BY set_quantifier expr_list WITH_LA ROLLUP
				{
					GroupClause *n = (GroupClause *) palloc(sizeof(GroupClause));
					n->distinct = $3 == SET_QUANTIFIER_DISTINCT;
					n->list = list_make1(makeGroupingSet(GROUPING_SET_ROLLUP, $4, @1));
					$$ = n;
				}
			| GROUP_P BY set_quantifier expr_list
				{
					GroupClause *n = (GroupClause *) palloc(sizeof(GroupClause));
					n->distinct = $3 == SET_QUANTIFIER_DISTINCT;
					n->list = $4;
...
opt_force_index_clause:
			force_index								
			| opt_force_index_clause force_index			
        ;
...
opt_alias_clause: 
            alias_clause						    { $$ = $1; }
			| opt_force_index_clause			    { $$ = NULL; }
			| alias_clause opt_force_index_clause	{ $$ = $1; }
			| /*EMPTY*/								{ $$ = NULL; }
		;
...
SimpleTypename:
            ...
            | BINARY                                {
                ...
                $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                         makeString("binary")));
                //$$->typmods = list_make1(mys_makeIntConst(1, -1));
                }
            | BINARY '(' Iconst ')'                 {
                //$$ = SystemTypeName("bytea");
                $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                         makeString("binary")));
                //$$->typmods = list_make1(mys_makeIntConst($3, -1));
                }
            | VARBINARY '(' Iconst ')'              {
                //$$ = SystemTypeName("bytea");
                $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                         makeString("varbinary")));
                //$$->typmods = list_make1(mys_makeIntConst($3, -1));
                }
            | YEAR_P opt_field_length opt_field_option
                {
                    if ( $2 != NULL)
                    {
                        ulong length;
                        ...
                        if (errno != 0 || length != 4)
                        {
                            ...
                            ereport(ERROR,
                                (errcode(ERRCODE_SYNTAX_ERROR),
                                 errmsg("Invalid display width. Use YEAR instead."),
                                 parser_errposition(@1)));
                        }
                    }

                    $$ = makeTypeName(pstrdup("year_"));
                    $$->location = @1;
                }
			| Bit									{ $$ = $1; }
            | Blob                                  { $$ = $1; }
			...
			| ConstInterval '(' Iconst ')'
				{
					$$ = $1;
					$$->typmods = list_make2(mys_makeIntConst(INTERVAL_FULL_RANGE, -1),
											 mys_makeIntConst($3, @3));
				} */
		;
 ...
 */
Numeric:
            TINYINT opt_field_length opt_field_option
                {
                    if (($2 != NULL) && (strncmp($2, "1", 1) == 0))
                    {
                        if (strncasecmp($3, "unsigned", 8) == 0)
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                     makeString("tinyint1 unsigned")));
                        }
                        else if (strncasecmp($3, "signed", 6) == 0)
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                     makeString("tinyint1 signed")));
                        }
                        else
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                    ...
                    else 
                    {
                        ...
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                     makeString("tinyint unsigned")));
                        }
                        else if (strncasecmp($3, "signed", 6) == 0)
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                     makeString("tinyint signed")));
                        }
                        else
                        {
                            $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
			...
			| SMALLINT opt_field_length opt_field_option
				{
					if (strncasecmp($3, "unsigned", 8) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("smallint unsigned")));
                    }
                    else if (strncasecmp($3, "signed", 6) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("smallint signed")));
                    }
                    else
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
            ...
            | MEDIUMINT opt_field_length opt_field_option
                {
					if (strncasecmp($3, "unsigned", 8) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("mediumint unsigned")));
                    }
                    else if (strncasecmp($3, "signed", 6) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("mediumint signed")));
                    }
                    else
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("mediumint")));
                    }
					$$->location = @1;
                }
            | INT_P opt_field_length opt_field_option
				{
                    if (strncasecmp($3, "unsigned", 8) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("int unsigned")));
                    }
                    else if (strncasecmp($3, "signed", 6) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("int signed")));
                    }
                    else
                    {
                        $$ = SystemTypeName("int4");
			...
			| INTEGER opt_field_length opt_field_option
				{
					if (strncasecmp($3, "unsigned", 8) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
			...
			| BIGINT opt_field_length opt_field_option
				{
					if (strncasecmp($3, "unsigned", 8) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("bigint unsigned")));
                    }
                    else if (strncasecmp($3, "signed", 6) == 0)
                    {
                        $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                                 makeString("bigint signed")));
                    }
                    else
                    {
                        $$ = SystemTypeName("int8");
			...
			| REAL opt_type_modifiers opt_field_option
				{
                    // TODO: MYSQL根据sql_mode是否有REAL_AS_FLOAT，将REAL视为float或double
                    //      real(M, D), unsigned怎么处理
                    // col_real real, col_realn2 real(8, 2)
                    $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                             makeString("real")));
					$$->location = @1;
				}
			| FLOAT_P opt_float opt_field_option
				{
                    // TODO: unsigned怎么处理
					$$ = $2;
					$$->location = @1;
				}
			| DOUBLE_P opt_PRECISION opt_precision opt_field_option
				{
                    // TODO: double(M,D), unsigned怎么处理
					// $$ = SystemTypeName("float8");
                    $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                                                             makeString("double")));
					$$->location = @1;
				}
			| DECIMAL_P opt_type_modifiers opt_field_option
				{
					$$ = SystemTypeName("numeric");
					$$->typmods = $2;
					$$->location = @1;
				}
			| DEC opt_type_modifiers opt_field_option
				{
					$$ = SystemTypeName("numeric");
					$$->typmods = $2;
					$$->location = @1;
				}
			| NUMERIC opt_type_modifiers opt_field_option
				{
					$$ = SystemTypeName("numeric");
					$$->typmods = $2;
					$$->location = @1;
...
opt_field_option:
             /* empty */    { $$ = pstrdup(""); }
            | SIGNED        { $$ = pstrdup("signed"); }
            | UNSIGNED      { $$ = pstrdup("unsigned"); }
        ;
            ...
            //| DATE    // 直接用DATE会冲突
                /* {
                    ...
                    else
                    {
                        ereport(ERROR,
                                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                                 errmsg("You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near '%s) as con' at line 1", $1),
                                 parser_errposition(@1)));
                    }
                } */
            | BINARY
            ...
            BIT opt_varying '(' ICONST ')'
				{
                    ...
                    $$->typmods = list_make1(mys_makeIntConst($4, -1));
                    $$->location = @1;
				}
			| BIT opt_varying
				{
...
Text:
            LONG varchar opt_charset_with_opt_binary
                {
                    ...
                    // {
                    //    Node *n = linitial($3);
                    //    if (nodeTag(n) == T_String)
                    //    {
                    //        Value *v = (Value *)n;

                    //        if (strncasecmp(strVal(v), "binary", 6) == 0)
                    //        {
                    //            $$ = SystemTypeName("bytea");
                    //        }
                    //    }
            ...
            | TEXT_P opt_field_length opt_charset_with_opt_binary
                {
                    //$$ = NULL;
                    ...
                    //            $$ = SystemTypeName("bytea");
                    //        }
                    //        //else if (strncasecmp(strVal(v), "byte", 4) == 0)
                    //        //{
                    //        //    //$$ = SystemTypeName("bytea");
                    //        //    $$ = makeTypeNameFromNameList(list_make2(makeString("mysql"),
                    //        //                                             makeString("text byte")));
                    //        //}
                    //    }
                    //}
                    //
...
CharacterWithoutLength:	 character opt_charset_with_opt_binary
				{
                    //$$ = NULL;
...
opt_charset_with_opt_binary:
            ...
            //| BYTE
            //    {
            //        // TODO: char, varchar...后跟byte, 需要转化为相应的二进制类型(binary, varbinary)
            //        $$ = list_make1(makeString(pstrdup("binary")));
            //        //$$ = list_make1(makeString(pstrdup("byte")));
            //    }
            | character_set charset_name opt_bin_mod
                {
                    // TODO: 字符集为$2, opt_bin_mod用于确定是否使用该字符集的二进制排序规则
			...
			| TIMESTAMP opt_timezone
				{
					if ($2)
						...
						$$ = SystemTypeName("timestamp");
                    $$->typmods = list_make1(mys_makeIntConst(0, -1));
					$$->location = @1;
				}
			| DATETIME '(' Iconst ')'
				{
...
opt_interval:
			YEAR_P
				{ $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(YEAR), @1)); }
			| MONTH_P
				{ $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(MONTH), @1)); }
			| DAY_P
				{ $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(DAY), @1)); }
			| HOUR_P
				{ $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(HOUR), @1)); }
			| MINUTE_P
				{ $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(MINUTE), @1)); }
			| interval_second
				{ $$ = $1; }
			| YEAR_P TO MONTH_P
				{
					$$ = list_make1(mys_makeIntConst(INTERVAL_MASK(YEAR) |
												 INTERVAL_MASK(MONTH), @1));
				}
			| DAY_P TO HOUR_P
				{
					$$ = list_make1(mys_makeIntConst(INTERVAL_MASK(DAY) |
			...
			| DAY_P TO interval_second
				{
					$$ = $3;
					linitial($$) = mys_makeIntConst(INTERVAL_MASK(DAY) |
												INTERVAL_MASK(HOUR) |
			...
			| HOUR_P TO MINUTE_P
				{
					$$ = list_make1(mys_makeIntConst(INTERVAL_MASK(HOUR) |
												 INTERVAL_MASK(MINUTE), @1));
				}
			| HOUR_P TO interval_second
				{
					$$ = $3;
					linitial($$) = mys_makeIntConst(INTERVAL_MASK(HOUR) |
												INTERVAL_MASK(MINUTE) |
												INTERVAL_MASK(SECOND), @1);
				}
			| MINUTE_P TO interval_second
				{
					$$ = $3;
					linitial($$) = mys_makeIntConst(INTERVAL_MASK(MINUTE) |
												INTERVAL_MASK(SECOND), @1);
				}
			| /*EMPTY*/
				{ $$ = NIL; }
...
interval_second:
			SECOND_P
				{
					$$ = list_make1(mys_makeIntConst(INTERVAL_MASK(SECOND), @1));
				}
			| SECOND_P '(' Iconst ')'
				{
					$$ = list_make2(mys_makeIntConst(INTERVAL_MASK(SECOND), @1),
									mys_makeIntConst($3, @3));
				}
		;
...
a_expr:		c_expr									{ $$ = $1; }
		 ...
		 */
            ...
            | a_expr '+' interval_expr
                {
                    TypeCast *expr2Text = NULL;

                    if (IsA($1, A_Const))
                    {
                        A_Const *n = (A_Const *) $1;
                        if (n->val.sval.type == T_String)
                        {
                            TypeName *textType = makeTypeName(pstrdup("text"));
                            textType->typmods = NIL;
					        textType->location = -1;

                            expr2Text = makeNode(TypeCast);
                            expr2Text->arg = $1;
                            expr2Text->typeName = textType;
                            expr2Text->location = -1;
                        }
                    }
                    if (expr2Text != NULL)
                    {
                        $$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "+", (Node *)expr2Text, $3, @2);
                    }
                    else
                    {
                        $$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "+", $1, $3, @2);
            ...
            | interval_expr '+' a_expr
                {
                    ...
                    if (expr2Text != NULL)
                    {
                        $$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "+", $1, (Node *)expr2Text, @2);
                    }
                    else
                    {
                        $$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "+", $1, $3, @2);
			...
			| a_expr REGEXP BINARY_LA a_expr       %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("~")), $1, $4, @2); }
			| a_expr REGEXP a_expr               %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("~*")), $1, $3, @2); }
			| a_expr RLIKE a_expr       %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("~*")), $1, $3, @2); }
			| a_expr NOT REGEXP BINARY_LA a_expr       %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("!~")), $1, $5, @2); }
			| a_expr NOT REGEXP a_expr               %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("!~*")), $1, $4, @2); }
			| a_expr NOT RLIKE a_expr       %prec Op
				{ $$ = (Node *) makeA_Expr(AEXPR_OP, list_make1(makeString("!~*")), $1, $4, @2); }
			| a_expr AND a_expr
				{
					$$ = makeAndExpr($1, $3, @2);
			...
			| a_expr OR a_expr
				{ 
                    $$ = makeOrExpr($1, $3, @2); 
					/* $$ = (Node *) makeSimpleA_Expr(AEXPR_OP, "||", $1, $3, @2); */
                    /* $$ = (Node *) makeA_Expr(AEXPR_OP, list_make2(makeString("mysql"), makeString("||")), $1, $3, @2); */
                }
			| NOT a_expr
				{ $$ = makeNotExpr($2, @1); }
			...
			| a_expr IS unicode_normal_form NORMALIZED			%prec IS
				{
					$$ = (Node *) makeFuncCall(SystemFuncName("is_normalized"),
											   list_make2($1, mys_makeStringConst($3, @3)),
											   COERCE_SQL_SYNTAX,
											   @2);
				}
			...
			| a_expr IS NOT unicode_normal_form NORMALIZED		%prec IS
				{
					$$ = makeNotExpr((Node *) makeFuncCall(SystemFuncName("is_normalized"),
														   list_make2($1, mys_makeStringConst($4, @4)),
														   COERCE_SQL_SYNTAX,
														   @2),
									 @2);
				}
...
interval_expr:
            INTERVAL a_expr interval_option
                {
                    Node *result;
                    TypeName *finalType;
                    ...
                    if (nodeTag(linitial($3)) == T_String)
                    {
                        Node *unit = (Node *)linitial($3);
                    ...
                    finalType->location = @1;
                    finalType->typmods = intervalTypeMods;

                    transferType = makeTypeName(pstrdup("text"));
                    transferType->typmods = NIL;
					transferType->location = -1;

                    expr2TransferType = makeNode(TypeCast);
                    expr2TransferType->arg = $2;
                    expr2TransferType->typeName = transferType;
                    ...
                    // 暂时没有multiple 与 divisor同时大于0的情况
                    if (multiple > 0)
                    {
                        result = (Node *) makeSimpleA_Expr(AEXPR_OP, "*", (Node *)transferType2Interval, (Node *)mys_makeIntConst(multiple, -1), -1);
                    }
                    else if (divisor > 0)
                    {
                        TypeCast *int2Numeric = makeNode(TypeCast);
                        TypeName *numericType = makeTypeName(pstrdup("numeric"));

                        numericType->typmods = NIL;
					    numericType->location = -1;

                        int2Numeric->arg = (Node *)mys_makeIntConst(divisor, -1);
                        int2Numeric->typeName = numericType;
                        int2Numeric->location = -1;

                        result = (Node *) makeSimpleA_Expr(AEXPR_OP, "/", (Node *)transferType2Interval, (Node *)int2Numeric, -1);
                    }
                    else
                    {
                        result = (Node *) transferType2Interval;
                    }
            ...
            IDENT
                {
                    ...
                    else if (strcmp($1, "quarter") == 0)
                    {
                        $$ = list_make1(makeString($1));
                    }
                    else if (strcmp($1, "microsecond") == 0)
                    {
                        $$ = list_make1(makeString($1));
                    }
                    else if (strcmp($1, "second_microsecond") == 0)
                    {
                        $$ = list_make2(mys_makeIntConst(INTERVAL_MASK(SECOND), @1),
									    mys_makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "minute_microsecond") == 0)
                    {
                        $$ = list_make2(mys_makeIntConst(INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    mys_makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "minute_second") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "hour_microsecond") == 0)
                    {
                        $$ = list_make2(mys_makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    mys_makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "hour_second") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "hour_minute") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE), @1));
                    }
                    else if (strcmp($1, "day_microsecond") == 0)
                    {
                        $$ = list_make2(mys_makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1),
									    mys_makeIntConst(6, @1));
                    }
                    else if (strcmp($1, "day_second") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND), @1));
                    }
                    else if (strcmp($1, "day_minute") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE), @1));
                    }
                    else if (strcmp($1, "day_hour") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR), @1));
                    }
                    else if (strcmp($1, "year_month") == 0)
                    {
                        $$ = list_make1(mys_makeIntConst(INTERVAL_MASK(YEAR) | INTERVAL_MASK(MONTH), @1));
                    }
                    else
                    {
                        ereport(ERROR,
                                (errcode(ERRCODE_SYNTAX_ERROR),
                                 errmsg("不支持的interval unit: %s.", $1),
                                 parser_errposition(@1)));
                    }
                }
    		| YEAR_P
...
func_expr_common_subexpr:
            ...
            | DATE '(' a_expr ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("date")),
                                               list_make1($3),
                                               COERCE_EXPLICIT_CALL,
                                               @1);
                }
            | UTC_DATE
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("utc_date")),
                                               NIL,
                                               COERCE_EXPLICIT_CALL,
                                               @1);
                }
            ...
            | UTC_TIME
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("utc_time")),
                                               NIL,
                                               COERCE_EXPLICIT_CALL,
                                               @1);
                }
            ...
            | UTC_TIME '(' Iconst ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("utc_time")),
                                               list_make1(mys_makeIntConst($3, @3)),
                                               COERCE_EXPLICIT_CALL,
                                               @1);
                }
            | UTC_TIMESTAMP
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("utc_timestamp")),
                                               NIL,
                                               COERCE_EXPLICIT_CALL,
                                               @1);
                }
			...
			| CURRENT_TIME
				{
					...
					$$ = makeSQLValueFunction(SVFOP_LOCALTIME_N, 0, @1);
				}
            | CURRENT_TIME '(' ')'
				{
					//$$ = makeSQLValueFunction(SVFOP_CURRENT_TIME_N, 0, @1);
					$$ = makeSQLValueFunction(SVFOP_LOCALTIME_N, 0, @1);
				}
			| CURRENT_TIME '(' Iconst ')'
				{
			...
			| CURRENT_TIMESTAMP
				{
					...
					$$ = makeSQLValueFunction(SVFOP_LOCALTIMESTAMP_N, 0, @1);
				}
            | CURRENT_TIMESTAMP '(' ')'
                {
                    //$$ = makeSQLValueFunction(SVFOP_CURRENT_TIMESTAMP, 0, @1);
					$$ = makeSQLValueFunction(SVFOP_LOCALTIMESTAMP_N, 0, @1);
                }
			| CURRENT_TIMESTAMP '(' Iconst ')'
				{
			...
			| CURRENT_USER
				{
                    ...
                    $$ = (Node *) makeFuncCall(list_make2(makeString(pstrdup("mysql")), makeString(pstrdup("current_user"))),
											   NIL,
											   COERCE_EXPLICIT_CALL,
											   @1);
				}
			...
			| CAST '(' a_expr AS MySQLConvertType ')'
				{
                    $$ = mys_makeTypeCast($3, $5, @1);
                }
			| CONVERT '(' a_expr USING IDENT ')'
                {
					$$ = (Node *) makeFuncCall(SystemFuncName("convert_to"),
											   list_make2($3, mys_makeStringConst($5, @5)),
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
			...
			| CONVERT '(' a_expr ',' MySQLConvertType ')'
				{
                    $$ = mys_makeTypeCast($3, $5, @1);
                }
			| EXTRACT '(' extract_list ')'
				{
					$$ = (Node *) makeFuncCall(list_make1(makeString(pstrdup("extract"))),
											   $3,
											   COERCE_SQL_SYNTAX,
											   @1);
				}
			| TIMESTAMP '(' a_expr ',' a_expr ')'
				{
					$$ = (Node *) makeFuncCall(list_make1(makeString(pstrdup("timestamp"))),
											   list_make2($3, $5),
											   COERCE_SQL_SYNTAX,
											   @1);
				}
			| STD '(' a_expr ')'
				{
					$$ = (Node *) makeFuncCall(SystemFuncName("stddev_pop"),
											   list_make1($3),
											   COERCE_SQL_SYNTAX,
											   @1);
				}
            ...
            | MOD '(' a_expr ',' a_expr ')'
                {
                    //$$ = (Node *) makeFuncCall(list_make2(makeString(pstrdup("mysql")), makeString(pstrdup("mod"))),
					//						   list_make2($3, $5),
					//						   COERCE_EXPLICIT_CALL,
					//						   @1);
                    $$ = (Node *) makeFuncCall(list_make1(makeString("mod")),
											   list_make2($3, $5),
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
            ...
            | SESSION_USER '(' ')'
                {
                    $$ = (Node *) makeFuncCall(list_make2(makeString(pstrdup("mysql")), makeString(pstrdup("session_user"))),
											   NIL,
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
            ...
            //| USER '(' ')'
            //    {
            //        $$ = (Node *) makeFuncCall(list_make1(makeString("user")),
			//								   NIL,
			//								   COERCE_EXPLICIT_CALL,
			//								   @1);
            //    }
            | LEFT '(' a_expr ',' a_expr ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("left")),
											   list_make2($3, $5),
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
            | RIGHT '(' a_expr ',' a_expr ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("right")),
											   list_make2($3, $5),
											   COERCE_EXPLICIT_CALL,
											   @1);
                }
            | TIMESTAMPADD '(' timestampadd_list ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("timestampadd")),
											   $3,
											   COERCE_SQL_SYNTAX,
											   @1);
                }
            | TIMESTAMPDIFF '(' timestampdiff_list ')'
                {
                    $$ = (Node *) makeFuncCall(list_make1(makeString("timestampdiff")),
											   $3,
											   COERCE_SQL_SYNTAX,
											   @1);
                }
			...
			| SUBSTR '(' substr_list ')'
				{
					 ...
					 */
					//$$ = (Node *) makeFuncCall(SystemFuncName("substring"),
					$$ = (Node *) makeFuncCall(list_make1(makeString("substr")),
											   $3,
											   COERCE_SQL_SYNTAX,
											   @1);
				}
			| SUBSTR '(' func_arg_list_opt ')'
				{
					/*
			...
			| XMLPARSE '(' document_or_content a_expr xml_whitespace_option ')'
				{
					XmlExpr *x = (XmlExpr *)
						makeXmlExpr(IS_XMLPARSE, NULL, NIL,
									list_make2($4, (Node *) mys_makeBoolAConst($5, -1)),
									@1);
					x->xmloption = $3;
					$$ = (Node *)x;
				}
...
opt_xml_root_standalone: ',' STANDALONE_P YES_P
				{ $$ = mys_makeIntConst(XML_STANDALONE_YES, -1); }
			| ',' STANDALONE_P NO
				{ $$ = mys_makeIntConst(XML_STANDALONE_NO, -1); }
			| ',' STANDALONE_P NO VALUE_P
				{ $$ = mys_makeIntConst(XML_STANDALONE_NO_VALUE, -1); }
			| /*EMPTY*/
				{ $$ = mys_makeIntConst(XML_STANDALONE_OMITTED, -1); }
		;
...
extract_list:
			extract_arg FROM a_expr
				{
					$$ = list_make2(mys_makeStringConst($1, @1), $3);
				}
		;
...
array_agg_list:
            func_arg_list opt_sort_clause
                {
                    FuncCall *n = makeFuncCall(SystemFuncName("array_agg"),
                                    $1,
                                    COERCE_EXPLICIT_CALL,
                                    @1);
                    n->agg_order = $2;
...
timestampadd_list:
            extract_arg ',' a_expr ',' a_expr
                {
                    $$ = list_make3(mys_makeStringConst($1, @1), $3, $5);
                }
        ;
			...
			| a_expr FOR a_expr
				{
					 ...
					 */
					$$ = list_make3($1, mys_makeIntConst(1, -1),
									mys_makeTypeCast($3,
...
columnref:	ColId
            ...
            | USER
				{
					$$ = mys_makeColumnRef(pstrdup("user"), NIL, @1, yyscanner);
				}
			| ColId indirection
				{
					$$ = mys_makeColumnRef($1, $2, @1, yyscanner);
				}
			| USER indirection
				{
					$$ = mys_makeColumnRef(pstrdup("user"), $2, @1, yyscanner);
				}
		;
            ...
            | ColId indirection
				{
					...
					switch (list_length($2))
					{
						case 1:
							$$->catalogname = NULL;
							$$->schemaname = $1;
                            if (memcmp($$->schemaname, "information_schema", 18) == 0)
                            {
                                $$->schemaname = pstrdup("mys_informa_schema");
                            }
                            else if ((memcmp($$->schemaname, "sys", 3) == 0) && (strlen($$->schemaname) == 3))
                            {
                                $$->schemaname = pstrdup("mys_sys");
                            }
...
AexprConst: Iconst
				{
					$$ = mys_makeIntConst($1, @1);
				}
			| FCONST
				{
					$$ = mys_makeFloatConst($1, @1);
				}
			| Sconst
				{
					$$ = mys_makeStringConst($1, @1);
            ...
            | UNDERSCORE_BINARY BCONST
                {
                    $$ = mys_makeBitStringConst($2, @2);
                }
            | UNDERSCORE_BINARY XCONST
                {
                    $$ = mys_makeByteaStringConst($2, @2);
                }
			//| func_name Sconst
			//	{
			...
			//		t->typmods = $3;
			//		t->location = @1;
			//		$$ = mys_makeStringConstCast($6, @6, t);
			//	}
			//| ConstTypename Sconst
			//	{
			//		$$ = mys_makeStringConstCast($2, @2, $1);
			//	}
            /* | TIMESTAMP '(' Sconst ')'
                {
                    TypeName *t = SystemTypeName("timestamp");

                    $$ = (Node *)mys_makeStringConstCast($3, @3, t);
                } */
            | TIMESTAMP '(' a_expr ')'
                {
                    TypeName *t = SystemTypeName("timestamp");

                    $$ = (Node *)mys_makeTypeCast($3, t, @3);
                }
            // 为 select id 'Ab' 让路（别名前无as，且别名有引号）
            | TIMESTAMP Sconst
                {
                    TypeName *t = SystemTypeName("timestamp");

                    $$ = (Node *)mys_makeStringConstCast($2, @2, t);
                }
            | TIME '(' a_expr ')'
                {
                    TypeName *t = SystemTypeName("time");
            ...
            | ConstInterval Iconst opt_interval
                {
                    ...
                    $$ = (Node *)mys_makeStringConstCast(s, @2, t);
                }
			| ConstInterval '(' Iconst ')' Sconst
				{
					TypeName *t = $1;
			...
			| TRUE_P
				{
					$$ = (Node *) mys_makeBoolAConst(true, @1);
				}
			| FALSE_P
				{
					$$ = (Node *) mys_makeBoolAConst(false, @1);
				}
			| NULL_P
				{
					$$ = mys_makeNullAConst(@1);
...
PLpgSQL_Expr: distinct_clause opt_target_list
...
type_function_name:	IDENT							{ $$ = $1; }
			| unreserved_keyword					{ $$ = pstrdup($1); }
			//| type_func_name_keyword				{ $$ = pstrdup($1); }   // 为binary类型让路
		;
...
BareColLabel:	IDENT								{ $$ = $1; }
			...
			| YEAR_P					            { $$ = pstrdup("year"); }
			| MONTH_P					            { $$ = pstrdup("month"); }
			| DAY_P					                { $$ = pstrdup("day"); }
			| HOUR_P					            { $$ = pstrdup("hour"); }
			| MINUTE_P					            { $$ = pstrdup("minute"); }
			| SECOND_P					            { $$ = pstrdup("second"); }
		;
 ...
 *
 * Make sure that each keyword's category in mysql/mys_kwlist.h matches where
 * it is listed here.  (Someday we may be able to generate these lists and
 * mysql/mys_kwlist.h's table from one source of truth.)
 */
...
col_name_keyword:
			...
			| SUBSTRING
			| TIME
			| TIMESTAMP %prec KEYWORD_USED_AS_IDENT
            | TIMESTAMPADD
            | TIMESTAMPDIFF
			| TREAT
			| TRIM
 ...
 * in mysql/mys_kwlist.h if it is included here, or AS_LABEL if it is not.
 */
bare_label_keyword:
			  ABORT_P
			| ABSOLUTE_P
...
mys_yyerror(YYLTYPE *yylloc, core_yyscan_t yyscanner, const char *msg)
...
static Node *
mys_makeColumnRef(char *colname, List *indirection,
			  int location, core_yyscan_t yyscanner)
{
	/*
...
static Node *
mys_makeTypeCast(Node *arg, TypeName *typename, int location)
{
...
static Node *
mys_makeStringConst(char *str, int location)
{
...
static Node *
mys_makeStringConstCast(char *str, int location, TypeName *typename)
{
	Node *s = mys_makeStringConst(str, location);

	return mys_makeTypeCast(s, typename, -1);
}

static Node *
mys_makeIntConst(int val, int location)
{
	A_Const *n = makeNode(A_Const);
...
static Node *
mys_makeFloatConst(char *str, int location)
{
	A_Const *n = makeNode(A_Const);
...
static Node *
mys_makeBitStringConst(char *str, int location)
{
	A_Const *n = makeNode(A_Const);
...
static Node *
mys_makeByteaStringConst(char *str, int location)
{
	A_Const *n = makeNode(A_Const);
...
static Node *
mys_makeNullAConst(int location)
{
	A_Const *n = makeNode(A_Const);
...
static Node *
mys_makeAConst(Node *v, int location)
{
	Node *n;
	...
	{
		case T_Float:
			n = mys_makeFloatConst(castNode(Float, v)->fval, location);
			break;

		case T_Integer:
			n = mys_makeIntConst(castNode(Integer, v)->ival, location);
			break;

		case T_String:
		default:
			n = mys_makeStringConst(castNode(String, v)->sval, location);
			break;
	}
 ...
 */
static Node *
mys_makeBoolAConst(bool state, int location)
{
	A_Const *n = makeNode(A_Const);

	n->val.sval.type = T_String;
	n->val.sval.sval = (state ? "t" : "f");
	n->location = location;

	return mys_makeTypeCast((Node *)n, SystemTypeName("bool"), -1);
}
				   ...
				   core_yyscan_t yyscanner)
{
	...
	ndirectargs =  (Node *)  (Node *) makeInteger(list_length(directargs));
					...
					core_yyscan_t yyscanner)
{
	...
	if (sortClause)
	{
        ...
        {
            ++col_num;
            res = (ResTarget*) lfirst(l);
            if (nodeTag(res->val) == T_FuncCall)
            {
                colname = FigureColname(res->val);
                if ((colname != NULL) &&
                    ((strncmp(colname, "count", 5) == 0) ||
                     (strncmp(colname, "sum", 3) == 0) ||
                     (strncmp(colname, "min", 3) == 0) ||
                     (strncmp(colname, "max", 3) == 0) ||
                     (strncmp(colname, "avg", 3) == 0)))
                {
                    ++aggregation_func_num;
                }
            }
        }

        if ((0 < aggregation_func_num) &&
            (aggregation_func_num == col_num) &&
            (!stmt->groupClause))
        {
	...
	if (limitClause && limitClause->limitOption != LIMIT_OPTION_COUNT)
	{
		if (stmt->limitOption)
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
				 ...
				 core_yyscan_t yyscanner)
{
	...
	*collClause = NULL;
	foreach(cell, qualList)
	{
		...
		if (IsA(n, Constraint) || IsA(n, CommentStmt) || (nodeTag(n) == T_String))
		{
			/* keep it in list */
			continue;
		}
 ...
 */
void
mys_parser_init(mys_yy_extra_type *yyext)
{
...
static ColumnDef *
createColumnDef(char *columnName, char *columnType, int typeMod)
{
    ColumnDef *columnDef = makeNode(ColumnDef);

    columnDef->colname = pstrdup(columnName);
    columnDef->typeName = SystemTypeName(columnType);
    if (0 < typeMod)
    {
        columnDef->typeName->typmods = list_make1(mys_makeIntConst(typeMod, -1));
    }
    else 
    {
        columnDef->typeName->typmods = NIL;
    }
    columnDef->typeName->arrayBounds = NULL;
    columnDef->inhcount = 0;
    columnDef->is_local = true;
    columnDef->is_not_null = false;
    columnDef->is_from_type = false;
    columnDef->storage = 0;
    columnDef->raw_default = NULL;
    columnDef->cooked_default = NULL;
    columnDef->collClause = NULL;
    columnDef->collOid = InvalidOid;
...
rectifyColumnsName(Node *node)
{
    ...
    if (node != NULL)
    {
        ...
        {
            ListCell *lc;
            ColumnRef *columnRef = castNode(ColumnRef, node);
            foreach (lc, columnRef->fields)
            {
                Node *val = (Node *)lfirst(lc);
                if (val->type == T_String)
                {
                    if (('a' <= strVal(val)[0]) && (strVal(val)[0] <= 'z'))
                    {
                        strVal(val)[0] -= 32;
                    }
                }
        ...
        {
            A_Expr *aExpr = castNode(A_Expr, node);
            rectifyColumnsName((Node *)aExpr->lexpr);
            rectifyColumnsName((Node *)aExpr->rexpr);
        }
        else if (IsA(node, BoolExpr))
        {
            ...
            {
                BoolExpr *subBoolExpr = (BoolExpr *)lfirst(lc);
                rectifyColumnsName((Node *)subBoolExpr);
            }
        }
    }
}
...
static ResTarget *
createResTargetWithColumn(char *name, char *val, core_yyscan_t yyscanner)
{
    ResTarget *resTarget;
    ...
    resTarget->indirection = NIL; 
    resTarget->val = (Node *)mys_makeColumnRef(val, NIL, -1, yyscanner);
    resTarget->location = -1; 
...
static ResTarget *
createResTargetWithString(char *name, char *val)
{
    ResTarget *resTarget;
    ...
    if (val != NULL)
    {
        resTarget->val = (Node *)mys_makeStringConst(pstrdup(val), -1);
    }
    else 
    {
        resTarget->val = (Node *)mys_makeNullAConst(-1);
    }
    resTarget->location = -1; 
...
static ResTarget *
createResTargetWithInt(char *name, int val)
{
    ResTarget *resTarget;
    ...
    resTarget->indirection = NIL; 
    resTarget->val = (Node *)mys_makeIntConst(val, -1);
    resTarget->location = -1; 
...
//static ResTarget *
//createResTargetWithNULL(char *name)
//{
//    ResTarget *resTarget;
//
//    resTarget = makeNode(ResTarget);
...
static ResTarget *
createResTargetWithFunc2Args(char *name, char *funcShemaName, char *funcName, 
                             char *arg1, char *arg2)
{
    ResTarget *resTarget;
    FuncCall *funcCall;
    ...
    resTarget->indirection = NIL; 
    funcNames = list_make2(makeString(pstrdup(funcShemaName)), makeString(pstrdup(funcName)));
    funcArgs = NIL;
    if (arg1 != NULL)
    {
        funcArgs = lappend(funcArgs, mys_makeStringConst(pstrdup(arg1), -1));
    }
    else 
    {
        funcArgs = lappend(funcArgs, mys_makeNullAConst(1));
    }
    if (arg2 != NULL)
    {
        funcArgs = lappend(funcArgs, mys_makeStringConst(pstrdup(arg2), -1));
    }
    else 
    {
        funcArgs = lappend(funcArgs, mys_makeNullAConst(1));
    }
    funcCall = makeFuncCall(funcNames, funcArgs, COERCE_EXPLICIT_CALL, -1);
...
static RangeVar *
createRangeVar(char *schemaName, char *tableName)
{
    RangeVar *rangeVar;

    rangeVar = makeRangeVar(NULL, NULL, -1);
    if (schemaName != NULL)
    {
        rangeVar->schemaname = pstrdup(schemaName);
    }
    else 
    {
        rangeVar->schemaname = pstrdup(get_namespace_name(getCurrentNamespaceOid()));
    }
    rangeVar->relname = pstrdup(tableName);
    rangeVar->inh = true;
    rangeVar->alias = NULL;
...
static void
checkJoinOn(JoinExpr *joinExpr, bool *hasNotOn)
{
    if (IsA(joinExpr->larg, JoinExpr))
    {
        checkJoinOn(castNode(JoinExpr, joinExpr->larg), hasNotOn);
    }
    if (IsA(joinExpr->rarg, JoinExpr))
    {
        checkJoinOn(castNode(JoinExpr, joinExpr->rarg), hasNotOn);
    }
...
static void
makeOnClause(Node **expr, JoinExpr *joinExpr)
{
    if (IsA(joinExpr->larg, JoinExpr))
    {
        makeOnClause(expr, castNode(JoinExpr, joinExpr->larg));
    }
    if (IsA(joinExpr->rarg, JoinExpr))
    {
        makeOnClause(expr, castNode(JoinExpr, joinExpr->rarg));
    }
    ...
    if (joinExpr->quals != NULL)
    {
        if (*expr != NULL)
        {
            *expr = makeAndExpr(*expr, (Node *)(joinExpr->quals), -1);
        }
        else 
        {
            *expr = joinExpr->quals;
