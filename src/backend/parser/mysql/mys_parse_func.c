 ...
 *
 * mys_parse_func.c
 *    MySQL-specific function/operator resolution.
 *
                      ...
                      FuncCall *fn, bool proc_call, int location)
{
	...
	if (!proc_call && list_length(funcname) == 1)
	{
		char	   *fname = strVal(linitial(funcname));

		if (pg_strcasecmp(fname, "json_object") == 0)
			funcname = list_make1(makeString("mys_json_object"));
		else if (pg_strcasecmp(fname, "json_array") == 0)
			funcname = list_make1(makeString("json_build_array"));
	}
	...
	else
	{
		...
		/* M3: preTransformWindowAgg (openHalo callback) not in PG18. Window-func setup is done inline. */
