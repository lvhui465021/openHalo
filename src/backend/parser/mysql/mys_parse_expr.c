 ...
 *
 * mys_parse_expr.c
 *    MySQL-specific expression transformation.
 *
 * Portions Copyright (c) 2026, HaloLab / UDB-TX Contributors
 *
 * src/backend/parser/mysql/mys_parse_expr.c
 *
 *-------------------------------------------------------------------------
 */
...
static Node *transformUserVarRef(ParseState *pstate, UserVarRef *userVarRef);
static Node *transformUserVarAssign(ParseState *pstate, UserVarAssign *userVarAssign);
static Node *transformSysVarRef(ParseState *pstate, SysVarRef *sysVarRef);
...
static Oid mys_select_common_type_case(ParseState *pstate, List *exprs, const char *context, Node **which_expr);
static Node *mys_coerce_to_common_type(ParseState *pstate, Node *node, Oid targetTypeId, const char *context);
static Node *transformSubLink(ParseState *pstate, SubLink *sublink);
static Node *transformArrayExpr(ParseState *pstate, A_ArrayExpr *a,
								Oid array_type, Oid element_type, int32 typmod);
static Node *transformRowExpr(ParseState *pstate, RowExpr *r, bool allowDefault);
static Node *transformCoalesceExpr(ParseState *pstate, CoalesceExpr *c);
static Oid mys_select_common_type_coalesce(ParseState *pstate, List *exprs, const char *context, Node **which_expr);
...
static Const *mys_make_const(ParseState *pstate, A_Const *aconst, int location);
 ...
 */
Node *
mys_transformExpr(ParseState *pstate, Node *expr, ParseExprKind exprKind)
{
	Node	   *result;
...
transformExprRecurse(ParseState *pstate, Node *expr)
{
	...
	switch (nodeTag(expr))
	{
        ...
        case T_UserVarRef:
            result = transformUserVarRef(pstate, (UserVarRef *) expr);
            break;

		case T_UserVarAssign:
			result = transformUserVarAssign(pstate, (UserVarAssign *) expr);
			break;

        case T_SysVarRef:
            result = transformSysVarRef(pstate, (SysVarRef *) expr);
            break;
		...
		case T_A_Const:
			{
				...
				result = (Node *) mys_make_const(pstate, con, con->location);
				break;
			}
...
transformColumnRef(ParseState *pstate, ColumnRef *cref)
{
	...
	if (node == NULL)
	{
		/* M3: finalTransformColumnRef not in PG18 */ node = NULL;
	}
...
static Node *
transformUserVarRef(ParseState *pstate, UserVarRef *userVarRef)
{
    List *funcName = list_make2(makeString(pstrdup("mysql")), makeString(pstrdup("get_user_var")));
    List *argList;
    A_Const *n;
    Node *funcCall;
    ...
    n = makeNode(A_Const);
	n->val.node.type = T_String;
	n->val.sval = *makeString(userVarRef->userVarName);
	n->location = userVarRef->location;

    argList = list_make1(n);
...
{
	List *funcName = list_make2(makeString(pstrdup("mysql")), makeString(pstrdup("set_user_var_in_expr")));
	A_Const *n = NULL;
    List *argList;
    Node *funcCall;
	Node *result;
	...
	n = makeNode(A_Const);
	n->val.node.type = T_String;
	n->val.sval = *makeString(userVarAssign->userVarName);
	n->location = userVarAssign->location;

    argList = list_make2(n, userVarAssign->expr);

    funcCall = (Node *) makeFuncCall(funcName, argList,
                                     COERCE_EXPLICIT_CALL,
                                     userVarAssign->location);
...
transformCaseExpr(ParseState *pstate, CaseExpr *c)
{
	...
	ptype = mys_select_common_type_case(pstate, resultexprs, "CASE", NULL);
	Assert(OidIsValid(ptype));
	newc->casetype = ptype;
	/* casecollid will be set by parse_collate.c */

	/* Convert default result clause, if necessary */
    newc->defresult = (Expr *)mys_coerce_to_common_type(pstate,
                                                        (Node *) newc->defresult,
                                                        ptype,
                                                        "CASE/ELSE");
	...
	foreach(l, newc->args)
	{
        ...
        w->result = (Expr *)mys_coerce_to_common_type(pstate,
                                                      (Node *) w->result,
                                                      ptype,
                                                      "CASE/WHEN");
    }
...
static Oid
mys_select_common_type_case(ParseState *pstate, List *exprs, const char *context,
                            Node **which_expr)
{
	Node	   *pexpr;
	Oid			ptype;
    ...
    if (strcmp(context, "CASE") == 0)
    {
        ...
        foreach(lc1, exprs)
        {
            Node	   *nexpr = (Node *) lfirst(lc1);
			Oid			ntype_temp = exprType(nexpr);

            if (nodeTag(nexpr) == T_Const && ((const Const *)nexpr)->constisnull)
                continue;
            ...
            else
            {
                if (ntype != ntype_temp)
                {
                    char *tmpTypeName = getTypeName(ntype_temp);
                    char *typeName = getTypeName(ntype);
                    if ((strncmp(tmpTypeName, "int8", 4) == 0) || 
                        (strncmp(tmpTypeName, "bigint signed", 13) == 0) ||
                        (strncmp(tmpTypeName, "bigint unsigned", 15) == 0))
                    {
                        if ((strncmp(typeName, "int8", 4) == 0) || 
                            (strncmp(typeName, "bigint signed", 13) == 0) ||
                            (strncmp(typeName, "bigint unsigned", 15) == 0) || 
                            (strncmp(typeName, "int", 3) == 0) || 
                            (strncmp(typeName, "int4", 4) == 0) ||
                            (strncmp(typeName, "int signed", 10) == 0) ||
                            (strncmp(typeName, "int unsigned", 12) == 0) || 
                            (strncmp(typeName, "mediumint", 9) == 0) || 
                            (strncmp(typeName, "mediumint signed", 16) == 0) || 
                            (strncmp(typeName, "mediumint unsigned", 18) == 0) ||
                            (strncmp(typeName, "smallint", 8) == 0) || 
                            (strncmp(typeName, "smallint signed", 15) == 0) || 
                            (strncmp(typeName, "smallint unsigned", 17) == 0) || 
                            (strncmp(typeName, "tinyint", 7) == 0) || 
                            (strncmp(typeName, "tinyint signed", 14) == 0) || 
                            (strncmp(typeName, "tinyint unsigned", 16) == 0))
                        {
                            ntype = ntype_temp;
                        }
                        else 
                    ...
                    else if ((strncmp(tmpTypeName, "int", 3) == 0) || 
                             (strncmp(tmpTypeName, "int4", 4) == 0) ||
                             (strncmp(tmpTypeName, "int signed", 10) == 0) ||
                             (strncmp(tmpTypeName, "int unsigned", 12) == 0))
                    {
                        if ((strncmp(typeName, "int8", 4) == 0) || 
                            (strncmp(typeName, "bigint signed", 13) == 0) ||
                            (strncmp(typeName, "bigint unsigned", 15) == 0) || 
                        ...
                        else if ((strncmp(typeName, "mediumint", 9) == 0) || 
                                 (strncmp(typeName, "mediumint signed", 16) == 0) || 
                                 (strncmp(typeName, "mediumint unsigned", 18) == 0) ||
                                 (strncmp(typeName, "smallint", 8) == 0) || 
                                 (strncmp(typeName, "smallint signed", 15) == 0) || 
                    ...
                    else if ((strncmp(tmpTypeName, "mediumint", 9) == 0) || 
                             (strncmp(tmpTypeName, "mediumint signed", 16) == 0) || 
                             (strncmp(tmpTypeName, "mediumint unsigned", 18) == 0))
                    {
                        if ((strncmp(typeName, "int8", 4) == 0) || 
                            (strncmp(typeName, "bigint signed", 13) == 0) ||
                            (strncmp(typeName, "bigint unsigned", 15) == 0) || 
                        ...
                        else if ((strncmp(typeName, "smallint", 8) == 0) || 
                                 (strncmp(typeName, "smallint signed", 15) == 0) || 
                                 (strncmp(typeName, "smallint unsigned", 17) == 0) || 
                                 (strncmp(typeName, "tinyint", 7) == 0) || 
                                 (strncmp(typeName, "tinyint signed", 14) == 0) || 
                    ...
                    else if ((strncmp(tmpTypeName, "smallint", 9) == 0) || 
                             (strncmp(tmpTypeName, "smallint signed", 15) == 0) || 
                             (strncmp(tmpTypeName, "smallint unsigned", 17) == 0))
                    {
                        if ((strncmp(typeName, "int8", 4) == 0) || 
                            (strncmp(typeName, "bigint signed", 13) == 0) ||
                            (strncmp(typeName, "bigint unsigned", 15) == 0) || 
                        ...
                        else if ((strncmp(typeName, "tinyint", 7) == 0) || 
                                 (strncmp(typeName, "tinyint signed", 14) == 0) || 
                                 (strncmp(typeName, "tinyint unsigned", 16) == 0))
                        {
                            ntype = ntype_temp;
                    ...
                    else if ((strncmp(tmpTypeName, "tinyint", 7) == 0) || 
                             (strncmp(tmpTypeName, "tinyint signed", 14) == 0) || 
                             (strncmp(tmpTypeName, "tinyint unsigned", 16) == 0))
                    {
                        if ((strncmp(typeName, "int8", 4) == 0) || 
                            (strncmp(typeName, "bigint signed", 13) == 0) ||
                            (strncmp(typeName, "bigint unsigned", 15) == 0) || 
        ...
        return (ntype == UNKNOWNOID || ntype == InvalidOid) ? TEXTOID : ntype;
    } 
	...
	for_each_cell(lc, exprs, lc)
	{
		...
		if (ntype != UNKNOWNOID && ntype != ptype)
		{
			...
			else if (ncategory != pcategory)
			{
				...
				if (context == NULL)
					return InvalidOid;
                if (strcmp(context, "UNION") == 0 && ptype == TEXTOID)
                {
                    pexpr = nexpr;
                    ptype = ntype;
                    pcategory = ncategory;
...
static Node *
mys_coerce_to_common_type(ParseState *pstate, Node *node,
                          Oid targetTypeId, const char *context)
{
    ...
    if ((strcmp(context, "CASE/ELSE") == 0) || 
        (strcmp(context, "CASE/WHEN") == 0) || 
        (strcmp(context, "COALESCE") == 0))
	{
        /* 针对case表达式 类型转换级别调整至显式转换 COERCION_EXPLICIT*/
...
transformCoalesceExpr(ParseState *pstate, CoalesceExpr *c)
{
	...
	newc->coalescetype = mys_select_common_type_coalesce(pstate, newargs, "COALESCE", NULL);
	...
	foreach(args, newargs)
	{
        ...
        newe = mys_coerce_to_common_type(pstate, e,
                                         newc->coalescetype,
                                         "COALESCE");
...
static Oid
mys_select_common_type_coalesce(ParseState *pstate, List *exprs, 
                                const char *context, Node **which_expr)
{
    ...
    {
        /* 
         * If any of the inputs is UNKNOWN type, then resolve as type TEXT.
         * Like SELECT (CASE WHEN foo THEN 1 ELSE 'baz' END);
         */
        if (have_unknown)
        {
            ptype = TEXTOID;
					   ...
					   List *largs, List *rargs, int location)
{
	...
	forboth(l, largs, r, rargs)
	{
        ...
        cmp = castNode(OpExpr, mys_make_op(pstate, opname, larg, rarg,
	...
	foreach(l, opexprs)
	{
		...
		foreach(j, opinfo_lists[i])
		{
			...
			this_strats = bms_add_member(this_strats, opinfo->cmptype);
		}
		if (i == 0)
			strats = this_strats;
		else
				 ...
				 int location)
{
    ...
    result = mys_make_op(pstate, opname, ltree, rtree,
                         pstate->p_last_srf, location);
 ...
 */
static Const *
mys_make_const(ParseState *pstate, A_Const *aconst, int location)
{
	Const	   *con;
	Datum		val;
	int64		val64;
