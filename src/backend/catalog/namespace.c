...
// Oid Cur_package_Oid = InvalidOid;
// Oid last_package_Oid = InvalidOid;
...
GetOverrideSearchPath(MemoryContext context)
{
	...
	while (schemas && linitial_oid(schemas) != activeCreationNamespace)
	{
		...
		else
		{
			...
			Oid spec_oid = get_specific_namespace_oid_by_env();
			if (spec_oid == PG_CATALOG_NAMESPACE)
...
recomputeNamespacePath(void)
{
	...
	Oid 		spec_oid; /* specific namespace OID according by database compatible mode */
	...
	spec_oid = get_specific_namespace_oid_by_env();
	if (spec_oid != PG_CATALOG_NAMESPACE)
	{
		if (!list_member_oid(oidlist, spec_oid))
			oidlist = lcons_oid(spec_oid, oidlist);
	}
...
check_search_path(char **newval, void **extra, GucSource source)
{
        ...
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        ...
        foreach(lc, namelist)
        {
            char *schemaName = (char *) lfirst(lc);
            Oid schemaId = get_namespace_oid(schemaName, true);
            Oid userId = GetUserId();
            AclResult aclresult;
            MemoryContext oldContext;
            /* 像MySQL一样，拒绝（报错）不存在的schema值 */
            if (!OidIsValid(schemaId))
            {
                elog(WARNING, "schema \"%s\" does not exists", schemaName);
                return false;
            }

            /* 检查权限 */
            aclresult = pg_namespace_aclcheck(schemaId, userId, ACL_USAGE);
            if (aclresult != ACLCHECK_OK)
            {
                const char *userName = GetUserNameFromId(userId, false);
                elog(WARNING, "Access denied for user '%s' to database '%s'", userName, schemaName);
                return false;
            }
            ...
            MyProcPort->database_name = pstrdup(schemaName);
...
getCurrentNamespcaeName()
{
	...
	if (activeTempCreationPending)
	{
		/* Need to initialize temp namespace */
		AccessTempTableNamespace(true);
		return get_namespace_name(myTempNamespace);
	}
	namespaceId = activeCreationNamespace;
	if (!OidIsValid(namespaceId))
		ereport(ERROR,
	...
	return get_namespace_name(namespaceId);
}
