...
### 9.7 当前复测(2026-09-06,统一 v2 回放驱动)

用仓库统一驱动(tmp_corpus_equiv.py,parse_faithful 分段 + 逐语句执行 +
.result 双指针对齐;colname_only/warnings_only 单列)重放三份语料。与
§9.1 旧数字的差异来自旧口径的解析/归桶不同(trigger.test 旧口径正
向 1359,本驱动 1220;注释/连接/命令行归类不同),同驱动内可横向比较。

| 语料 | 正向可执行 | 通过(含仅列名/仅警告块) | 严格数据等价(不含显示类) | 失败构成 | 负向 |
|---|---|---|---|---|---|
| sp.test | 2787 | **2196 = 78.8%** | 2085 = 74.8% | mismatch 218 + exec_failed 372 | err 55 / silent 35 |
| trigger.test | 1220 | **936 = 76.9%** | 907 = 74.5% | mismatch 181 + exec_failed 98 | err 84 / silent 24 |
| sp_trans.test | 253 | **193 = 76.3%** | 188 = 74.3% | mismatch 24 + exec_failed 36 | err 16 / silent 0 |
| 合计 | 4257 | **3325 ≈ 78.1%** | 3180 ≈ 74.7% | | err 155 / silent 59(214 条负向中约 72% 会报错) |

要点:
- 引擎非确定性造成每轮 ±0.5–1pp 抖动(sp_trans 两轮 74.7%/76.3%),取最新轮。
- sp.test 相对旧基线(71.6% 严格口径)提升 ~3pp:语句级回滚、SHOW
  WARNINGS 真实诊断区、REPLACE 触发、IF(TEXT) 等提交后的净效果;剩余
  exec_failed 大桶=不存在对象/语法(138+52 条,多为未实现语句面,见
  §9.2/§9.3 清单)与 CALL 挂起。
- 解析/编译接受率仍接近全量(sp 96.9% 编译,trigger/sp_trans 100%)——
  差距集中在行为等价,不是语法接受。
